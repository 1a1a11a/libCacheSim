...
void verify_prefetched_hashtable(gpointer key, gpointer value,
  ...
  if (!g_hash_table_contains(h, key)) {
    ERROR("prefetched %ld not in cacheAlgo %d\n", *(long *)key,
...
//  cacheAlgo->add_only = Mithril_add_only;
//  cacheAlgo->add_withsize = Mithril_add_withsize;
  ...
  /* now adjust the cacheAlgo size by deducting current meta data size
  ...
  if (strcmp(init_params->cache_type, "LRU") == 0)
    ...
    fprintf(stderr, "can't recognize cacheAlgo obj_id_type: %s\n",
...
void Mithril_add_to_prefetch_table(cache_t *Mithril, gpointer gp1,
  ...
  if (prefetch_table_index) {
    ...
    if (Mithril->obj_id_type == OBJ_ID_NUM) {
      ...
      for (i = 1; i < Mithril_params->pf_list_size + 1; i++) {
        // if this element is already in the cacheAlgo, then don't need prefetch
        // again
