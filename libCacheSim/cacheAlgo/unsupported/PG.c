...
//gboolean PG_add_withsize(cache_t *cacheAlgo, request_t *req) {
//  int i;
//  gboolean ret_val;
//
//  *(gint64 *)(req->obj_id_ptr) =
//      (gint64)(*(gint64 *)(req->obj_id_ptr) * req->disk_sector_size /
//               cacheAlgo->block_size);
//  ret_val = PG_add(cacheAlgo, req);
//
//  int n = (int)ceil((double)req->size / cacheAlgo->block_size);
//
//  for (i = 0; i < n - 1; i++) {
//    (*(guint64 *)(req->obj_id_ptr))++;
...
//  cacheAlgo->add_only = PG_add_only;
//  cacheAlgo->add_withsize = PG_add_withsize;
