...
//  if (cp->disk_sector_size != 0 && cacheAlgo->block_size != 0) {
//    *(gint64 *)(cp->obj_id_ptr) =
//        (gint64)(*(gint64 *)(cp->obj_id_ptr) * cp->disk_sector_size /
//                 cacheAlgo->block_size);
//    n = (int)ceil((double)cp->size / cacheAlgo->block_size);
...
//  cacheAlgo->add_only = AMP_add_only;
//  cacheAlgo->add_withsize = AMP_add_withsize;
