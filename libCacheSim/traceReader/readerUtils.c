...
//uint64_t *get_window_boundary(reader_t *reader, guint32 window,
//                            gint32 *n_window) {
//  uint64_t start_ts, end_ts, prev_ts, n_req = 0;
...
//
//  *n_window = (gint32) floor((double) (end_ts - start_ts) / window);
//  uint64_t *boundary = g_new0(uint64_t, *n_window);
//
//  prev_ts = start_ts;
//  uint64_t idx = 0;
//  read_one_req(reader, req);
...
//    *n_window = *n_window - 1;
//    uint64_t *resized_boundary = g_new0(uint64_t, *n_window);
//    memcpy(resized_boundary, boundary, sizeof(uint64_t) * (*n_window));
//    g_free(boundary);
