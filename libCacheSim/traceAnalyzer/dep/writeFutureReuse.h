...
class WriteFutureReuseDistribution {
 public:
  ...
  ~WriteFutureReuseDistribution() { delete[] reuse_info_; }
  ...
  inline void add_req(request_t *req) {
    ...
    if (it == last_write_.end()) {
      // only consider the writes in the first 3 days so that the distribution
      // won't be distorted due to trace end
    ...
    if (pos_rt >= reuse_info_array_size_) {
      printf("CNT_ARRAY_SIZE size too small %d %d\n", pos_rt,
             GET_POS_RT(rtime_since_write));
      abort();
    }

    if (req->op == OP_SET || req->op == OP_REPLACE) {
      utils::my_assert(
          req->next_access_vtime == -1 || req->next_access_vtime == INT64_MAX,
          "write request should not have reuse");
      last_write_[req->obj_id] = {(int32_t)req->clock_time, -1, 0};
    } else if (req->op == OP_DELETE) {
      last_write_.erase(req->obj_id);
      utils::my_assert(
          req->next_access_vtime == -1 || req->next_access_vtime == INT64_MAX,
          "delete request should not have reuse");
    ...
    if (req->op == OP_GET || req->op == OP_GETS || req->op == OP_INCR ||
        req->op == OP_DECR) {
      last_write_[req->obj_id] = {it->second.write_rtime,
  ...
  void dump(string &path_base) {
    ...
    ofs << "# real time: reuse_cnt, stop_reuse_cnt, reuse_access_age_sum, "
           "stop_reuse_access_age_sum, reuse_freq_sum, stop_reuse_freq_sum"
        << "\n";
 ...
 private:
  ...
  //  robin_hood::unordered_flat_map<obj_id_t, struct info_last_write>
  //  last_write_;
