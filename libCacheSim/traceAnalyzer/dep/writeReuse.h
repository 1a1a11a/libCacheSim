 ...
 public:
  ...
  inline void add_req(request_t *req) {
    ...
    if (it == last_write_rtime_.end()) {
      if (req->op == OP_SET || req->op == OP_ADD || req->op == OP_REPLACE ||
          req->op == OP_CAS || req->op == OP_WRITE) {
    ...
    utils::my_assert(pos_rt < reuse_info_array_size_,
                     "CNT_ARRAY_SIZE size too small\n");

    if (req->op == OP_GET || req->op == OP_GETS || req->op == OP_INCR ||
        req->op == OP_DECR) {
      read_reuse_cnt_[pos_rt]++;
    } else if (req->op == OP_SET || req->op == OP_ADD ||
               req->op == OP_REPLACE || req->op == OP_CAS) {
