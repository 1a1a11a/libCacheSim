...
void cal_group_metric_task(vector<fast23::obj_reuse_stat> &obj_reuse_stat,
  ...
  uniform_int_distribution<std::mt19937::result_type> dist(
      static_cast<std::mt19937::result_type>(idx_start),
      static_cast<std::mt19937::result_type>(idx_end - group_size));
  ...
  for (int i = 0; i < n_repeat; i++) {
    ...
    std::mt19937::result_type seq_base_idx = dist(rng);
    for (int j = 0; j < group_size; j++) {
      int idx = static_cast<int>(seq_base_idx) + j;
    ...
    int n_drop_n_reuse =
        static_cast<int>(drop_frac * static_cast<double>(n_reuse_group.size()));
    int n_drop_reuse_time = static_cast<int>(
        drop_frac * static_cast<double>(reuse_time_group.size()));
    if (n_drop_n_reuse > 0) {
    ...
    if (rnd_idx_vec_idx + group_size >= static_cast<int>(rnd_idx_vec.size())) {
      rnd_idx_vec_idx = 0;
    ...
    n_drop_n_reuse =
        static_cast<int>(drop_frac * static_cast<double>(n_reuse_group.size()));
    n_drop_reuse_time = static_cast<int>(
        drop_frac * static_cast<double>(reuse_time_group.size()));
    if (n_drop_n_reuse > 0) {
      sort(n_reuse_group.begin(), n_reuse_group.end(), greater<double>());
      sort(reuse_time_group.begin(), reuse_time_group.end());
...
/**
 * @brief calculate the mean and std of 1. reuse time, 2. n_reuse of a
 * write-time-based object group and a random object group for different group
 * sizes,
 *
    ...
    if (n_thread == 1) {
      ...
      for (int i = 0; i < n_batch; i++) {
        boost::asio::post(workers, [&, group_size, batch_size] {
          cal_group_metric_task(obj_reuse_stat, idx_start, idx_end, group_size,
                                batch_size, drop_frac, ofilepath, &mtx);
        });
      }
    }
  }
...
void cal_group_metric_utility_seq_task(struct request_info *request_info,
                                       size_t request_info_size, int group_size,
                                       int n_repeat, string ofilepath,
                                       mutex *mtx) {
  std::random_device dev;
  std::mt19937 rng(dev());
  uniform_int_distribution<mt19937::result_type> dist(
      0, request_info_size - group_size);
  ...
  for (int i = 0; i < n_repeat; i++) {
    ...
    for (int j = 0; j < group_size; j++) {
      ...
      if (request_info[idx].next_access_dist == -1) {
        ...
        utility_vec.push_back(1000000.0 /
                              ((double)request_info[idx].next_access_dist *
                               (double)request_info[idx].obj_size));
      }
    }
    ...
    // utility_vec[group_size - 4], utility_vec[group_size - 3],
    // utility_vec[group_size - 2], utility_vec[group_size - 1]);
    utility_mean_vec.push_back(mathUtils::mean(utility_vec, 0, 0));
    utility_std_vec.push_back(mathUtils::stdev(utility_vec, 0, 0));
  }

  auto guard = std::lock_guard(*mtx);
  ofstream ofs(ofilepath + ".grp" + to_string(group_size) + ".seq",
...
void cal_group_metric_utility_rnd_task(struct request_info *request_info,
                                       size_t request_info_size, int group_size,
                                       int n_repeat, string ofilepath,
                                       mutex *mtx) {
  std::random_device dev;
  ...
  auto guard = std::lock_guard(*mtx);
  ofstream ofs(ofilepath + ".grp" + to_string(group_size) + ".rnd",
               ofstream::out | ofstream::app);
 ...
 * @param n_thread
 */
void compareGrouping::cal_group_metric_utility(string trace_path,
                                               string ofilepath,
  ...
  fast23::request_info *request_info =
      new fast23::request_info[n_total_req - n_skip];
  ...
  while (req->valid) {
    if (req->next_access_vtime != -1 && req->next_access_vtime != INT64_MAX)
      request_info[n_req - n_skip].next_access_dist =
          req->next_access_vtime - n_req;
    else
