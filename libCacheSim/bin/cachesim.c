...
sim_arg_t parse_cmd(int argc, char *argv[]) {
  ...
  printf("trace type %s, trace %s cache_size %ld MiB alg %s metadata_size %d, "
         ...
         args.trace_path, (long) args.cache_size, args.alg, args.per_obj_metadata,
...
int main(int argc, char **argv) {
  ...
  common_cache_params_t cc_params = {.cache_size = args.cache_size * MiB, .hashpower = 26, .default_ttl = 86400 * 300, .per_obj_overhead = args.per_obj_metadata};
  cache_t *cache;
  ...
  if (strcasecmp(args.alg, "lru") == 0)
    ...
    LLSC_init_params_t init_params = {.segment_size = args.seg_size, .n_merge = args.n_merge, .type = args.lsc_type, .rank_intvl = args.rank_intvl};
  ...
  if (args.debug)
    ...
    sim_res_t *result = get_miss_ratio_curve(reader, cache, args.n_cache_size, args.cache_sizes, NULL, 0, args.n_thread);
