...
sTree * splay (key_type i, sTree * t) {
    ...
    for (;;) {
        if (key_cmp(i, t->key) < 0){
            ...
            if (key_cmp(i, t->left->key) < 0) {
                ...
                t->left = y->right;
                y->right = t;
                t->total_size = node_value(t->left) + node_value(t->right) + t->obj_size;
                t = y;
                if (t->left == NULL) break;
            }
            r->left = t;                               /* link right */
            r = t;
            t = t->left;
            r_size += r->obj_size+node_value(r->right);
            ...
            l = t;
            t = t->right;
            l_size += l->obj_size+node_value(l->left);
        } else {
            break;
        }
    }
    ...
    t->total_size = l_size + r_size + t->obj_size;
    ...
    for (y = N.right; y != NULL; y = y->right) {
        y->total_size = l_size;
        l_size -= y->obj_size+node_value(y->left);
    }
    for (y = N.left; y != NULL; y = y->left) {
        y->total_size = r_size;
        r_size -= y->obj_size+node_value(y->right);
    }
...
sTree * insert(key_type i, long long obj_size,sTree * t) {
    ...
    if (key_cmp(i, t->key) < 0) {
        ...
        new->right = t;
        t->left = NULL;
        t->total_size = t->obj_size + node_value(t->right);
    } else if (key_cmp(i, t->key) > 0) {
        new->right = t->right;
        new->left = t;
        t->right = NULL;
        t->total_size = t->obj_size + node_value(t->left);
    } else { /* We get here if it's already in the sTree */
        /* Don't add it again                      */
        free_node(new);
        assert (t->total_size == t->obj_size + node_value(t->left) + node_value(t->right));
        return t;
    }
    new->total_size = new->obj_size + node_value(new->left) + node_value(new->right);
    return new;
}
...
sTree * splay_delete(key_type i, sTree * t) {
    ...
    long long root_value = t->total_size;
    
    t = splay(i,t);
    long long delete_value=t->obj_size;
    if (key_cmp(i, t->key) == 0) {               /* found it */
        ...
        free_node(t);
        if (x != NULL) {
            x->total_size = root_value-delete_value;
        }
        return x;
    }
...
void check_sTree(sTree* t){
    ...
    if (t==NULL)
        return;
    assert(node_value(t) == node_value(t->left)+node_value(t->right)+t->obj_size);
    if (t->left != NULL)
        check_sTree(t->left);
    if (t->right != NULL)
        check_sTree(t->right);
...
int find_range(key_type low, sTree *t) {
    int count = 0;

    while (t != NULL) {
        if (key_cmp(t->key, low) <= 0) {
            t = t->right;
        } else {
            count+=t->obj_size;
            if (t->right != NULL) {
                count += node_value(t->right);
            }
            t = t->left;
        }
    }
