...
int main(int argc, char *argv[]) {
  ...
  desc.add_options()(
      "output_trace_type",
      po::value<std::string>(&(oarg.output_trace_type))->default_value(""),
      "output_trace_type "
      "[oracleSysTwrNS/oracleAkamai/oracleWiki16u/oracleWiki19u/oracleGeneral]/"
      "oracleGeneralOpNS")(
      "oracle_type",
      po::value<std::string>(&(oarg.oracle_type))->default_value("reuse"),
      "freq/reuse");
  ...
  std::cout << "input traces " << cli_arg.ipath << ", type "
            << cli_arg.trace_type << ", output " << oarg.output_trace_type
            << std::endl;
  ...
  if (oarg.oracle_type == "freq") {
    ...
    if (oarg.output_trace_type == "oracleSysTwrNS") {
      OracleReuseTraceGen<struct reqEntryReuseRealSystemTwrNS> trace_gen(
          reader, cli_arg.opath);
      trace_gen.run(false, true, false);
    } else if (oarg.output_trace_type == "oracleCF1") {
      OracleReuseTraceGen<struct reqEntryReuseCF1> trace_gen(reader,
                                                             cli_arg.opath);
      trace_gen.run(false, false, true);
    } else if (oarg.output_trace_type == "oracleAkamai") {
      OracleReuseTraceGen<struct reqEntryReuseAkamai> trace_gen(reader,
                                                                cli_arg.opath);
      trace_gen.run(false, false, true);
    } else if (oarg.output_trace_type == "oracleWiki16u") {
      OracleReuseTraceGen<struct reqEntryReuseWiki16u> trace_gen(reader,
                                                                 cli_arg.opath);
      trace_gen.run(false, false, true);
    } else if (oarg.output_trace_type == "oracleWiki19u") {
      OracleReuseTraceGen<struct reqEntryReuseWiki19u> trace_gen(reader,
                                                                 cli_arg.opath);
      trace_gen.run(false, false, true);
    } else if (oarg.output_trace_type == "oracleGeneral") {
      OracleReuseTraceGen<struct reqEntryReuseGeneral> trace_gen(reader,
                                                                 cli_arg.opath);
      trace_gen.run(false, false, true);
    } else if (oarg.output_trace_type == "oracleGeneralOpNS") {
      OracleReuseTraceGen<struct reqEntryReuseGeneralOpNS> trace_gen(
          reader, cli_arg.opath);
      trace_gen.run(false, false, false);
    } else {
      std::cerr << "unknown output type " << oarg.output_trace_type
                << std::endl;
      abort();
    }
  } else {
