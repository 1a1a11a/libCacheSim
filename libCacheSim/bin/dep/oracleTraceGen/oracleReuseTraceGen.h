...
struct reqEntryReuseRealSystemTwrNS {
  ...
  int get_obj_size() { return key_size_ + val_size_; }
  ...
  int32_t get_ttl() { return ttl_; }
  void set_ttl(int32_t ttl) { ttl_ = ttl; }
  uint32_t get_real_time() { return real_time_; }
...
// ts, obj, sz, ttl, age, hostname, content (h), extension (h), n_level,
// n_param, method, colo
struct reqEntryReuseCF1 {
  ...
  int get_obj_size() { return obj_size_; }
  void set_obj_size(int obj_size) { this->obj_size_ = obj_size; }
  bool is_size_zero() { return obj_size_ == 0; }
  bool is_op_write_or_delete() { return false; }
  int32_t get_ttl() { return ttl_; }
  void set_ttl(int32_t ttl) { ttl_ = ttl; }
  uint32_t get_real_time() { return real_time_; }
  void init(request_t *req) {
...
struct reqEntryReuseWiki16u {
  ...
  uint32_t get_real_time() { return 0; }
  void init(request_t *req) {
    this->obj_id_ = req->obj_id;
    this->obj_size_ = req->obj_size;
    this->content_type_ = req->content_type;
...
class OracleReuseTraceGen {
 public:
  OracleReuseTraceGen(reader_t *reader, std::string ofile_path)
      : reader_(reader), ofile_path_(std::move(ofile_path)) {
    temp_ofile_path_ = ofile_path_ + ".reverse";
    ofile_temp_.open(temp_ofile_path_,
                     std::ios::out | std::ios::binary | std::ios::trunc);
    ofile_.open(ofile_path_,
                std::ios::out | std::ios::binary | std::ios::trunc);
    if (!ofile_) {
  ...
  void run(bool output_txt, bool backfill_ttl, bool remove_size_change) {
    ...
    auto *last_access =
        new robin_hood::unordered_flat_map<uint64_t, struct last_access_info>;

    /* n_req_written counts the number of requests written to file,
     * n_req_total is the total number of requests in the trace
     * n_req_curr is the number of requests scanned, this is different from
     * n_req_written when we remove miss caused fills in twr traces */
    uint64_t n_req_written = 0, n_req_curr = 0,
             n_req_total = get_num_of_req(reader_);
#ifndef __APPLE__
    ...
    std::cout << reader_->trace_path << ": " << (double)n_req_total / 1000000.0
              << "M requests, " << n_miss_fill << " miss fill" << std::endl;
    ...
    while (true) {
      if (typeid(T) == typeid(struct reqEntryReuseRealSystemTwrNS) &&
          (*is_cache_miss_bitset)[n_req_total - n_req_curr - 1]) {
        /* this is on-demand fill, because we remove the get miss (they have
         * zero size), we change the on-demand fill to GET request, which will
         * cause an on-demand fill if it is cache miss */
        req->op = OP_GET;
      }
      ...
      if (typeid(T) == typeid(struct reqEntryReuseRealSystemTwrNS) &&
          re->is_op_write_or_delete()) {
        /* if this request is write or delete */
        last_access_vtime = -1;
      }

      (*last_access)[req->obj_id] = {.vtime = last_access_vtime,
                                     .ttl = last_ttl};
      ...
      if (n_req_curr % 100000000 == 0 && n_req_curr != 0) {
        ...
        std::cout << std::fixed << std::setprecision(2)
                  << (double)n_req_curr / 1000000 << " MReq, ts "
                  << -req->real_time + start_ts << ", working set/req GB "
                  << (double)unique_bytes / GiB << "/"
                  << (double)total_bytes / GiB << std::endl;
      }
    ...
    std::cout << n_size_zero << "("
              << (double)n_size_zero / (double)n_req_total * 100
              << "%) size zero requests";
    if (backfill_ttl) {
      std::cout << ", missing ttl " << n_ttl_missing << "/" << n_req_written
                << " (" << (double)n_ttl_missing / (double)n_req_written
                << ")\n";
    } else {
      std::cout << std::endl;
    }
 ...
 private:
  ...
  boost::dynamic_bitset<> *gen_miss_fill_vec(int64_t *n_fill) {
    ...
    boost::dynamic_bitset<> *is_cache_miss_bitset =
        new boost::dynamic_bitset<>(n_req);
    ...
    while (req->valid) {
      //      printf("%ld time %ld obj_id %lu obj_size %d + %u op %d namespace
      //      %d ttl %d next %ld xxxxxxxx \n", (long) n_req_curr,
      //             (long) req->real_time, (unsigned long) req->obj_id, (int)
      //             req->key_size, (unsigned) req->val_size, (int) req->op,
      //             (int) req->obj_namespace, (int) req->ttl, (long)
      ...
      if (req->op == OP_SET || req->op == OP_ADD || req->op == OP_CAS ||
          req->op == OP_REPLACE) {
  ...
  void _reverse_file(uint64_t n_total_req, bool output_txt,
                     bool remove_size_change) {
    utilsPrint::print_time();
    uint64_t n_req = 0;
    size_t file_size;
    char *mapped_file =
        reinterpret_cast<char *>(_setup_mmap(temp_ofile_path_, &file_size));
    ...
    std::cout << "reverse output " << n_total_req << " requests, "
              << "entry size " << req_entry_size << std::endl;
    ...
    while (pos >= req_entry_size) {
      ...
      if (output_txt) {
        if (typeid(T) == typeid(struct reqEntryReuseRealSystemTwrNS)) {
          auto re2 =
              reinterpret_cast<struct reqEntryReuseRealSystemTwrNS *>(&re);
          ofile_txt_ << re.get_real_time() << " " << re.obj_id_ << " "
                     << re2->op_ << " " << re.next_access_vtime_ << "\n";
        } else {
          ofile_txt_ << re.get_real_time() << " " << re.obj_id_ << " "
                     << re.next_access_vtime_ << "\n";
        }
      }
      n_req += 1;
      ...
      if (n_req % 100000000 == 0) {
        utilsPrint::print_time();
        std::cout << "write " << std::setprecision(4) << (double)n_req / 1000000
                  << " MReq" << std::endl;
      }
    }
    ...
    ofile_.close();
    if (output_txt) ofile_txt_.close();
  }
};
}  // namespace oracleTraceGen
