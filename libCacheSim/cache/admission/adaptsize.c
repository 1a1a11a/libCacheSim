...
void adaptsize_update_stats(admissioner_t *admissioner, const request_t *req) {
  ...
  bool exist_interval = g_hash_table_contains(pa->interval_metadata, key);
  bool exist_longterm = g_hash_table_contains(pa->longterm_metadata, key);
  
  if (exist_interval || exist_longterm) {
    adaptsize_obj_info_t* obj_info;
    if (exist_interval) {
      obj_info = g_hash_table_lookup(pa->interval_metadata, key);
    } else {
      obj_info = g_hash_table_lookup(pa->longterm_metadata, key);
    }
  ...
  adaptsize_obj_info_t* obj_info = g_new(adaptsize_obj_info_t, 1);
  obj_info->obj_seen_times = 1.0;
  obj_info->obj_size = req->obj_size;

  g_hash_table_insert(pa->interval_metadata, key, obj_info);
  pa->stat_size += req->obj_size; 
}
...
void adaptsize_reconfiguration(adaptsize_admission_params_t *pa) {
  ...
  while (g_hash_table_iter_next(&iter, &key, &val)) {
    ...
    if (obj_info->obj_seen_times < 0.1) {
      pa->stat_size -= obj_info->obj_size;
      g_hash_table_remove(pa->longterm_metadata, key);
    } else {
...
static void adaptsize_admissioner_parse_params(
  ...
  if (init_params == NULL) {
    INFO("use default adaptsize admission:");
  } else {
