...
static void S3FIFO_delay_free(cache_t *cache);
static bool S3FIFO_delay_get(cache_t *cache, const request_t *req);

static cache_obj_t *S3FIFO_delay_find(cache_t *cache, const request_t *req,
                                      const bool update_cache);
static cache_obj_t *S3FIFO_delay_insert(cache_t *cache, const request_t *req);
static cache_obj_t *S3FIFO_delay_to_evict(cache_t *cache, const request_t *req);
static void S3FIFO_delay_evict(cache_t *cache, const request_t *req);
static bool S3FIFO_delay_remove(cache_t *cache, const obj_id_t obj_id);
static inline int64_t S3FIFO_delay_get_occupied_byte(const cache_t *cache);
static inline int64_t S3FIFO_delay_get_n_obj(const cache_t *cache);
static inline bool S3FIFO_delay_can_insert(cache_t *cache,
                                           const request_t *req);
static void S3FIFO_delay_parse_params(cache_t *cache,
                                      const char *cache_specific_params);

static void S3FIFO_delay_evict_fifo(cache_t *cache, const request_t *req);
static void S3FIFO_delay_evict_main(cache_t *cache, const request_t *req);
...
cache_t *S3FIFO_delay_init(const common_cache_params_t ccache_params,
                           const char *cache_specific_params) {
  cache_t *cache =
      cache_struct_init("S3FIFO_delay", ccache_params, cache_specific_params);
  cache->cache_init = S3FIFO_delay_init;
  cache->cache_free = S3FIFO_delay_free;
  cache->get = S3FIFO_delay_get;
  cache->find = S3FIFO_delay_find;
  cache->insert = S3FIFO_delay_insert;
  cache->evict = S3FIFO_delay_evict;
  cache->remove = S3FIFO_delay_remove;
  cache->to_evict = S3FIFO_delay_to_evict;
  cache->get_n_obj = S3FIFO_delay_get_n_obj;
  cache->get_occupied_byte = S3FIFO_delay_get_occupied_byte;
  cache->can_insert = S3FIFO_delay_can_insert;

  cache->obj_md_size = 0;

  cache->eviction_params = malloc(sizeof(S3FIFO_delay_params_t));
  memset(cache->eviction_params, 0, sizeof(S3FIFO_delay_params_t));
  S3FIFO_delay_params_t *params =
      (S3FIFO_delay_params_t *)cache->eviction_params;
  params->req_local = new_request();
  params->hit_on_ghost = false;
  params->delay_perc = 0.2;

  S3FIFO_delay_parse_params(cache, DEFAULT_CACHE_PARAMS);
  if (cache_specific_params != NULL) {
    S3FIFO_delay_parse_params(cache, cache_specific_params);
  }
  ...
  snprintf(cache->cache_name, CACHE_NAME_ARRAY_LEN, "S3FIFO_delay-%.4lf-%d-%.2lf",
...
static cache_obj_t *S3FIFO_delay_find(cache_t *cache, const request_t *req,
  ...
  if (obj != NULL) {
    if (params->n_obj_admit_to_fifo - obj->S3FIFO_delay.insertion_time >
        params->fifo->cache_size * params->delay_perc) {
      obj->S3FIFO_delay.freq += 1;
    }
    return obj;
  }
...
static cache_obj_t *S3FIFO_delay_insert(cache_t *cache, const request_t *req) {
  ...
  if (params->hit_on_ghost) {
    ...
    obj->S3FIFO_delay.insertion_time = params->n_obj_admit_to_fifo;
  }
 ...
 */
static cache_obj_t *S3FIFO_delay_to_evict(cache_t *cache,
                                          const request_t *req) {
  assert(false);
  return NULL;
}
...
static void S3FIFO_delay_evict_fifo(cache_t *cache, const request_t *req) {
  ...
  while (!has_evicted && fifo->get_occupied_byte(fifo) > 0) {
    ...
    if (obj_to_evict->S3FIFO_delay.freq >= params->move_to_main_threshold) {
#if defined(TRACK_DEMOTION)
...
static void S3FIFO_delay_evict_main(cache_t *cache, const request_t *req) {
  ...
  while (!has_evicted && main->get_occupied_byte(main) > 0) {
    ...
    DEBUG_ASSERT(obj_to_evict != NULL);
    int freq = obj_to_evict->S3FIFO_delay.freq;
#if defined(TRACK_EVICTION_V_AGE)
    ...
    if (freq >= 1) {
      ...
      new_obj->S3FIFO_delay.freq = MIN(freq, 3) - 1;
...
static void S3FIFO_delay_evict(cache_t *cache, const request_t *req) {
  ...
  if (main->get_occupied_byte(main) > main->cache_size ||
    ...
    return S3FIFO_delay_evict_main(cache, req);
  }
  return S3FIFO_delay_evict_fifo(cache, req);
}
...
// ****                                                               ****
// ***********************************************************************
static const char *S3FIFO_delay_current_params(S3FIFO_delay_params_t *params) {
...
static void S3FIFO_delay_parse_params(cache_t *cache,
  ...
  while (params_str != NULL && params_str[0] != '\0') {
    ...
    if (strcasecmp(key, "fifo-size-ratio") == 0) {
      ...
      printf("parameters: %s\n", S3FIFO_delay_current_params(params));
      exit(0);
    } else {
