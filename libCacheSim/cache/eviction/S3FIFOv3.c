//
//  this uses an adaptive move_to_main_threshold based on the mean frequency of objects in the small queue
//  it currently does not work well
//
//
//  S3FIFOv3.c
//  libCacheSim
...
// ****                                                               ****
// ***********************************************************************
cache_t *S3FIFOv3_init(const common_cache_params_t ccache_params, const char *cache_specific_params);
static void S3FIFOv3_free(cache_t *cache);
static bool S3FIFOv3_get(cache_t *cache, const request_t *req);

static cache_obj_t *S3FIFOv3_find(cache_t *cache, const request_t *req, const bool update_cache);
static cache_obj_t *S3FIFOv3_insert(cache_t *cache, const request_t *req);
static cache_obj_t *S3FIFOv3_to_evict(cache_t *cache, const request_t *req);
static void S3FIFOv3_evict(cache_t *cache, const request_t *req);
static bool S3FIFOv3_remove(cache_t *cache, const obj_id_t obj_id);
static inline int64_t S3FIFOv3_get_occupied_byte(const cache_t *cache);
static inline int64_t S3FIFOv3_get_n_obj(const cache_t *cache);
static inline bool S3FIFOv3_can_insert(cache_t *cache, const request_t *req);
static void S3FIFOv3_parse_params(cache_t *cache, const char *cache_specific_params);

static void S3FIFOv3_evict_fifo(cache_t *cache, const request_t *req);
static void S3FIFOv3_evict_main(cache_t *cache, const request_t *req);
...
cache_t *S3FIFOv3_init(const common_cache_params_t ccache_params, const char *cache_specific_params) {
  cache_t *cache = cache_struct_init("S3FIFOv3", ccache_params, cache_specific_params);
  cache->cache_init = S3FIFOv3_init;
  cache->cache_free = S3FIFOv3_free;
  cache->get = S3FIFOv3_get;
  cache->find = S3FIFOv3_find;
  cache->insert = S3FIFOv3_insert;
  cache->evict = S3FIFOv3_evict;
  cache->remove = S3FIFOv3_remove;
  cache->to_evict = S3FIFOv3_to_evict;
  cache->get_n_obj = S3FIFOv3_get_n_obj;
  cache->get_occupied_byte = S3FIFOv3_get_occupied_byte;
  cache->can_insert = S3FIFOv3_can_insert;

  cache->obj_md_size = 0;

  cache->eviction_params = malloc(sizeof(S3FIFOv3_params_t));
  memset(cache->eviction_params, 0, sizeof(S3FIFOv3_params_t));
  S3FIFOv3_params_t *params = (S3FIFOv3_params_t *)cache->eviction_params;
  params->req_local = new_request();
  params->hit_on_ghost = false;

  S3FIFOv3_parse_params(cache, DEFAULT_CACHE_PARAMS);
  if (cache_specific_params != NULL) {
    S3FIFOv3_parse_params(cache, cache_specific_params);
  }
  ...
  snprintf(cache->cache_name, CACHE_NAME_ARRAY_LEN, "S3FIFOv3-%.4lf-%d", params->fifo_size_ratio,
...
static void adjust_move_to_main_threshold(cache_t *cache) {
  S3FIFOv3_params_t *params = (S3FIFOv3_params_t *)cache->eviction_params;

  int64_t n_obj_small_queue = params->fifo->get_n_obj(params->fifo);
  int64_t n_hit_small_queue = params->small_fifo_hits;
  double n_hit_mean = (double)n_hit_small_queue / n_obj_small_queue;

  //   if (cache->n_req % 1000 == 0) {
  //     printf("%ld %ld %ld %.2lf\n", cache->n_req, n_obj_small_queue, n_hit_small_queue, n_hit_mean);
  //   }

  if (n_hit_mean < 0.2) {
    if (params->move_to_main_threshold != 1) {
      printf("%ld, %ld %ld %.2lf, change move_to_main_threshold from %d to 1\n", cache->n_req, n_obj_small_queue,
             n_hit_small_queue, n_hit_mean, params->move_to_main_threshold);
      params->move_to_main_threshold = 1;
    }
  } else if (n_hit_mean > 1.8) {
    if (params->move_to_main_threshold != 2) {
      printf("%ld, %ld %ld %.2lf, change move_to_main_threshold from %d to 2\n", cache->n_req, n_obj_small_queue,
             n_hit_small_queue, n_hit_mean, params->move_to_main_threshold);
...
static void S3FIFOv3_evict_fifo(cache_t *cache, const request_t *req) {
  ...
  while (!has_evicted && fifo->get_occupied_byte(fifo) > 0) {
    ...
    params->small_fifo_hits -= obj_to_evict->S3FIFO.freq;
  ...
  if (cache->n_req % 10 == 0) {
    adjust_move_to_main_threshold(cache);
  }
}
...
static void S3FIFOv3_evict(cache_t *cache, const request_t *req) {
  ...
  if (main->get_occupied_byte(main) > main->cache_size || fifo->get_occupied_byte(fifo) == 0) {
    return S3FIFOv3_evict_main(cache, req);
  }
  return S3FIFOv3_evict_fifo(cache, req);
}
...
// ****                                                               ****
// ***********************************************************************
static const char *S3FIFOv3_current_params(S3FIFOv3_params_t *params) {
...
static void S3FIFOv3_parse_params(cache_t *cache, const char *cache_specific_params) {
  ...
  while (params_str != NULL && params_str[0] != '\0') {
    ...
    if (strcasecmp(key, "fifo-size-ratio") == 0) {
      ...
      printf("parameters: %s\n", S3FIFOv3_current_params(params));
      exit(0);
    } else {
