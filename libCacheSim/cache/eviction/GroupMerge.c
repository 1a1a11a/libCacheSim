//
//  GroupMerge: group-based eviction where groups are defined by byte size.
...
//  Retained objects stay at their original position in the FIFO.
//
//  GroupMerge.c
//  libCacheSim
//
...
typedef struct GroupMerge_params {
  cache_obj_t *q_head;
  cache_obj_t *q_tail;
...
static void GroupMerge_parse_params(cache_t *cache,
                                    const char *cache_specific_params);
static void GroupMerge_free(cache_t *cache);
static bool GroupMerge_get(cache_t *cache, const request_t *req);
static cache_obj_t *GroupMerge_find(cache_t *cache, const request_t *req,
                                    bool update_cache);
static cache_obj_t *GroupMerge_insert(cache_t *cache, const request_t *req);
static cache_obj_t *GroupMerge_to_evict(cache_t *cache, const request_t *req);
static void GroupMerge_evict(cache_t *cache, const request_t *req);
static bool GroupMerge_remove(cache_t *cache, obj_id_t obj_id);
static void GroupMerge_remove_obj(cache_t *cache, cache_obj_t *obj);
...
cache_t *GroupMerge_init(const common_cache_params_t ccache_params,
                         const char *cache_specific_params) {
  cache_t *cache =
      cache_struct_init("GroupMerge", ccache_params, cache_specific_params);
  cache->cache_init = GroupMerge_init;
  cache->cache_free = GroupMerge_free;
  cache->get = GroupMerge_get;
  cache->find = GroupMerge_find;
  cache->insert = GroupMerge_insert;
  cache->evict = GroupMerge_evict;
  cache->remove = GroupMerge_remove;
  cache->to_evict = GroupMerge_to_evict;
  ...
  GroupMerge_params_t *params = my_malloc(GroupMerge_params_t);
  ...
  if (cache_specific_params != NULL) {
    GroupMerge_parse_params(cache, cache_specific_params);
  }
...
static void GroupMerge_evict(cache_t *cache, const request_t *req) {
  ...
  if (params->pos_in_metric_list < params->n_objs_to_evict) {
    ...
    // after the last eviction in the batch, halve freq of retained objects
    // (they remain in their original position in the FIFO chain)
    if (params->pos_in_metric_list >= params->n_objs_to_evict) {
  ...
  // if only one object to evict in the batch, halve retained freq now
  if (params->pos_in_metric_list >= params->n_objs_to_evict) {
    for (int i = params->n_objs_to_evict; i < params->n_objs_in_batch; i++) {
      cache_obj_t *retained = params->metric_list[i].cache_obj;
      retained->GroupMerge.freq = (retained->GroupMerge.freq + 1) / 2;
...
static bool GroupMerge_remove(cache_t *cache, obj_id_t obj_id) {
  ...
  GroupMerge_remove_obj(cache, obj);
  return true;
}
...
// ****                                                               ****
// ***********************************************************************
static const char *GroupMerge_current_params(GroupMerge_params_t *params) {
  ...
  while (params_str != NULL && params_str[0] != '\0') {
    ...
    if (strcasecmp(key, "group-size") == 0) {
      ...
      printf("%s parameters: %s\n", cache->cache_name,
             GroupMerge_current_params(params));
      exit(0);
    } else {
