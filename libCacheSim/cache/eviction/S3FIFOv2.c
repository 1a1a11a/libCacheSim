...
static void S3FIFOv2_free(cache_t *cache);
static bool S3FIFOv2_get(cache_t *cache, const request_t *req);

static cache_obj_t *S3FIFOv2_find(cache_t *cache, const request_t *req,
                                  const bool update_cache);
static cache_obj_t *S3FIFOv2_insert(cache_t *cache, const request_t *req);
static cache_obj_t *S3FIFOv2_to_evict(cache_t *cache, const request_t *req);
static void S3FIFOv2_evict(cache_t *cache, const request_t *req);
static bool S3FIFOv2_remove(cache_t *cache, const obj_id_t obj_id);
static inline int64_t S3FIFOv2_get_occupied_byte(const cache_t *cache);
static inline int64_t S3FIFOv2_get_n_obj(const cache_t *cache);
static inline bool S3FIFOv2_can_insert(cache_t *cache, const request_t *req);
static void S3FIFOv2_parse_params(cache_t *cache,
                                  const char *cache_specific_params);

static void S3FIFOv2_evict_fifo(cache_t *cache, const request_t *req);
static void S3FIFOv2_evict_main(cache_t *cache, const request_t *req);
...
cache_t *S3FIFOv2_init(const common_cache_params_t ccache_params,
                       const char *cache_specific_params) {
  cache_t *cache =
      cache_struct_init("S3FIFOv2", ccache_params, cache_specific_params);
  cache->cache_init = S3FIFOv2_init;
  cache->cache_free = S3FIFOv2_free;
  cache->get = S3FIFOv2_get;
  cache->find = S3FIFOv2_find;
  cache->insert = S3FIFOv2_insert;
  cache->evict = S3FIFOv2_evict;
  cache->remove = S3FIFOv2_remove;
  cache->to_evict = S3FIFOv2_to_evict;
  cache->get_n_obj = S3FIFOv2_get_n_obj;
  cache->get_occupied_byte = S3FIFOv2_get_occupied_byte;
  cache->can_insert = S3FIFOv2_can_insert;

  cache->obj_md_size = 0;

  cache->eviction_params = malloc(sizeof(S3FIFOv2_params_t));
  memset(cache->eviction_params, 0, sizeof(S3FIFOv2_params_t));
  S3FIFOv2_params_t *params = (S3FIFOv2_params_t *)cache->eviction_params;
  params->req_local = new_request();
  params->hit_on_ghost = false;

  S3FIFOv2_parse_params(cache, DEFAULT_CACHE_PARAMS);
  if (cache_specific_params != NULL) {
    S3FIFOv2_parse_params(cache, cache_specific_params);
  }
  ...
  snprintf(cache->cache_name, CACHE_NAME_ARRAY_LEN, "S3FIFOv2-%.4lf-%d",
...
static void S3FIFOv2_evict(cache_t *cache, const request_t *req) {
  ...
  if (main->get_occupied_byte(main) > main->cache_size ||
    ...
    return S3FIFOv2_evict_main(cache, req);
  }
  return S3FIFOv2_evict_fifo(cache, req);
}
...
// ****                                                               ****
// ***********************************************************************
static const char *S3FIFOv2_current_params(S3FIFOv2_params_t *params) {
...
static void S3FIFOv2_parse_params(cache_t *cache,
  ...
  while (params_str != NULL && params_str[0] != '\0') {
    ...
    if (strcasecmp(key, "fifo-size-ratio") == 0) {
      ...
      printf("parameters: %s\n", S3FIFOv2_current_params(params));
      exit(0);
    } else {
