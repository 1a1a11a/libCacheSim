...
cache_t *SLRU_init(common_cache_params_t ccache_params, void *init_params) {
  ...
  SLRU_init_params_t *SLRU_init_params = (SLRU_init_params_t *) malloc(sizeof(SLRU_init_params_t));
  ...
  SLRU_params->LRUs = (cache_t **)malloc(sizeof(cache_t *) * SLRU_params->n_seg);
  ...
  for (i = 0; i < SLRU_params->n_seg; i++){
    ...
    if (ret == cache_ck_hit){
      ...
      if (i != SLRU_params->n_seg - 1){
        ...
        while (SLRU_params->LRUs[i + 1]->occupied_size + req->obj_size + cache->per_obj_overhead > SLRU_params->LRUs[i + 1]->cache_size)
          SLRU_cool(cache, req, i + 1);
...
void SLRU_insert(cache_t *cache, request_t *req) {
  ...
  for (i = 0; i < SLRU_params->n_seg; i++){
    ...
    if (SLRU_params->LRUs[i]->occupied_size + req->obj_size + cache->per_obj_overhead <= SLRU_params->LRUs[i]->cache_size){
  ...
  if (i == SLRU_params->n_seg){
    while (SLRU_params->LRUs[0]->occupied_size + req->obj_size + cache->per_obj_overhead > SLRU_params->LRUs[0]->cache_size){
