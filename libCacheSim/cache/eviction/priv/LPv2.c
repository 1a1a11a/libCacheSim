...
//  the object is requested after written, LPv2 tracks frequency, and each round
...
void LPv2_evict(cache_t *cache, const request_t *req,
  ...
  cache_obj_t *obj_to_evict = LPv2_to_evict(cache);
  if (evicted_obj != NULL) {
...
void LPv2_remove_obj(cache_t *cache, cache_obj_t *obj) {
  LPv2_params_t *params = (LPv2_params_t *)cache->eviction_params;
...
void LPv2_free(cache_t *cache) { cache_struct_free(cache); }
