...
static bool S3FIFOd_rebalance(cache_t *cache);
static void S3FIFOd_choose_threshold(cache_t *cache);
static void S3FIFOd_update_main_reinsert_threshold(cache_t *cache);
static void S3FIFOd_update_main_reinsert_threshold_incorrect(cache_t *cache);
static void S3FIFOd_update_small_reinsert_threshold(cache_t *cache);
...
static void S3FIFOd_evict_fifo(cache_t *cache, const request_t *req);
static void S3FIFOd_evict_main(cache_t *cache, const request_t *req);
...
cache_t *S3FIFOd_init(const common_cache_params_t ccache_params,
  ...
  snprintf(cache->cache_name, CACHE_NAME_ARRAY_LEN, "S3FIFOd-%.4lf-%d",
...
static bool S3FIFOd_get(cache_t *cache, const request_t *req) {
  ...
  if (params->main_cache->get_occupied_byte(params->main_cache) >
      ...
      cache->n_req % 1000 == 0) {
    // S3FIFOd_rebalance(cache);
    // S3FIFOd_choose_threshold(cache);
    S3FIFOd_update_main_reinsert_threshold(cache);
    S3FIFOd_update_main_reinsert_threshold_incorrect(cache);
    // S3FIFOd_update_small_reinsert_threshold(cache);
  }
...
static bool S3FIFOd_rebalance0(cache_t *cache) {
  S3FIFOd_params_t *params = (S3FIFOd_params_t *)cache->eviction_params;
...
static void S3FIFOd_evict(cache_t *cache, const request_t *req) {
  ...
  if (main->get_occupied_byte(main) > main->cache_size ||
    ...
    return S3FIFOd_evict_main(cache, req);
  }
  return S3FIFOd_evict_fifo(cache, req);
}
