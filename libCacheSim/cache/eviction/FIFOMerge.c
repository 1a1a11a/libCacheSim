...
//  it is similar to multi-bit clock, but it does bounded work, guarantees some
//  objects are evicted, some are retained
//
//  TODO: wrap back to the head of the list early before reaching the end of the
//  list
//
//  FIFOMerge.c
//  libCacheSim
...
cache_t *FIFOMerge_init(const common_cache_params_t ccache_params,
  ...
  if (init_params != NULL) {
    FIFOMerge_init_params_t *init_params_ptr =
        (FIFOMerge_init_params_t *)init_params;
    ...
    // params->n_merge_obj = -1; // make this adaptive to cache size and object
    // size

    /* can we make this parameter learned? */
    params->n_merge_obj = 100;
    params->metric_list =
        my_malloc_n(struct fifo_merge_sort_list_node, params->n_merge_obj);
  ...
  //         ccache_params.cache_size / 1024 / 1024, params->n_merge_obj,
  //         params->n_keep_obj);
...
void FIFOMerge_free(cache_t *cache) {
  ...
  my_free(sizeof(struct fifo_merge_sort_list_node) * params->n_merge_obj,
          params->metric_list);
...
double oracle_metric(cache_t *cache, cache_obj_t *cache_obj) {
  if (cache_obj->FIFOMerge.next_access_vtime == -1 ||
      cache_obj->FIFOMerge.next_access_vtime == INT64_MAX)
    return -1;
  return (double)1e9 / (cache_obj->FIFOMerge.next_access_vtime - cache->n_req) /
         (double)cache_obj->obj_size;
}

double freq_metric(cache_t *cache, cache_obj_t *cache_obj) {
  return (double)cache_obj->FIFOMerge.freq / (double)cache_obj->obj_size *
         1000.0;
}
...
double FIFOMerge_promote_metric(cache_t *cache, cache_obj_t *cache_obj) {
  ...
  // printf("%ld %.4lf %.4lf %ld\n", cache_obj->obj_id, freq_metric(cache,
  // cache_obj), oracle_metric(cache, cache_obj),
  // cache_obj->FIFOMerge.next_access_vtime);
  // if (cache_obj->FIFOMerge.next_access_vtime == -1 ||
  // cache_obj->FIFOMerge.next_access_vtime == INT64_MAX)
  //   return -1;
...
void FIFOMerge_evict(cache_t *cache, const request_t *req,
  ...
  for (int i = 0; i < params->n_merge_obj; i++) {
    ...
    struct fifo_merge_sort_list_node n = {
        FIFOMerge_promote_metric(cache, cache_obj), cache_obj};
    ...
    if (cache_obj == NULL) {
      ...
      if (n_loop == 2) {
        ERROR(
            "FIFOMerge_evict: loop too many times because not enough objects "
            "to merge\n");
        abort();
      }
    }
  ...
  // sort metrics
  qsort(params->metric_list, params->n_merge_obj,
        sizeof(struct fifo_merge_sort_list_node), cmp_list_node);
