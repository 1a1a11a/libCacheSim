...
//  if (cp->disk_sector_size != 0 && evictionAlgo->block_size != 0) {
//    *(gint64 *)(cp->obj_id_ptr) =
//        (gint64)(*(gint64 *)(cp->obj_id_ptr) * cp->disk_sector_size /
//                 evictionAlgo->block_size);
//    n = (int)ceil((double)cp->size / evictionAlgo->block_size);
...
//    }
  ...
  // evictionAlgo miss, load from disk
  else {
    if (page != NULL)
    ...
    if (cp->disk_sector_size != 0 && evictionAlgo->block_size != 0) {
      ...
      AMP_lookup(evictionAlgo, block)->last_block_number = last_block;
      ...
      for (i = 0; i < n - 1; i++) {
        ...
        AMP_add_only(evictionAlgo, cp);
        AMP_lookup(evictionAlgo, (*(gint64 *)(cp->obj_id_ptr)))->last_block_number =
            last_block;
      }
      *(gint64 *)(cp->obj_id_ptr) -= (n - 1);
      ...
      if (AMP_lookup(evictionAlgo, block) == NULL)
        ERROR("requested block is not in evictionAlgo after inserting, n %d, evictionAlgo "
              "size %ld\n",
              n, evictionAlgo->size);
#endif

      // if n==1, then page_last is the same only page
      struct AMP_page *page_last = AMP_lookup(evictionAlgo, last_block);
      ...
      if (page_last->p > (int)(AMP_params->APT)) {
        ...
        struct AMP_page *tag_page = AMP_lookup(
            evictionAlgo, page_last->block_number - (int)(AMP_params->APT / 2));
        if (tag_page) {
          tag_page->tag = TRUE;
    ...
    if (page_prev) {
      ...
      if (check) {
        ...
        //                evictionAlgo->block_size != 0){
        //                    block = (*(gint64*)(cp->obj_id_ptr)) + n -1;
        //                    length -= (long)(n - 1);
        //                }
        //                if (length > 0)
        //                    createPages_no_eviction(evictionAlgo, block + 1, length);
        // end
...
//  evictionAlgo->add_only = AMP_add_only;
//  evictionAlgo->add_withsize = AMP_add_withsize;
