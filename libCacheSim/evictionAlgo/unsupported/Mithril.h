...
/* recording table size, unit: percentage of evictionAlgo size */
...
typedef struct {
  /* obj_id_type of evictionAlgo, LRU, FIFO, Optimal, AMP */
  char *cache_type;
  ...
  /** size of block
   *  this should be equal to block_unit_size in evictionAlgo->core
   **/
  guint64 block_size;
...
typedef struct {
  /* the underlying evictionAlgo, like LRU, LFU, AMP */
  cache_t *cache;
...
/**
 check whether an block/obj is in the evictionAlgo

 @param Mithril the evictionAlgo struct
...
/**
 add a request to evictionAlgo, if it is in the evictionAlgo, update it
 if not, insert into evictionAlgo, this is the upper level API
 return TRUE if request is in the evictionAlgo
...
/**
 update the obj/block in the evictionAlgo,
 it is used internally during a evictionAlgo hit
...
/**
 insert the obj/block into evictionAlgo
 it is used internally during a evictionAlgo miss
...
/**
 evict one obj/block from evictionAlgo
 it is used internally when evictionAlgo is full
...
/**
 the free function for evictionAlgo struct
...
/**
 ...
 Mithril_destroy_uniq only destroy uniq struct related to current evictionAlgo,
...
/**
 get current evictionAlgo size usage

 @param cache evictionAlgo struct
 @return current size
 */
