...
gboolean Optimal_add(cache_t *cache, request_t *req) {
  ...
  while ((long) g_hash_table_size(Optimal_params->hashtable) > cache->size)
...
gboolean Optimal_add_only(cache_t *cache, request_t *req) {
  ...
  if (Optimal_check(cache, req)) {
    ...
    _Optimal_insert(cache, req);
    if ((long) g_hash_table_size(Optimal_params->hashtable) > cache->size)
      _Optimal_evict(cache, req);
    retval = FALSE;
  }
  cache->req_cnt += 1;
...
void Optimal_destroy(cache_t *cache) {
  ...
  ((struct Optimal_init_params *) (cache->cache_specific_init_params))
    ->next_access = NULL;
