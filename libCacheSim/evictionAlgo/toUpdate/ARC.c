...
#ifdef SANITY_CHECK
  if (ARC_params->LRU1->check(ARC_params->LRU1, cp)) {
    fprintf(stderr, "ERROR: in ARC insert, inserted item is in evictionAlgo\n");
    exit(1);
  }
#endif

  ARC_params->LRU1->_insert(ARC_params->LRU1, cp);
  ARC_params->size1++;
}
...
gboolean ARC_check(cache_t *cache, request_t *cp) {
  /* check both segments */
  ARC_params_t *ARC_params = (ARC_params_t *) (cache->cache_params);
  return (ARC_params->LRU1->check(ARC_params->LRU1, cp) ||
          ARC_params->LRU2->check(ARC_params->LRU2, cp));
}
...
void _ARC_update(cache_t *cache, request_t *cp) {
  ...
  ARC_params_t *ARC_params = (ARC_params_t *) (cache->cache_params);
  if (ARC_params->LRU1->check(ARC_params->LRU1, cp)) {
    ARC_params->LRU1->remove_obj(ARC_params->LRU1, cp->obj_id_ptr);
    ARC_params->LRU2->_insert(ARC_params->LRU2, cp);
    ARC_params->size1--;
    ARC_params->size2++;
  } else {
#ifdef SANITY_CHECK
    if (!ARC_params->LRU2->check(ARC_params->LRU2, cp)) {
      fprintf(stderr, "ERROR: in ARC insert, update element in LRU2, but"
                      " item is not in evictionAlgo\n");
      exit(1);
    }
#endif
    ARC_params->LRU2->_update(ARC_params->LRU2, cp);
  }
}
...
void _ARC_evict(cache_t *cache, request_t *cp) {
  ...
  gint64 *size;
  if (ARC_params->LRU1g->check(ARC_params->LRU1g, cp)) {
  ...
  } else if (ARC_params->LRU2g->check(ARC_params->LRU2g, cp)) {
  ...
  evicted_item = evicted_from->evict_with_return(evicted_from, cp);
  if (remove_ghost)
    remove_ghost->remove_obj(remove_ghost, cp->obj_id_ptr);
  (*size)--;
  old_cp_itemp = cp->obj_id_ptr;
  cp->obj_id_ptr = evicted_item;
#ifdef SANITY_CHECK
  if (add_ghost->check(add_ghost, cp)) {
    fprintf(stderr, "ERROR: in ARC evict, evicted element in ghost list\n");
    exit(1);
  }
#endif
  add_ghost->get(add_ghost, cp);
...
gpointer _ARC_evict_with_return(cache_t *cache, request_t *cp) {
  ...
  if (ARC_params->LRU1g->check(ARC_params->LRU1g, cp)) {
    ...
    // then add evicted to LRU2g
    evicted_item =
      ARC_params->LRU2->evict_with_return(ARC_params->LRU2, cp);
    ARC_params->LRU1g->remove_obj(ARC_params->LRU1g, cp->obj_id_ptr);
    ARC_params->size2--;
    old_cp_itemp = cp->obj_id_ptr;
    cp->obj_id_ptr = evicted_item;
    ARC_params->LRU2g->get(ARC_params->LRU2g, cp);
    cp->obj_id_ptr = old_cp_itemp;
  } else if (ARC_params->LRU2g->check(ARC_params->LRU2g, cp)) {
    // hit on LRU2g, evict from LRU1 and remove from LRU2g,
    // then add evicted to LRU1g
    evicted_item =
      ARC_params->LRU1->evict_with_return(ARC_params->LRU1, cp);
    ARC_params->LRU2g->remove_obj(ARC_params->LRU2g, cp->obj_id_ptr);
    ARC_params->size1--;
    old_cp_itemp = cp->obj_id_ptr;
    cp->obj_id_ptr = evicted_item;
    ARC_params->LRU1g->get(ARC_params->LRU1g, cp);
    cp->obj_id_ptr = old_cp_itemp;
  } else {
    // not hit anywhere, evict from LRU1, then add evicted to LRU1g
    evicted_item =
...
gboolean ARC_add(cache_t *cache, request_t *cp) {
  ...
  if (ARC_check(cache, cp)) {
    ...
    if (ARC_params->size1 + ARC_params->size2 > cache->size)
      _ARC_evict(cache, cp);
    retval = FALSE;
#ifdef SANITY_CHECK
    if ((ARC_params->size1 + ARC_params->size2 > evictionAlgo->size) ||
        (ARC_params->LRU1->get_used_size(ARC_params->LRU1) +
         ARC_params->LRU2->get_used_size(ARC_params->LRU2)) >
            evictionAlgo->size) {
      fprintf(stderr,
              "ERROR: in ARC add, after inserting, "
              "sum of two LRUs sizes: %lu, ARC size1+size2=%lu, "
              "but evictionAlgo size=%ld\n",
              (unsigned long)(ARC_params->LRU1->get_used_size(
                                  ARC_params->LRU1) +
                              ARC_params->LRU2->get_used_size(
                                  ARC_params->LRU2)),
...
//   unique to the evictionAlgo, freeing these resources won't affect
//   other caches copied from original evictionAlgo
//   in Optimal, next_access should not be freed in destroy_cloned_cache,
//   because it is shared between different caches copied from the original one.
//   */
//
//  ARC_params_t *ARC_params = (ARC_params_t *) (cache->cache_params);
//  ARC_params->LRU1->destroy(ARC_params->LRU1);
//  ARC_params->LRU1g->destroy(ARC_params->LRU1g);
//  ARC_params->LRU2->destroy(ARC_params->LRU2);
//  ARC_params->LRU2g->destroy(ARC_params->LRU2g);
