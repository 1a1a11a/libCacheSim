 ...
 *
 * this LFU clear the frequency of an obj_id after evicting from evictionAlgo
...
gboolean LFU_add(cache_t *cache, request_t *req) {
  ...
  if (LFU_check(cache, req)) {
    ...
    if ((long) g_hash_table_size(LFU_params->hashtable) > cache->size)
