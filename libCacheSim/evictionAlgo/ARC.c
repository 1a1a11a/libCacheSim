...
cache_t *ARC_init(common_cache_params_t ccache_params_, void *init_params_) {
  ...
  cache->cache_params = my_malloc_n(ARC_params_t, 1);
...
void ARC_insert(cache_t *cache, request_t *req) {
  ...
  cache->occupied_size += req->obj_size;
  assert(cache->occupied_size == ARC_params->LRU1->occupied_size + ARC_params->LRU2->occupied_size);
}
...
void ARC_remove_obj(cache_t *cache, cache_obj_t *obj_to_remove) {
  ...
  ARC_params_t *ARC_params = (ARC_params_t *) (cache->cache_params);
  copy_cache_obj_to_request(req_local, obj_to_remove);

  if (ARC_params->LRU1->check(ARC_params->LRU1, req_local, false)) {
    ARC_params->LRU1->remove(ARC_params->LRU1, obj_to_remove->obj_id_int);
  }
  if (ARC_params->LRU2->check(ARC_params->LRU2, req_local, false)) {
    ARC_params->LRU2->remove(ARC_params->LRU2, obj_to_remove->obj_id_int);
  }

  assert(cache->occupied_size >= obj_to_remove->obj_size);
