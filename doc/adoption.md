...
| Merlin: An Efficient Adaptive Cache Eviction Algorithm via Fine-Grained Characterization — Liujia Li, Jinhao Guo, Yi Fan, Jianyu Wu, Jie Zhang, Xiaolin Wang, Yingwei Luo, Diyu Zhou (Peking University), Zhenlin Wang (Michigan Tech), Yuval Tamir (UCLA) | OSDI '26 | B + C | "We implemented Merlin in both CacheLib [11] … and libCacheSim [2], an efficient cache simulator"; "For hit rates, given the thousands of traces, we follow the conventional approach to evaluate with a cache simulator: libCacheSim [2]", and flash-friendliness is evaluated "on an extension of libCacheSim". The authors also state they "contributed … an implementation of CAR to libCacheSim" — recorded as their claim; [`CAR.c`](/libCacheSim/cache/eviction/CAR.c) ships here, but this repository's history does not attribute it to them. | [USENIX](https://www.usenix.org/conference/osdi26/presentation/li-liujia) |
| Linear Elastic Caching via Ski Rental — Ravi Kumar, Todd Lipcon, Manish Purohit, Tamas Sarlos (Google) | CIDR '25 | B | "We implemented our ski rental based algorithms in libCacheSim [1]. LibCacheSim is designed for high-throughput cache simulations and allows for efficient multi-threaded parallel simulations." The resulting policy — not libCacheSim itself — is reported as deployed in Spanner. | [CIDR](https://mail.vldb.org/cidrdb/papers/2025/p22-kumar.pdf) |
| Writeback Modeling: Theory and Application to Zipfian Workloads — Wesley Smith, Chen Ding (University of Rochester), Daniel Byrne (Michigan Tech) | MEMSYS '21 | B | "The simulator is based on libCacheSim [36] written in C++ compiled with g++ 5.4 (-O3)." The earliest third-party use found in this edition. | [PDF](https://www.memsys.io/wp-content/uploads/2023/09/p2.pdf), [DOI](https://doi.org/10.1145/3488423.3519331) |
...
| Docker Hub | [`1a1a11a/libcachesim`](https://hub.docker.com/r/1a1a11a/libcachesim), built from the repository's [`dockerfile`](/dockerfile) | D | 1,014 pulls as of the census date; the only libCacheSim image on Docker Hub | [Docker Hub](https://hub.docker.com/r/1a1a11a/libcachesim) |
...
the single Docker Hub image is the project's own, tabulated above. deps.dev and GitHub's
dependency graph report zero reverse dependencies for the PyPI package. Whatever reach the library has, it
...
| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-08-13 | First edition. **31 adoption entries** — 8 first-party artifacts and papers, 13 third-party works, 1 independent cross-validation, 6 practitioner and community entries, and 3 ecosystem projects — of which **19 are third-party**. Plus 4 first-party distribution channels, which are reach rather than adoption, for **35 classed rows**. By evidence class: 11×A, 12×B, 14×C, 4×D (41 assignments over 35 rows; six rows carry two classes). Recorded but counted nowhere: the OSDI '20 row that introduced the simulator, 4 derived-implementation and trace-format rows, repository signals, and downstream algorithm adoption. |
