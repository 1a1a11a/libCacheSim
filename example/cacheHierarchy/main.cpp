...
int main(int argc, char* argv[]){
  ...
  for (int i=0; i<config.n_l1; i++){
    ...
    std::thread t(Simulator::gen_miss_trace, cache_algo, cache_size, trace_path, miss_output_path);
  ...
  std::cout <<"************* create L2 trace " <<
    TraceUtils::merge_l1_trace(config.l1_miss_output_path, config.l2_trace_path) << " req" << std::endl;


  // start L2: get a MRC
  Simulator::output_mrc(cache_algo, config.l2_sizes, config.l2_trace_path, config.l2_mrc_output_path);
