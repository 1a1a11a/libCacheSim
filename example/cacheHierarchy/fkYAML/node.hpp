...
#if FK_YAML_MAJOR_VERSION != 0 || FK_YAML_MINOR_VERSION != 4 || \
    FK_YAML_PATCH_VERSION != 2
...
#define FK_YAML_NAMESPACE_VERSION_CONCAT_IMPL(major, minor, patch) \
...
#define FK_YAML_NAMESPACE_VERSION   \
  FK_YAML_NAMESPACE_VERSION_CONCAT( \
      FK_YAML_MAJOR_VERSION, FK_YAML_MINOR_VERSION, FK_YAML_PATCH_VERSION)
...
// To avoid redundant inclusion, do not include version_macros.hpp file as the
// other files do.
...
// Skip detection if the definitions listed below already exist.
    ...
    !defined(FK_YAML_HAS_CXX_17) && !defined(FK_YAML_HAS_CXX_14) && \
...
// switch usage of the deprecated attribute. [[deprecated]] is available since
// C++14.
...
// switch usage of the no_sanitize attribute only when Clang sanitizer is
// active.
...
// switch usages of the std::to_chars()/std::from_chars() functions which have
// been available since C++17.
#if defined(FK_YAML_HAS_CXX_17) && defined(__cpp_lib_to_chars) && \
...
#if defined(FK_YAML_HAS_CXX_20) && defined(__cpp_char8_t) && \
...
// switch usage of [[likely]] C++ attribute which has been available since
// C++20.
...
// switch usage of [[unlikely]] C++ attribute which has been available since
// C++20.
#if defined(FK_YAML_HAS_CXX_20) && \
    FK_YAML_HAS_CPP_ATTRIBUTE(unlikely) >= 201803L
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
//   For contributors:
//     This file is for supplementing future C++ STL implementations to utilize
//     some useful features implemented in C++14 or better. This file is needed
//     to keep the fkYAML library requirement to C++11.
//     **DO NOT** implement features which are not included any version of STL
//     in this file. Such implementations must be in the type_traits.hpp file.
/////////////////////////////////////////////////////////////////////////////////////////////////////////
...
struct integer_sequence {
  ...
  static constexpr std::size_t size() noexcept { return sizeof...(I); }
    ...
    !FK_YAML_HAS_BUILTIN(__integer_pack)
...
struct generator {
  using type =
      typename merger<typename generator<T, Num / 2>::type,
...
#if FK_YAML_HAS_BUILTIN(__make_integer_seq)
    // clang defines built-in __make_integer_seq to generate an integer
    // sequence.
    = __make_integer_seq<integer_sequence, T, Num>;
#elif FK_YAML_HAS_BUILTIN(__integer_pack)
    // GCC or other compilers may implement built-in __integer_pack to generate
    // an integer sequence.
...
/// @brief A partial specialization of conjunction if more than one traits are
/// given.
/// @tparam First The first type trait to be checked if its ::value is true.
/// @tparam Rest The rest of traits passed as another conjunction template
/// arguments if First::value is true.
...
/// @brief A partial specialization of disjunction if more than one traits are
/// given.
/// @tparam First The first type trait to be checked if its ::value is true.
/// @tparam Rest The rest of traits passed as another conjunction template
/// arguments if First::value is false.
template <typename First, typename... Rest>
...
/// @brief A wrapper function to call std::unreachable() (since C++23) or
/// similar compiler specific extensions.
/// @note This function is implemented only for better code optimization against
/// dead code and thus excluded from coverage report.
// LCOV_EXCL_START
[[noreturn]] inline void unreachable() {
  // use compiler specific extensions if possible.
  // undefined behavior should be raised by an empty function with noreturn
  // attribute.
...
/// @brief A partial specialization of detector if desired operation type is
/// found.
/// @tparam Default A type to represent detection failure.
/// @tparam Op A type for desired operation type.
/// @tparam Args Argument types passed to desired operation.
...
template <template <typename...> class Op, typename... Args>
using detected_t =
    typename detector_impl::detector<nonesuch, void, Op, Args...>::type;

/// @brief Type traits to check if Expected and a detected type are exactly the
/// same.
...
/// @tparam T A target type.
template <typename T>
using reserve_fn_t = decltype(std::declval<T>().reserve(
    std::declval<typename remove_cvref_t<T>::size_type>()));
...
struct is_iterator_traits
    : conjunction<is_detected<difference_type_t, remove_cvref_t<T>>,
...
/// @brief Type traits to check if begin/end functions can be called on a T
/// object.
/// @tparam T A target type.
...
struct has_begin_end<
    T, void_t<decltype(begin(std::declval<T>()), end(std::declval<T>()))>>
    : std::true_type {};
...
/// @brief A partial specialization of is_comparable if T and U are comparable
/// types.
/// @tparam Comparator An object type to compare T and U objects.
/// @tparam T A type for comparison.
/// @tparam U The other type for comparison.
template <typename Comparator, typename T, typename U>
struct is_comparable<Comparator, T, U,
                     void_t<decltype(std::declval<Comparator>()(
                                std::declval<T>(), std::declval<U>())),
                            decltype(std::declval<Comparator>()(
...
template <typename T>
using is_non_bool_integral =
    conjunction<std::is_integral<T>, negation<std::is_same<bool, T>>>;
...
using is_iterator_of = std::is_same<
    remove_cv_t<typename std::iterator_traits<ItrType>::value_type>, T>;
...
template <typename T>
struct static_const {
  static FK_YAML_INLINE_VAR constexpr T
      value{};  // NOLINT(readability-identifier-naming)
};

#ifndef FK_YAML_HAS_CXX_17
/// @brief A instantiation of static_const::value instance.
/// @note This is required if inline variables are not available. C++11-14 do
/// not provide such a feature yet.
/// @tparam T A target type for the resulting static constant instance.
template <typename T>
...
/// @brief An ADL friendly converter between basic_node objects and native data
/// objects.
...
/// @tparam IgnoredCompare A placeholder for key comparison. This will be
/// ignored.
/// @tparam Allocator A class for allocators.
/// @sa https://fktn-k.github.io/fkYAML/api/ordered_map/
template <typename Key, typename Value, typename IgnoredCompare,
          typename Allocator>
class ordered_map;
...
/// @tparam T A class to be checked if it's a basic_node template class instance
/// type.
template <typename T>
struct is_basic_node_impl : std::false_type {};

/// @brief A partial specialization of is_basic_node_impl for basic_node
/// template class.
...
template <template <typename, typename...> class SequenceType,
          template <typename, typename, typename...> class MappingType,
          typename BooleanType, typename IntegerType, typename FloatNumberType,
          typename StringType, template <typename, typename> class Converter>
...
/// @brief A struct to check the template parameter class is a basic_node
...
/// @brief A struct to check the template parameter class is a kind of
/// node_ref_storage_template class.
/// @tparam T A type to be checked if it's a kind of node_ref_storage template
/// class.
template <typename T>
...
/// @brief Type traits to check if T is a compatible type for BasicNodeType in
/// terms of from_node function.
...
/// @brief A partial specialization of has_from_node if T is not a basic_node
/// template instance type.
/// @tparam BasicNodeType A basic_node template instance type.
/// @tparam T A target type passed to from_node function.
template <typename BasicNodeType, typename T>
struct has_from_node<BasicNodeType, T,
                     enable_if_t<negation<is_basic_node<T>>::value>> {
  using converter =
      typename BasicNodeType::template value_converter_type<T, void>;

  // NOLINTNEXTLINE(readability-identifier-naming)
  static constexpr bool value =
      is_detected_exact<void, from_node_function_t, converter,
                        const BasicNodeType&, T&>::value;
};

/// @brief Type traits to check if T is a compatible type for BasicNodeType in
/// terms of to_node function.
/// @warning Do not pass basic_node type as BasicNodeType to avoid infinite type
/// instantiation.
/// @tparam BasicNodeType A basic_node template instance type.
...
/// @brief A partial specialization of has_to_node if T is not a basic_node
/// template instance type.
/// @tparam BasicNodeType A basic_node template instance type.
/// @tparam T A target type passed to to_node function.
template <typename BasicNodeType, typename T>
struct has_to_node<BasicNodeType, T,
                   enable_if_t<negation<is_basic_node<T>>::value>> {
  using converter =
      typename BasicNodeType::template value_converter_type<T, void>;
  ...
  // NOLINTNEXTLINE(readability-identifier-naming)
  static constexpr bool value =
      is_detected_exact<void, to_node_function_t, converter, BasicNodeType&,
                        T>::value;
};
...
/// @brief Type traits implementation of is_node_compatible_type to check if
/// CompatibleType is a compatible type for BasicNodeType.
/// @tparam BasicNodeType A basic_node template instance type.
...
/// @brief A partial specialization of is_node_compatible_type_impl if
/// CompatibleType is a complete type and is compatible for BasicNodeType.
/// @tparam BasicNodeType A basic_node template instance type.
/// @tparam CompatibleType A target type for compatibility check.
...
struct is_node_compatible_type_impl<
    ...
    enable_if_t<conjunction<is_complete_type<CompatibleType>,
                            has_to_node<BasicNodeType, CompatibleType>>::value>>
    : std::true_type {};

/// @brief Type traits to check if CompatibleType is a compatible type for
/// BasicNodeType.
/// @tparam BasicNodeType A basic_node template instance type.
/// @tparam CompatibleType A target type for compatibility check.
template <typename BasicNodeType, typename CompatibleType>
struct is_node_compatible_type
    : is_node_compatible_type_impl<BasicNodeType, CompatibleType> {};
...
template <typename BasicNodeType,
          typename = enable_if_t<is_basic_node<BasicNodeType>::value>>
...
/// @brief Helper struct which ensures destruction/deallocation of
...
struct tidy_guard {
  ...
  /// @param p_obj
  tidy_guard(ObjT* p_obj) noexcept : p_obj(p_obj) {}
  ...
  /// @brief Destroys this tidy_guard object. Destruction/deallocation happen if
  ...
  ObjT* get() const noexcept { return p_obj; }

  /// @brief Checks if the pointer is not null.
  explicit operator bool() const noexcept { return p_obj != nullptr; }

  /// @brief Releases the pointer to the object. No destruction/deallocation
  /// happen after this function gets called.
  /// @return The pointer to the object.
...
class uri_encoding {
 public:
  ...
  /// @param end An iterator to the past-the-end element of the character
  /// sequence.
 ...
 private:
  ...
  /// @param end An iterator to the past-the-end element of the whole character
  /// sequence.
  ...
  /// @return true if the given character is allowed as a URI character, false
  /// otherwise.
  static bool validate_character(const char c) {
    // Check if the current character is one of reserved/unreserved characters
    // which are allowed for use. See the following links for details:
    // * reserved characters:
...
class invalid_encoding : public exception {
 public:
  ...
  explicit invalid_encoding(const char* msg,
  ...
  /// @param u16_l The second UTF-16 encoded element used for the UTF-8
  /// encoding.
  explicit invalid_encoding(const char* msg,
                            std::array<char16_t, 2> u16) noexcept
 ...
 private:
  static std::string generate_error_message(
      const char* msg, const std::initializer_list<uint8_t>& u8) noexcept {
    const auto* itr = u8.begin();
    const auto* end_itr = u8.end();
    std::string formatted =
        detail::format("invalid_encoding: %s in=[ 0x%02x", msg, *itr++);
    while (itr != end_itr) {
  ...
  /// @brief Generate an error message from the given parameters for the UTF-16
  /// encoding.
  /// @param msg An error message.
  ...
  static std::string generate_error_message(
      const char* msg, std::array<char16_t, 2> u16) noexcept {
  ...
  /// @brief Generate an error message from the given parameters for the UTF-32
  /// encoding.
  /// @param msg An error message.
  /// @param u32 The UTF-32 encoded element used for the UTF-8 encoding.
  /// @return A genereated error message.
  static std::string generate_error_message(const char* msg,
                                            char32_t u32) noexcept {
    // uint32_t is large enough for UTF-32 encoded elements.
    return detail::format("invalid_encoding: %s in=0x%08x", msg,
...
class parse_error : public exception {
 public:
  /// @brief Constructs a new parse_error object with an error message and
  /// counts of lines and colums at the error.
  /// @param[in] msg An error message.
  /// @param[in] lines Count of lines.
  /// @param[in] cols_in_line Count of colums.
  explicit parse_error(const char* msg, uint32_t lines,
 ...
 private:
  static std::string generate_error_message(const char* msg, uint32_t lines,
                                            uint32_t cols_in_line) noexcept {
    return detail::format("parse_error: %s (at line %u, column %u)", msg, lines,
                          cols_in_line);
  }
};
...
class type_error : public exception {
 public:
  /// @brief Construct a new type_error object with an error message and a node
  /// type.
  /// @param[in] msg An error message.
  ...
  /// @brief Construct a new type_error object with an error message and a node
  /// type.
  /// @deprecated Use type_error(const char*, node_type) constructor. (since
  /// 0.3.12).
  /// @param[in] msg An error message.
  /// @param[in] type The type of a source node value.
  FK_YAML_DEPRECATED(
      "Since 0.3.12; Use explicit type_error(const char*, node_type)")
...
class invalid_tag : public exception {
 public:
  /// @brief Constructs a new invalid_tag object with an error message and
  /// invalid tag contents.
  /// @param[in] msg An error message.
...
/// @return true if the given bytes a valid 3-byte UTF-8 character, false
/// otherwise.
...
/// @return true if the given bytes a valid 2-byte UTF-8 character, false
/// otherwise.
...
/// @return true if the given bytes a valid 4-byte UTF-8 character, false
/// otherwise.
inline bool validate(uint8_t byte0, uint8_t byte1, uint8_t byte2,
...
/// @param[out] consumed_size The number of UTF-16 encoded characters used for
/// the conversion.
/// @param[out] encoded_size The size of UTF-encoded bytes.
inline void from_utf16(std::array<char16_t, 2> utf16,
                       std::array<uint8_t, 4>& utf8, uint32_t& consumed_size,
  ...
  if (first < 0x80u) {
    ...
    const auto utf8_chunk = static_cast<uint16_t>(
        0xC080u | ((first & 0x07C0u) << 2) | (first & 0x3Fu));
    ...
    const auto utf8_chunk =
        static_cast<uint32_t>(0xE08080u | ((first & 0xF000u) << 4) |
                              ((first & 0x0FC0u) << 2) | (first & 0x3Fu));
    ...
    const uint32_t code_point =
        0x10000u + ((first & 0x03FFu) << 10) + (second & 0x03FFu);
    const auto utf8_chunk = static_cast<uint32_t>(
        0xF0808080u | ((code_point & 0x1C0000u) << 6) |
        ((code_point & 0x03F000u) << 4) | ((code_point & 0x0FC0u) << 2) |
        (code_point & 0x3Fu));
...
inline void from_utf32(const char32_t utf32, std::array<uint8_t, 4>& utf8,
                       uint32_t& encoded_size) {
  if (utf32 < 0x80u) {
    ...
    const auto utf8_chunk = static_cast<uint16_t>(
        0xC080u | ((utf32 & 0x07C0u) << 2) | (utf32 & 0x3Fu));
    utf8[0] = static_cast<uint8_t>(utf8_chunk >> 8);
    utf8[1] = static_cast<uint8_t>(utf8_chunk);
    encoded_size = 2;
  } else if (utf32 <= 0xFFFFu) {
    const auto utf8_chunk =
        static_cast<uint32_t>(0xE08080u | ((utf32 & 0xF000u) << 4) |
    ...
    const auto utf8_chunk = static_cast<uint32_t>(
        0xF0808080u | ((utf32 & 0x1C0000u) << 6) | ((utf32 & 0x03F000u) << 4) |
...
enum class chomping_indicator_t : std::uint8_t {
  STRIP,  //!< excludes final line breaks and trailing empty lines indicated by
          //!< `-`.
  CLIP,   //!< preserves final line breaks but excludes trailing empty lines. no
          //!< indicator means this type.
...
/// @note
/// This class is a minimal implementation of std::basic_string_view which has
/// been available since C++17 but pretty useful and efficient for
/// referencing/investigating character sequences.
/// @warning
/// This class intentionally omits a lot of value checks to improve efficiency.
/// Necessary checks should be made before calling this class' APIs for safety.
/// @tparam CharT Character type
/// @tparam Traits Character traits type which defaults to
/// std::char_traits<CharT>.
template <typename CharT, typename Traits = std::char_traits<CharT>>
class basic_str_view {
  static_assert(!std::is_array<CharT>::value,
                "CharT must not be an array type.");
  static_assert(std::is_trivial<CharT>::value &&
 ...
 public:
  ...
  /// (Always constant since this class isn't meant to provide any mutating
  /// features.)
  ...
  /// @brief Constructs a basic_str_view object from a pointer to a character
  /// sequence.
  /// @note std::char_traits::length() is constexpr from C++17.
  /// @param p_str A pointer to a character sequence. (Must be null-terminated,
  ...
  /// @note
  /// This constructor assumes the last element is the null character ('\0'). If
  /// that's not desirable, consider using one of the other overloads.
  ...
  constexpr basic_str_view(const value_type (&str)[N]) noexcept
      : m_len(N - 1), mp_str(&str[0]) {}

  /// @brief Construction from a null pointer is forbidden.
  basic_str_view(std::nullptr_t) = delete;

  /// @brief Constructs a basic_str_view object from a pointer to a character
  /// sequence and its size.
  /// @param p_str A pointer to a character sequence. (May or may not be
  /// null-terminated.)
  ...
  /// @brief Constructs a basic_str_view object from compatible begin/end
  /// iterators
  ...
  template <
      typename ItrType,
      enable_if_t<
          conjunction<is_iterator_of<ItrType, CharT>,
                      std::is_base_of<std::random_access_iterator_tag,
                                      typename std::iterator_traits<
                                          ItrType>::iterator_category>>::value,
          int> = 0>
  basic_str_view(ItrType first, ItrType last) noexcept
      : m_len(last - first), mp_str(&*first) {}

  /// @brief Constructs a basic_str_view object from a compatible
  ...
  const_iterator begin() const noexcept { return mp_str; }

  /// @brief Get the iterator to the past-the-end element. (Always constant)
  /// @return The iterator to the past-the-end element.
  const_iterator end() const noexcept { return mp_str + m_len; }

  /// @brief Get the iterator to the first element. (Always constant)
  /// @return The iterator to the first element.
  const_iterator cbegin() const noexcept { return mp_str; }

  /// @brief Get the iterator to the past-the-end element. (Always constant)
  /// @return The iterator to the past-the-end element.
  const_iterator cend() const noexcept { return mp_str + m_len; }

  /// @brief Get the iterator to the first element in the reverse order. (Always
  /// constant)
  ...
  /// @brief Get the iterator to the past-the-end element in the reverse order.
  /// (Always constant)
  ...
  /// @warning Throws an fkyaml::out_of_range exception if the position exceeds
  /// the character sequence size.
  /// @param pos The position of the target element.
  /// @return The element at the given position.
  ...
  const_reference front() const noexcept { return *mp_str; }

  /// @brief Get the last element.
  /// @return The last element.
  const_reference back() const { return *(mp_str + m_len - 1); }
  ...
  const_pointer data() const noexcept { return mp_str; }
  ...
  /// @brief Shrinks the referenced character sequence from the last by `n`
  /// elements.
  /// @param n The number of elements by which to shrink the sequence from the
  /// last.
  void remove_suffix(size_type n) noexcept { m_len -= n; }
  ...
  /// @brief Copys the referenced character sequence values from `pos` by `n`
  /// size.
  /// @warning Throws an fkyaml::out_of_range exception if the given `pos` is
  ...
  /// @param n The number of elements to the end of a new sub basic_str_view
  /// object.
  ...
  /// @brief Compares the referenced character sequence values with the given
  /// basic_str_view object.
  /// @param sv The basic_str_view object to compare with.
  /// @return The lexicographical comparison result. The values are same as
  /// std::strncmp().
  ...
  /// @brief Compares the referenced character sequence values from `pos1` by
  /// `n1` characters with `sv`.
  ...
  /// @brief Compares the referenced character sequence value from `pos1` by
  /// `n1` characters with `sv` from `pos2` by `n2` characters.
  /// @param pos1 The offset of the beginning element in this character
  /// sequence.
  ...
  /// @return The lexicographical comparison result. The values are same as
  /// std::strncmp().
  int compare(size_type pos1, size_type n1, basic_str_view sv, size_type pos2,
              size_type n2) const {
    return substr(pos1, n1).compare(sv.substr(pos2, n2));
  }

  /// @brief Compares the referenced character sequence with `s` character
  /// sequence.
  /// @param s The pointer to a character sequence to compare with.
  /// @return The lexicographical comparison result. The values are same as
  /// std::strncmp().
  int compare(const CharT* s) const { return compare(basic_str_view(s)); }

  /// @brief Compares the referenced character sequence from `pos1` by `n1`
  /// characters with `s` character sequence.
  /// @param pos1 The offset of the beginning element in this character
  /// sequence.
  ...
  /// @brief Compares the referenced character sequence from `pos1` by `n1`
  /// characters with `s` character sequence by `n2` characters.
  /// @param pos1 The offset of the beginning element in this character
  /// sequence.
  /// @param n1 The length of this character sequence used fo comparison.
  /// @param s The pointer to a character sequence to compare with.
  ...
  /// @return true if the character sequence starts with `sv` characters, false
  /// otherwise.
  ...
  /// @return true if the character sequence starts with `c` character, false
  /// otherwise.
  ...
  /// @return true if the character sequence starts with `s` characters, false
  /// otherwise.
  ...
  /// @param sv The character sequence to compare with.
  /// @return true if the character sequence ends with `sv` characters, false
  /// otherwise.
  bool ends_with(basic_str_view sv) const noexcept {
    const size_type size = m_len;
    const size_type sv_size = sv.size();
    return size >= sv_size &&
           traits_type::compare(end() - sv_size, sv.data(), sv_size) == 0;
  }

  /// @brief Checks if this character sequence ends with `c` character.
  /// @param c The character to compare with.
  /// @return true if the character sequence ends with `c` character, false
  /// otherwise.
  ...
  /// @param s The character sequence to compare with.
  /// @return true if the character sequence ends with `s` characters, false
  /// otherwise.
  ...
  /// @param sv The character sequence to compare with.
  /// @return true if the character sequence contains `sv` characters, false
  /// otherwise.
  bool contains(basic_str_view sv) const noexcept { return find(sv) != npos; }

  /// @brief Checks if this character sequence contains `c` character.
  /// @param c The character to compare with.
  /// @return true if the character sequence contains `c` character, false
  /// otherwise.
  bool contains(CharT c) const noexcept { return find(c) != npos; }

  /// @brief Checks if this character sequence contains `s` characters.
  /// @param s The character sequence to compare with.
  /// @return true if the character sequence contains `s` characters, false
  /// otherwise.
  bool contains(const CharT* s) const noexcept { return find(s) != npos; }

  /// @brief Finds the beginning position of `sv` characters in this referenced
  /// character sequence.
  /// @param sv The character sequence to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  ...
  /// @brief Finds the beginning position of `c` character in this referenced
  /// character sequence.
  ...
  /// @brief Finds the beginning position of `s` character sequence by `n`
  ...
  /// @brief Finds the beginning position of `s` character sequence in this
  ...
  /// @brief Retrospectively finds the beginning position of `sv` characters in
  /// this referenced character sequence.
  /// @param sv The character sequence to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  /// @return The beginning position of `sv` characters, `npos` otherwise.
  ...
  /// @brief Retrospectively finds the beginning position of `c` character in
  /// this referenced character sequence.
  /// @param sv The character to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  ...
  /// @brief Retrospectively finds the beginning position of `s` character
  /// sequence by `n` characters in this referenced character sequence.
  /// @param s The character sequence to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  /// @param n The length of `s` character sequence used for comparison.
  ...
  /// @brief Retrospectively finds the beginning position of `s` character
  /// sequence in this referenced character sequence.
  /// @param s The character sequence to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  /// @return The beginning position of `s` characters, `npos` otherwise.
  ...
  /// @brief Finds the first occurence of `sv` character sequence in this
  /// referenced character sequence.
  /// @param sv The character sequence to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  ...
  /// @brief Finds the first occurence of `c` character in this referenced
  /// character sequence.
  /// @param c The character to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  ...
  /// @brief Finds the first occurence of `s` character sequence by `n`
  /// characters in this referenced character sequence.
  /// @param s The character sequence to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  ...
  /// @brief Finds the first occurence of `s` character sequence in this
  /// referenced character sequence.
  /// @param s The character sequence to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  ...
  /// @brief Finds the last occurence of `sv` character sequence in this
  /// referenced character sequence.
  /// @param sv The character sequence to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  /// @return The beginning position of `sv` characters, `npos` otherwise.
  size_type find_last_of(basic_str_view sv,
                         size_type pos = npos) const noexcept {
    return find_last_of(sv.mp_str, pos, sv.m_len);
  }

  /// @brief Finds the last occurence of `c` character in this referenced
  /// character sequence.
  /// @param c The character to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  ...
  /// @brief Finds the last occurence of `s` character sequence by `n`
  /// characters in this referenced character sequence.
  /// @param s The character sequence to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  ...
  /// @brief Finds the last occurence of `s` character sequence in this
  /// referenced character sequence.
  /// @param s The character sequence to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  ...
  /// @brief Finds the first absence of `sv` character sequence in this
  /// referenced character sequence.
  /// @param sv The character sequence to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  /// @return The beginning position of non `sv` characters, `npos` otherwise.
  size_type find_first_not_of(basic_str_view sv,
                              size_type pos = 0) const noexcept {
    return find_first_not_of(sv.mp_str, pos, sv.m_len);
  }

  /// @brief Finds the first absence of `c` character in this referenced
  /// character sequence.
  /// @param c The character to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  ...
  /// @brief Finds the first absence of `s` character sequence by `n` characters
  /// in this referenced character sequence.
  /// @param s The character sequence to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  /// @param n The length of `s` character sequence used for comparison.
  ...
  /// @brief Finds the first absence of `s` character sequence in this
  /// referenced character sequence.
  /// @param s The character sequence to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  ...
  /// @brief Finds the last absence of `sv` character sequence in this
  /// referenced character sequence.
  /// @param sv The character sequence to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  /// @return The beginning position of non `sv` characters, `npos` otherwise.
  size_type find_last_not_of(basic_str_view sv,
                             size_type pos = npos) const noexcept {
    return find_last_not_of(sv.mp_str, pos, sv.m_len);
  }

  /// @brief Finds the last absence of `c` character in this referenced
  /// character sequence.
  /// @param c The character to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  ...
  /// @brief Finds the last absence of `s` character sequence by `n` characters
  /// in this referenced character sequence.
  /// @param s The character sequence to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
  ...
  /// @brief Finds the last absence of `s` character sequence in this referenced
  /// character sequence.
  /// @param s The character sequence to compare with.
  /// @param pos The offset of the search beginning position in this referenced
  /// character sequence.
...
// Prior to C++17, a static constexpr class member needs an out-of-class
// definition.
#ifndef FK_YAML_HAS_CXX_17

template <typename CharT, typename Traits>
constexpr typename basic_str_view<CharT, Traits>::size_type
    basic_str_view<CharT, Traits>::npos;
...
inline bool operator==(basic_str_view<CharT, Traits> lhs,
  ...
  // Comparing the lengths first will omit unnecessary value comparison in
  // compare().
...
/// @return true if the two objects are the same, false otherwise.
template <typename CharT, typename Traits, std::size_t N>
inline bool operator==(const CharT (&lhs)[N],
                       basic_str_view<CharT, Traits> rhs) noexcept {
...
class position_tracker {
 public:
  ...
  /// @note This function doesn't support cases where cur_pos has moved backward
  /// from the last call.
  ...
  uint32_t get_cur_pos() const noexcept { return m_cur_pos; }

  /// @brief Get the current position in the current line.
  /// @return uint32_t The current position in the current line.
  uint32_t get_cur_pos_in_line() const noexcept { return m_cur_pos_in_line; }
  ...
  uint32_t get_lines_read() const noexcept { return m_lines_read; }
...
enum class lexical_token_t : std::uint8_t {
  ...
  YAML_VER_DIRECTIVE,   //!< a YAML version directive found. use
                        //!< get_yaml_version() to get a value.
  TAG_DIRECTIVE,  //!< a TAG directive found. use GetTagInfo() to get the tag
                  //!< information.
  TAG_PREFIX,     //!< the character for tag prefix `!`
  INVALID_DIRECTIVE,      //!< an invalid directive found. do not try to get the
                          //!< value.
...
class lexical_analyzer {
 ...
 public:
  ...
  /// @brief Get the next lexical token by scanning the left of the input
  /// buffer.
  ...
  lexical_token get_next_token() {
    ...
    switch (*m_cur_itr) {
        ...
        switch (*m_cur_itr) {
          ...
          default:
            ...
            switch (*m_cur_itr) {
              ...
              case '{':
              case '}':
                // Flow indicators are not "safe" to be followed in a flow
                // context. See https://yaml.org/spec/1.2.2/#733-plain-style for
                // more details.
                return {lexical_token_t::KEY_SEPARATOR};
              default:
                ...
                // if a key inside a flow mapping is JSON-like (surrounded by
                // indicators, see below), YAML allows the following value to be
                // specified adjacent to the ":" mapping value indicator.
                // ```yaml
                // # the following flow mapping entries are all valid.
                // {
                //   "foo":true,
                //   'bar':false,          # 'bar' is actually not JSON but
                //   allowed in YAML
                //                         # since its surrounded by the single
                //                         quotes.
                //   {[1,2,3]:null}:"baz"
                // }
                // ```
      ...
      case '%':  // directive prefix
        ...
        // The '%' character can be safely used as the first character in
        // document contents. See
        // https://yaml.org/spec/1.2.2/#912-document-markers for more details.
        break;
      case '-': {
        switch (*(m_cur_itr + 1)) {
          case ' ':
        ...
        if (m_pos_tracker.get_cur_pos_in_line() == 0) {
          if ((m_end_itr - m_cur_itr) > 2) {
            const bool is_dir_end =
                std::equal(m_token_begin_itr, m_cur_itr + 3, "---");
            if (is_dir_end) {
              m_cur_itr += 3;
      ...
      case '@':
        emit_error(
            "Any token cannot start with at(@). It is a reserved indicator for "
            "YAML.");
      case '`':
        emit_error(
            "Any token cannot start with grave accent(`). It is a reserved "
            "indicator for YAML.");
      case '\"':
        ++m_token_begin_itr;
      ...
      case '\'':
        ++m_token_begin_itr;
        return {lexical_token_t::SINGLE_QUOTED_SCALAR,
                determine_single_quoted_scalar_range()};
      case '.': {
        if (m_pos_tracker.get_cur_pos_in_line() == 0) {
              ...
              if (rem_size > 3) {
                switch (*(m_cur_itr + 3)) {
                  ...
                  case '\n':
                    ...
                    break;
                  default:
                    // See https://yaml.org/spec/1.2.2/#912-document-markers for
                    // more details.
                    emit_error(
                        "The document end marker \"...\" must not be followed "
                        "by non-ws char.");
                }
              } else {
                m_cur_itr += 3;
      ...
      case '>': {
        ...
        const uint32_t base_indent =
            get_current_indent_level(&sv[header_end_pos]);
        ...
        return {type, determine_block_scalar_content_range(
                          base_indent, m_block_scalar_header.indent,
  ...
  str_view get_yaml_version() const noexcept { return m_yaml_version; }

  /// @brief Get the YAML tag handle defined in the TAG directive.
  /// @return str_view A tag handle.
  str_view get_tag_handle() const noexcept { return m_tag_handle; }

  /// @brief Get the YAML tag prefix defined in the TAG directive.
  /// @return str_view A tag prefix.
  str_view get_tag_prefix() const noexcept { return m_tag_prefix; }
 ...
 private:
  uint32_t get_current_indent_level(const char* p_line_end) {
    ...
    std::size_t line_begin_pos =
        str_view(m_begin_itr, p_line_end - 1).find_last_of('\n');
    ...
    // If "- ", "? " and/or ": " occur in the first line of this plain scalar
    // content.
    if (context > 0) {
      // Check if the first line contains the key separator ": ".
      // If so, the indent value remains the current one.
      // Otherwise, the indent value is changed based on the last ocurrence of
      // the above 3. In any case, multiline plain scalar content must be
      // indented more than the indent value.
  ...
  /// @note Currently, only %YAML directive is supported. If not, returns
  /// invalid or throws an exception.
  ...
  lexical_token_t scan_directive() {
    ...
    if (dir_name == "TAG") {
      if FK_YAML_UNLIKELY (!ends_loop) {
        emit_error(
            "There must be at least one white space between \"%TAG\" and tag "
            "info.");
      }
      skip_white_spaces();
    ...
    if (dir_name == "YAML") {
      if FK_YAML_UNLIKELY (!ends_loop) {
        emit_error(
            "There must be at least one white space between \"%YAML\" and "
            "version.");
      }
      skip_white_spaces();
  ...
  lexical_token_t scan_tag_directive() {
          ...
          switch (*m_cur_itr) {
            ...
            default:
              if FK_YAML_UNLIKELY (!isalnum(*m_cur_itr)) {
                // See https://yaml.org/spec/1.2.2/#rule-c-named-tag-handle for
                // more details.
                emit_error(
                    "named handle can contain only numbers(0-9), "
                    "alphabets(A-Z,a-z) and hyphens(-).");
              }
              break;
          }
    ...
    switch (*m_cur_itr) {
      ...
      // See https://yaml.org/spec/1.2.2/#rule-ns-global-tag-prefix for more
      // details.
      case ',':
      case '[':
      case ']':
      ...
      case '}':
        emit_error(
            "tag prefix must not start with flow indicators (\',\', [], {}).");
      default:
        break;
    }
  ...
  /// @return lexical_token_t The lexical token type for YAML version
  /// directives.
  ...
  str_view extract_anchor_name() {
    ...
    for (; m_cur_itr != m_end_itr; ++m_cur_itr) {
      switch (*m_cur_itr) {
        // anchor name must not contain white spaces, newline codes and flow
        // indicators. See https://yaml.org/spec/1.2.2/#692-node-anchors for
        // more details.
        case ' ':
        case '\t':
        case '\n':
  ...
  str_view extract_tag_name() {
    ...
    if (is_verbatim) {
      ...
      const bool is_valid_uri =
          uri_encoding::validate(tag_body.begin(), tag_body.end());
    ...
    if (is_named_handle) {
      ...
      if FK_YAML_UNLIKELY (last == '!') {
        ...
        // See the "Tag Shorthands" section in
        // https://yaml.org/spec/1.2.2/#691-node-tags.
        emit_error("named handle has no suffix.");
      }
    }

    // get the position of last tag prefix character (!) to extract body of tag
    // shorthands. tag shorthand is either primary(!tag), secondary(!!tag) or
    // named(!handle!tag).
    ...
    const bool is_valid_uri =
        uri_encoding::validate(tag_uri.begin(), tag_uri.end());
    if FK_YAML_UNLIKELY (!is_valid_uri) {
  ...
  /// @brief Determines the range of single quoted scalar by scanning remaining
  ...
  str_view determine_single_quoted_scalar_range() {
    ...
    while (pos != str_view::npos) {
      ...
      // If single quotation marks are repeated twice in a single quoted scalar,
      // they are considered as an escaped single quotation mark. Skip the
      // second one which would otherwise be detected as a closing single
      // quotation mark in the next loop.
      pos = sv.find('\'', pos + 2);
    }
  ...
  /// @brief Determines the range of double quoted scalar by scanning remaining
  /// input buffer contents.
  ...
  str_view determine_double_quoted_scalar_range() {
    ...
    while (pos != str_view::npos) {
      ...
      if FK_YAML_LIKELY (pos > 0) {
        // Double quotation marks can be escaped by a preceding backslash and
        // the number of backslashes matters to determine if the found double
        // quotation mark is escaped since the backslash itself can also be
        // escaped:
        // * odd number of backslashes  -> double quotation mark IS escaped
        // (e.g., "\\\"")
        // * even number of backslashes -> double quotation mark IS NOT escaped
        // (e.g., "\\"")
  ...
  /// @brief Determines the range of plain scalar by scanning remaining input
  /// buffer contents.
    ...
    do {
      ...
      switch (sv[pos]) {
        case '\n': {
          ...
          const std::size_t non_space_pos =
              sv.find_first_not_of(space_filter, pos);
          const std::size_t last_newline_pos =
              sv.find_last_of('\n', non_space_pos);
        ...
        case ' ':
          ...
          // Allow a space in a plain scalar only if the space is surrounded by
          // non-space characters, but not followed by the comment prefix " #".
          // Also, flow indicators are not allowed to be followed after a space
          // in a flow context. See https://yaml.org/spec/1.2.2/#733-plain-style
          // for more details.
          switch (sv[pos + 1]) {
            case ' ':
            case '\t':
            ...
            case '#':
              ...
              break;
            case ':':
              // " :" is permitted in a plain style string token, but not when
              // followed by a space.
  ...
  /// @brief Scan a block style string token either in the literal or folded
  /// style.
  /// @param base_indent The base indent level of the block scalar.
  /// @param indicated_indent The indicated indent level in the block scalar
  /// header. 0 means it's not indicated.
  /// @param token Storage for the scanned block scalar range.
  /// @return The content indentation level of the block scalar.
  str_view determine_block_scalar_content_range(uint32_t base_indent,
                                                uint32_t indicated_indent,
    ...
    while (cur_itr != m_end_itr) {
      switch (*cur_itr++) {
        case ' ':
          ...
          continue;
        case '\t':
          // Tabs are not counted as an indent character but still part of an
          // empty line. See https://yaml.org/spec/1.2.2/#rule-s-indent and
    ...
    // all the block scalar contents are empty lines, and no subsequent token
    // exists.
    if FK_YAML_UNLIKELY (cur_itr == m_end_itr) {
      ...
      content_indent = indicated_indent == 0
                           ? std::max(cur_indent, max_leading_indent)
                           : base_indent + indicated_indent;
      return sv;
    }

    // Any leading empty line must not contain more spaces than the first
    // non-empty line.
    if FK_YAML_UNLIKELY (cur_indent < max_leading_indent) {
      emit_error(
          "Any leading empty line must not be more indented than the first "
          "non-empty line.");
    }
    ...
    if (indicated_indent == 0) {
      ...
      emit_error(
          "The first non-empty line in the block scalar is less indented.");
    }

    std::size_t last_newline_pos =
    ...
    while (last_newline_pos < remain_input_len) {
      ...
      cur_indent = static_cast<uint32_t>(cur_line_content_begin_pos -
                                         last_newline_pos - 1);
      if (cur_indent < content_indent &&
          sv[cur_line_content_begin_pos] != '\n') {
        if FK_YAML_UNLIKELY (cur_indent > base_indent) {
          // This path assumes an input like the following:
          // ```yaml
          // foo: |
          //   text
          //  invalid # this line is less indented than the content indent level
          //  (2)
          //          # but more indented than the base indent level (0)
          // ```
          // In such cases, the less indented line cannot be the start of the
          // next token.
          emit_error("A content line of the block scalar is less indented.");
        }

        // Interpret less indented non-space characters as the start of the next
        // token.
        break;
      }
    ...
    // include last newline character if not all characters have been consumed
    // yet.
  ...
  /// @brief Checks if the given scalar contains no unescaped control
  /// characters.
  ...
  void check_scalar_content(const str_view& scalar) const {
    ...
    while (p_current != p_end) {
      const uint32_t num_bytes =
          utf8::get_num_bytes(static_cast<uint8_t>(*p_current));
      if (num_bytes > 1) {
      ...
      switch (*p_current++) {
        ...
        case 0x02:
          emit_error(
              "Control character U+0002 (STX) must be escaped to \\u0002.");
        case 0x03:
          emit_error(
              "Control character U+0003 (ETX) must be escaped to \\u0003.");
        case 0x04:
          emit_error(
              "Control character U+0004 (EOT) must be escaped to \\u0004.");
        case 0x05:
          emit_error(
              "Control character U+0005 (ENQ) must be escaped to \\u0005.");
        case 0x06:
          emit_error(
              "Control character U+0006 (ACK) must be escaped to \\u0006.");
        case 0x07:
          emit_error(
        ...
        case 0x08:
          emit_error(
              "Control character U+0008 (BS) must be escaped to \\b or "
              "\\u0008.");
        case 0x09:  // HT
        ...
        case 0x0B:
          emit_error(
              "Control character U+000B (VT) must be escaped to \\v or "
              "\\u000B.");
        case 0x0C:
          emit_error(
              "Control character U+000C (FF) must be escaped to \\f or "
              "\\u000C.");
        // 0x0D(CR) has already been handled above.
        case 0x0E:
          emit_error(
              "Control character U+000E (SO) must be escaped to \\u000E.");
        case 0x0F:
          emit_error(
              "Control character U+000F (SI) must be escaped to \\u000F.");
        case 0x10:
          emit_error(
              "Control character U+0010 (DLE) must be escaped to \\u0010.");
        case 0x11:
          emit_error(
              "Control character U+0011 (DC1) must be escaped to \\u0011.");
        case 0x12:
          emit_error(
              "Control character U+0012 (DC2) must be escaped to \\u0012.");
        case 0x13:
          emit_error(
              "Control character U+0013 (DC3) must be escaped to \\u0013.");
        case 0x14:
          emit_error(
              "Control character U+0014 (DC4) must be escaped to \\u0014.");
        case 0x15:
          emit_error(
              "Control character U+0015 (NAK) must be escaped to \\u0015.");
        case 0x16:
          emit_error(
              "Control character U+0016 (SYN) must be escaped to \\u0016.");
        case 0x17:
          emit_error(
              "Control character U+0017 (ETB) must be escaped to \\u0017.");
        case 0x18:
          emit_error(
              "Control character U+0018 (CAN) must be escaped to \\u0018.");
        case 0x19:
          emit_error(
              "Control character U+0019 (EM) must be escaped to \\u0019.");
        case 0x1A:
          emit_error(
              "Control character U+001A (SUB) must be escaped to \\u001A.");
        case 0x1B:
          emit_error(
              "Control character U+001B (ESC) must be escaped to \\e or "
              "\\u001B.");
        case 0x1C:
          emit_error(
              "Control character U+001C (FS) must be escaped to \\u001C.");
        case 0x1D:
          emit_error(
              "Control character U+001D (GS) must be escaped to \\u001D.");
        case 0x1E:
          emit_error(
              "Control character U+001E (RS) must be escaped to \\u001E.");
        case 0x1F:
          emit_error(
      ...
      switch (c) {
        ...
        case '9':
          if FK_YAML_UNLIKELY (header.indent > 0) {
            emit_error(
                "Invalid indentation level for a block scalar. It must be "
                "between 1 and 9.");
          }
  ...
  /// @brief Skip white spaces (half-width spaces and tabs) from the current
  /// position.
  void skip_white_spaces() {
    m_cur_itr = std::find_if_not(
        m_cur_itr, m_end_itr, [](char c) { return (c == ' ' || c == '\t'); });
  }

  /// @brief Skip white spaces and newline codes (CR/LF) from the current
  /// position.
...
// Implementations in this header file are intentionally optimized for
// conversions between YAML scalars and native C++ types. So, some
// implementations don't follow the convensions in the standard C++ functions.
// For example, octals must begin with "0o" (not "0"), which is specified in the
// YAML spec 1.2.
...
/// @brief A structure which provides limits for conversions between scalars and
/// integers.
/// @note This structure contains common limits in both signed and unsigned
/// integers.
/// @tparam NumBytes The number of bytes for the integer type.
template <std::size_t NumBytes>
struct conv_limits_base {};

/// @brief The specialization of conv_limits_base for 1 byte integers, e.g.,
/// int8_t, uint8_t.
template <>
struct conv_limits_base<1u> {
  ...
  /// @brief Check if the given hexadecimals are safely converted into 1 byte
  /// integer.
  /// @param octs The pointer to hexadecimal characters
  /// @param len The length of hexadecimal characters
  /// @return true is safely convertible, false otherwise.
  static bool check_if_hexs_safe(const char* /*unused*/,
                                 std::size_t len) noexcept {
    return len <= max_chars_hex;
  }
};

/// @brief The specialization of conv_limits_base for 2 byte integers, e.g.,
/// int16_t, uint16_t.
template <>
struct conv_limits_base<2u> {
  ...
  /// @brief Check if the given hexadecimals are safely converted into 2 byte
  /// integer.
  /// @param octs The pointer to hexadecimal characters
  /// @param len The length of hexadecimal characters
  /// @return true is safely convertible, false otherwise.
...
/// @brief The specialization of conv_limits_base for 4 byte integers, e.g.,
/// int32_t, uint32_t.
template <>
struct conv_limits_base<4u> {
  ...
  /// @brief Check if the given hexadecimals are safely converted into 4 byte
  /// integer.
  /// @param octs The pointer to hexadecimal characters
  /// @param len The length of hexadecimal characters
  /// @return true is safely convertible, false otherwise.
...
/// @brief The specialization of conv_limits_base for 8 byte integers, e.g.,
/// int64_t, uint64_t.
template <>
struct conv_limits_base<8u> {
  /// max characters for octals (0o1777777777777777777777) without the prefix
  /// part.
  static constexpr std::size_t max_chars_oct = 22;
  /// max characters for hexadecimals (0xFFFFFFFFFFFFFFFF) without the prefix
  /// part.
  ...
  /// @brief Check if the given hexadecimals are safely converted into 8 byte
  /// integer.
  /// @param octs The pointer to hexadecimal characters
  /// @param len The length of hexadecimal characters
  /// @return true is safely convertible, false otherwise.
...
/// @brief The specialization of conv_limits for 1 byte signed integers, e.g.,
/// int8_t.
template <>
struct conv_limits<1u, true> : conv_limits_base<1u> {
  ...
  static const char* max_value_chars_dec() noexcept {
    ...
    // Although the issue has been fixed since C++17, this workaround is
    // necessary to let this functionality work with C++11 (the library's
    // default C++ standard version). The same thing is applied to similar
    // functions in the other specializations.
...
/// @brief The specialization of conv_limits for 1 byte unsigned integers, e.g.,
/// uint8_t.
template <>
...
/// @brief The specialization of conv_limits for 2 byte signed integers, e.g.,
/// int16_t.
template <>
...
/// @brief The specialization of conv_limits for 2 byte unsigned integers, e.g.,
/// uint16_t.
template <>
...
/// @brief The specialization of conv_limits for 4 byte signed integers, e.g.,
/// int32_t.
template <>
...
/// @brief The specialization of conv_limits for 4 byte unsigned integers, e.g.,
/// uint32_t.
template <>
...
/// @brief The specialization of conv_limits for 8 byte signed integers, e.g.,
/// int64_t.
template <>
struct conv_limits<8u, true> : conv_limits_base<8u> {
  ...
  /// max characters for decimals (-9223372036854775808..9223372036854775807)
  /// without sign.
...
/// @brief The specialization of conv_limits for 8 byte unsigned integers, e.g.,
/// uint64_t.
template <>
...
/// @tparam CharItr Type of char iterators. Its value type must be `char` (maybe
/// cv-qualified).
...
/// @param /*unused*/ The null value holder (unused since it can only have
/// `nullptr`)
/// @return true if the conversion completes successfully, false otherwise.
template <typename CharItr>
inline bool aton(CharItr begin, CharItr end,
                 std::nullptr_t& /*unused*/) noexcept {
  static_assert(is_iterator_of<CharItr, char>::value,
                "aton() accepts iterators for char type");
  ...
  if FK_YAML_LIKELY (len == 4) {
    ...
    return (std::strncmp(p_begin, "null", 4) == 0) ||
...
/// @tparam CharItr The type of char iterators. Its value type must be `char`
/// (maybe cv-qualified).
...
inline bool atob(CharItr begin, CharItr end, BoolType& boolean) noexcept {
  static_assert(is_iterator_of<CharItr, char>::value,
                "atob() accepts iterators for char type");
...
/// @brief Converts a scalar into decimals. This is common implementation for
/// both signed/unsigned integer types.
/// @warning
/// This function does NOT care about overflows if IntType is unsigned. The
/// source string value must be validated beforehand by calling either
/// atoi_dec_pos() or atoi_dec_neg() functions. Furthermore, `p_begin` and
/// `p_end` must NOT be null. Validate them before calling this function.
/// @tparam IntType The output integer type. It can be either signed or
/// unsigned.
...
inline bool atoi_dec_unchecked(const char* p_begin, const char* p_end,
                               IntType& i) noexcept {
  static_assert(is_non_bool_integral<IntType>::value,
                "atoi_dec_unchecked() accepts non-boolean integral types as an "
                "output type");
...
/// @brief Converts a scalar into positive decimals. This function executes
/// bounds check to avoid overflow.
/// @warning `p_begin` and `p_end` must not be null. Validate them before
...
/// @return true if the conversion completes successfully, false otherwise.
template <typename IntType>
inline bool atoi_dec_pos(const char* p_begin, const char* p_end,
                         IntType& i) noexcept {
  static_assert(
  ...
  using conv_limits_type =
      conv_limits<sizeof(IntType), std::is_signed<IntType>::value>;
...
/// @brief Converts a scalar into negative decimals. This function executes
/// bounds check to avoid underflow.
/// @warning `p_begin` and `p_end` must not be null. Validate them before
/// calling this function.
...
/// @return true if the conversion completes successfully, false otherwise.
template <typename IntType>
inline bool atoi_dec_neg(const char* p_begin, const char* p_end,
                         IntType& i) noexcept {
  static_assert(
      is_non_bool_integral<IntType>::value,
...
/// @brief Converts a scalar into octals. This function executes bounds check to
/// avoid overflow.
/// @warning `p_begin` and `p_end` must not be null. Validate them before
/// calling this function.
/// @tparam IntType The output integer type. It can be either signed or
...
/// @return true if the conversion completes successfully, false otherwise.
template <typename IntType>
inline bool atoi_oct(const char* p_begin, const char* p_end,
                     IntType& i) noexcept {
  static_assert(
      is_non_bool_integral<IntType>::value,
...
/// @brief Converts a scalar into hexadecimals. This function executes bounds
...
/// @return true if the conversion completes successfully, false otherwise.
template <typename IntType>
inline bool atoi_hex(const char* p_begin, const char* p_end,
                     IntType& i) noexcept {
  static_assert(
      is_non_bool_integral<IntType>::value,
...
/// @brief Converts a scalar into integers. This function executes bounds check
...
inline bool atoi(CharItr begin, CharItr end, IntType& i) noexcept {
  static_assert(is_iterator_of<CharItr, char>::value,
                "atoi() accepts iterators for char type");
  static_assert(is_non_bool_integral<IntType>::value,
...
/// @param f The output `float` value holder.
inline void set_nan(float& f) noexcept { f = std::nanf(""); }

/// @brief Set a NaN `double` value.
/// @param f The output `double` value holder.
inline void set_nan(double& f) noexcept { f = std::nan(""); }
...
inline bool atof_impl(const char* p_begin, const char* p_end,
                      FloatType& f) noexcept {
...
inline bool atof(CharItr begin, CharItr end,
                 FloatType& f) noexcept(noexcept(atof_impl(&*begin, &*begin,
                                                           f))) {
  static_assert(is_iterator_of<CharItr, char>::value,
                "atof() accepts iterators for char type");
  ...
  if (*p_begin == '-' || *p_begin == '+') {
    ...
    if (*p_begin == '+') {
      // Skip the positive sign since it's sometimes not recognized as part of
      // float value.
      ++p_begin;
    }
  } else if (len == 4) {
...
class yaml_escaper {
 ...
 private:
  ...
  static bool extract_codepoint(const char*& begin, const char* end,
                                int bytes_to_read, char32_t& codepoint) {
...
class scalar_scanner {
 public:
  /// @brief Detects a scalar value type by scanning the contents ranged by the
  /// given iterators.
  /// @param begin The iterator to the first element of the scalar.
  /// @param end The iterator to the past-the-end element of the scalar.
 ...
 private:
  /// @brief Detects a scalar value type from the contents (possibly an integer
  ...
  /// @return A detected scalar value type.
  static node_type scan_possible_number_token(const char* itr,
                                              uint32_t len) noexcept {
    FK_YAML_ASSERT(len > 0);

    switch (*itr) {
      case '-':
        return (len > 1) ? scan_negative_number(++itr, --len)
                         : node_type::STRING;
      case '+':
        return (len > 1) ? scan_decimal_number(++itr, --len)
                         : node_type::STRING;
      case '.':
        // some integer(s) required after the decimal point as a floating point
        // value.
        return (len > 1) ? scan_after_decimal_point(++itr, --len)
                         : node_type::STRING;
      case '0':
        return (len > 1) ? scan_after_zero_at_first(++itr, --len)
                         : node_type::INTEGER;
      case '1':
      case '2':
      case '3':
  ...
  /// @brief Detects a scalar value type by scanning the contents right after
  /// the negative sign.
  /// @param itr The iterator to the past-the-negative-sign element of the
  /// scalar.
  /// @param len The length of the scalar contents left unscanned.
  /// @return A detected scalar value type.
  static node_type scan_negative_number(const char* itr,
                                        uint32_t len) noexcept {
    FK_YAML_ASSERT(len > 0);
  ...
  /// @return A detected scalar value type.
  static node_type scan_after_zero_at_first(const char* itr,
                                            uint32_t len) noexcept {
    FK_YAML_ASSERT(len > 0);

    if (is_digit(*itr)) {
      // a token consisting of the beginning '0' and some following numbers,
      // e.g., `0123`, is not an integer according to
    ...
    switch (*itr) {
      ...
      case 'e':
      case 'E':
        // some integer(s) required after the exponent sign as a floating point
        // value.
        return (len > 1) ? scan_after_exponent(++itr, --len)
                         : node_type::STRING;
      case 'o':
        return (len > 1) ? scan_octal_number(++itr, --len) : node_type::STRING;
      case 'x':
        return (len > 1) ? scan_hexadecimal_number(++itr, --len)
                         : node_type::STRING;
      default:
        return node_type::STRING;
    }
  ...
  /// @brief Detects a scalar value type by scanning the contents part starting
  /// with a decimal.
  ...
  /// @brief Detects a scalar value type by scanning the contents right after a
  /// decimal point.
  /// @param itr The iterator to the past-the-decimal-point element of the
  /// scalar.
  /// @param len The length of the scalar left unscanned.
  /// @return A detected scalar value type.
  static node_type scan_after_decimal_point(const char* itr,
                                            uint32_t len) noexcept {
    FK_YAML_ASSERT(len > 0);
  ...
  /// @brief Detects a scalar value type by scanning the contents right after
  /// the exponent prefix ("e" or "E").
  /// @param itr The iterator to the past-the-exponent-prefix element of the
  /// scalar.
  /// @param len The length of the scalar left unscanned.
  /// @return A detected scalar value type.
  ...
  /// @brief Detects a scalar value type by scanning the contents assuming octal
  /// numbers.
  ...
  /// @brief Detects a scalar value type by scanning the contents assuming
  /// hexadecimal numbers.
  /// @param itr The iterator to the hexadecimal-number element of the scalar.
  /// @param len The length of the scalar left unscanned.
  /// @return A detected scalar value type.
  static node_type scan_hexadecimal_number(const char* itr,
                                           uint32_t len) noexcept {
    FK_YAML_ASSERT(len > 0);
  ...
  /// @note This function is needed to avoid assertion failures in
  /// `std::isdigit()` especially when compiled with MSVC.
  ...
  /// @note This function is needed to avoid assertion failures in
  /// `std::isxdigit()` especially when compiled with MSVC.
  /// @param c A character to be checked.
...
class scalar_parser {
 ...
 public:
  ...
  /// @brief Parses a token into a flow scalar (either plain, single quoted or
  /// double quoted)
  ...
  basic_node_type parse_flow(lexical_token_t lex_type, tag_t tag_type,
                             str_view token) {
  ...
  basic_node_type parse_block(lexical_token_t lex_type, tag_t tag_type,
                              str_view token,
 ...
 private:
      ...
      if (token[pos] == '\\') {
        ...
        if (token[pos + 1] != '\n') {
          ...
          if FK_YAML_UNLIKELY (!is_valid_escaping) {
            throw parse_error(
                "Unsupported escape sequence is found in a double quoted "
                "scalar.",
                m_line, m_indent);
          }
  ...
  /// @param header Block scalar header information.
  /// @return View into the parsed scalar contents.
  str_view parse_block_literal_scalar(str_view token,
                                      const block_scalar_header& header) {
  ...
  /// @param header Block scalar header information.
  /// @return View into the parsed scalar contents.
  str_view parse_block_folded_scalar(str_view token,
                                     const block_scalar_header& header) {
    if FK_YAML_UNLIKELY (token.empty()) {
      return token;
    }
    ...
    do {
      ...
      const str_view line = token.substr(cur_line_begin_pos, line_size);
      const bool is_empty =
          line.find_first_not_of(white_space_filter) == str_view::npos;
      ...
      if (line.size() <= header.indent) {
        ...
        const std::size_t non_space_pos =
            line.find_first_not_of(white_space_filter);
        const bool is_more_indented = (non_space_pos != str_view::npos) &&
                                      (non_space_pos > header.indent);
  ...
  /// @brief Discards final content line break and trailing empty lines
  /// depending on the given chomping type.
    ...
    switch (chomp) {
      case chomping_indicator_t::STRIP: {
        ...
        if (content_end_pos == std::string::npos) {
          // if the scalar has no content line, all lines are considered as
  ...
  void process_line_folding(str_view& token, std::size_t newline_pos) noexcept {
    // discard trailing white spaces which precedes the line break in the
    // current line.
  ...
  /// @brief Decides scalar value type based on the lexical/tag types and scalar
  /// contents.
  /// @param lex_type Lexical token type for the scalar.
  /// @param tag_type Tag type for the scalar.
  /// @param token Scalar contents.
  ...
  node_type decide_value_type(lexical_token_t lex_type, tag_t tag_type,
    ...
    switch (tag_type) {
      ...
      case tag_t::NON_SPECIFIC:
        ...
        // See the "Non-Specific Tags" section in
        // https://yaml.org/spec/1.2.2/#691-node-tags.
  ...
  basic_node_type create_scalar_node(node_type val_type, tag_t tag_type,
                                     str_view token) {
    switch (val_type) {
      case node_type::NULL_OBJECT: {
        ...
        if FK_YAML_UNLIKELY (!converted) {
          throw parse_error("Failed to convert a scalar to a null.", m_line,
                            m_indent);
        }
      ...
      case node_type::BOOLEAN: {
        ...
        const bool converted =
            detail::atob(token.begin(), token.end(), boolean);
        if FK_YAML_UNLIKELY (!converted) {
          throw parse_error("Failed to convert a scalar to a boolean.", m_line,
                            m_indent);
        }
      ...
      case node_type::INTEGER: {
        ...
        const bool converted =
            detail::atoi(token.begin(), token.end(), integer);
        if FK_YAML_LIKELY (converted) {
          return basic_node_type(integer);
        }
        if FK_YAML_UNLIKELY (tag_type == tag_t::INTEGER) {
          throw parse_error("Failed to convert a scalar to an integer.", m_line,
                            m_indent);
        }
      ...
      case node_type::FLOAT: {
        ...
        const bool converted =
            detail::atof(token.begin(), token.end(), float_val);
        if FK_YAML_LIKELY (converted) {
          return basic_node_type(float_val);
        }
        if FK_YAML_UNLIKELY (tag_type == tag_t::FLOATING_NUMBER) {
          throw parse_error(
              "Failed to convert a scalar to a floating point value", m_line,
              m_indent);
        }
  ...
  /// Owned buffer storage for parsing. This buffer is used when scalar contents
  /// need mutation.
  std::string m_buffer;
};
...
class tag_resolver {
 ...
 public:
  /// @brief Resolve the input tag name into an expanded tag name prepended with
  /// a registered prefix.
  /// @param tag The input tag name.
  /// @return The type of a node deduced from the given tag name.
  static tag_t resolve_tag(
      const str_view tag,
 ...
 private:
  static std::string normalize_tag_name(
    ...
    if FK_YAML_UNLIKELY (tag[0] != '!') {
      throw invalid_tag("tag must start with \'!\'",
                        std::string(tag.begin(), tag.end()).c_str());
    }
    ...
    if (tag.size() == 1) {
      ...
      // See the "Non-Specific Tags" section in
      // https://yaml.org/spec/1.2.2/#691-node-tags. The interpretation cannot
      // take place here because the input lacks the corresponding value.
    ...
    switch (tag[1]) {
      ...
      default: {
        ...
        if (tag_end_pos != std::string::npos) {
          ...
          const bool is_null_or_empty =
              !directives || directives->named_handle_map.empty();
          if FK_YAML_UNLIKELY (is_null_or_empty) {
            throw invalid_tag("named handle has not been registered.",
                              std::string(tag.begin(), tag.end()).c_str());
          }

          // find the extracted named handle in the map.
          const str_view named_handle = tag.substr(0, tag_end_pos + 1);
          auto named_handle_itr = directives->named_handle_map.find(
              {named_handle.begin(), named_handle.end()});
          ...
          // The YAML spec prohibits expanding the percent-encoded characters
          // (%xx -> a UTF-8 byte). So no conversion takes place. See
          // https://yaml.org/spec/1.2.2/#56-miscellaneous-characters for more
          // details.
...
/// @brief Type traits to check if InputAdapterType has get_buffer_view member
/// function.
/// @tparam InputAdapterType An input adapter type to check if it has
/// get_buffer_view function.
/// @tparam typename N/A
...
/// @brief A partial specialization of is_input_adapter if T is an input adapter
/// type.
/// @tparam InputAdapterType
template <typename InputAdapterType>
struct is_input_adapter<
    InputAdapterType, enable_if_t<has_get_buffer_view<InputAdapterType>::value>>
    : std::true_type {};
...
constexpr node_attr_t scalar_bits =
    null_bit | bool_bit | int_bit | float_bit | string_bit;
...
/// @brief Get an anchor offset used to reference an anchor node from the given
/// attribute bits.
...
struct node_property {
  ...
  std::string tag{};  // NOLINT(readability-redundant-member-init) necessary for
                      // older compilers
  /// The anchor name property.
  std::string anchor{};  // NOLINT(readability-redundant-member-init) necessary
                         // for older compilers
};
...
class basic_deserializer {
  ...
  enum class context_state_t : std::uint8_t {
    ...
    BLOCK_MAPPING_EXPLICIT_KEY,    //!< The underlying node is an explicit block
                                   //!< mapping key.
    BLOCK_MAPPING_EXPLICIT_VALUE,  //!< The underlying node is an explicit block
                                   //!< mapping value.
  ...
  struct parse_context {
    ...
    /// @brief Construct a new parse_context object with non-default values for
    /// each parameter.
    /// @param line The current line. (count from zero)
    /// @param indent The indentation width in the current line. (count from
    /// zero)
  ...
  enum class flow_token_state_t : std::uint8_t {
    ...
    NEEDS_SEPARATOR_OR_SUFFIX,  //!< Either separator (`,`) or flow suffix (`]`
                                //!< or `}`)
  };
 ...
 public:
  ...
  /// @note
  /// If the input consists of multiple YAML documents, this function only
  /// parses the first. If the input may have multiple YAML documents all of
  /// which must be parsed into nodes, prefer the `deserialize_docs()` function.
  ...
  /// @return basic_node_type A root YAML node deserialized from the source
  /// string.
  template <typename InputAdapterType,
            enable_if_t<is_input_adapter<InputAdapterType>::value, int> = 0>
  basic_node_type deserialize(
      InputAdapterType&&
          input_adapter) {  // NOLINT(cppcoreguidelines-missing-std-forward)
  ...
  /// @param input_adapter An input adapter object for the input source buffer.
  /// @return std::vector<basic_node_type> Root YAML nodes for deserialized YAML
  /// documents.
  template <typename InputAdapterType,
            enable_if_t<is_input_adapter<InputAdapterType>::value, int> = 0>
  // NOLINTNEXTLINE(cppcoreguidelines-missing-std-forward)
  std::vector<basic_node_type> deserialize_docs(
 ...
 private:
  ...
  /// @return basic_node_type A root YAML node deserialized from the YAML
  /// document.
  basic_node_type deserialize_document(lexer_type& lexer,
    ...
    switch (token.type) {
      case lexical_token_t::SEQUENCE_BLOCK_PREFIX: {
        ...
        if (found_props) {
          // If node properties are found before the block sequence entry
          // prefix, the properties belong to the root sequence node.
      ...
      case lexical_token_t::EXPLICIT_KEY_PREFIX: {
        // If the explicit key prefix (? ) is detected here, the root node of
        // current document must be a mapping. Also, tag and anchor if any are
        // associated to the root mapping node. No get_next_token() call here to
        // handle the token event in the deserialize_node() function.
        root = basic_node_type::mapping();
        apply_directive_set(root);
        apply_node_properties(root);
        parse_context context(lexer.get_lines_processed(),
      ...
      case lexical_token_t::BLOCK_FOLDED_SCALAR:
        // If a block scalar token is detected here, current document contains
        // single scalar. Do nothing here since the token is handled in the
      ...
      case lexical_token_t::ALIAS_PREFIX:
        // Defer handling the above token events until the next call on the
        // deserialize_scalar() function since the meaning depends on subsequent
        // events.
        if (found_props && line < lexer.get_lines_processed()) {
          // If node properties and a followed node are on the different line,
          // the properties belong to the root node.
      ...
      switch (token.type) {
          ...
          switch (tag_handle_view.size()) {
            case 1 /* ! */: {
              const bool is_already_specified =
                  !mp_meta->primary_handle_prefix.empty();
              ...
              mp_meta->primary_handle_prefix.assign(tag_prefix.begin(),
                                                    tag_prefix.end());
              lacks_end_of_directives_marker = true;
              break;
            }
            case 2 /* !! */: {
              const bool is_already_specified =
                  !mp_meta->secondary_handle_prefix.empty();
              if FK_YAML_UNLIKELY (is_already_specified) {
                throw parse_error(
              ...
              const str_view tag_prefix = lexer.get_tag_prefix();
              mp_meta->secondary_handle_prefix.assign(tag_prefix.begin(),
                                                      tag_prefix.end());
              lacks_end_of_directives_marker = true;
              break;
            }
            default /* !<handle>! */: {
              std::string tag_handle(tag_handle_view.begin(),
                                     tag_handle_view.end());
              const str_view tag_prefix_view = lexer.get_tag_prefix();
              std::string tag_prefix(tag_prefix_view.begin(),
                                     tag_prefix_view.end());
              const bool is_already_specified =
        ...
        default:
          if FK_YAML_UNLIKELY (lacks_end_of_directives_marker) {
            throw parse_error(
                "The end of directives marker (---) is missing after "
                "directives.",
                lexer.get_lines_processed(), lexer.get_last_token_begin_pos());
          }
  ...
  void deserialize_node(lexer_type& lexer, const lexical_token& first_token,
                        uint32_t first_line, uint32_t first_indent,
                        lexical_token_t& last_type) {
      ...
      switch (token.type) {
        case lexical_token_t::EXPLICIT_KEY_PREFIX: {
          ...
          switch (m_context_stack.back().state) {
            ...
            case context_state_t::BLOCK_SEQUENCE_ENTRY:
              // This path is needed in case the input contains nested explicit
              // keys.
              // ```yaml
              // foo:
              //   ? ? foo
          ...
          if (token.type == lexical_token_t::SEQUENCE_BLOCK_PREFIX) {
            ...
            m_context_stack.emplace_back(
                line, indent, context_state_t::BLOCK_MAPPING_EXPLICIT_KEY,
                p_node);
        ...
        case lexical_token_t::KEY_SEPARATOR: {
          ...
          if FK_YAML_UNLIKELY (m_context_stack.back().state ==
            ...
            // - : foo
            // ```
            throw parse_error("sequence key should not be empty.", line,
                              indent);
          }
          ...
          const bool found_props =
              deserialize_node_properties(lexer, token, line, indent);
          if (found_props && line == lexer.get_lines_processed()) {
            // defer applying node properties for the subsequent node on the
            // same line.
            continue;
          }
          ...
          if (is_implicit_same_line) {
            // a key separator for an implicit key with its value on the same
            // line.
            continue;
          }
          ...
          if (line > old_line) {
            if (m_needs_tag_impl) {
              ...
              if (tag_type == tag_t::MAPPING || tag_type == tag_t::CUSTOM_TAG) {
                // set YAML node properties here to distinguish them from those
                // for the first key node as shown in the following snippet:
                //
                // ```yaml
                // foo: !!map
                //   !!str 123: true
            ...
            if (token.type == lexical_token_t::SEQUENCE_BLOCK_PREFIX) {
              ...
              *mp_current_node = basic_node_type::sequence({basic_node_type()});
              ...
              if (has_props) {
                ...
                // if node properties and the followed node are on different
                // lines (i.e., the properties are for a container node), the
            ...
            if (indent <= m_context_stack.back().indent) {
              ...
              // ```
              pop_to_parent_node(
                  line, indent, [indent](const parse_context& c) {
                    return (c.state == context_state_t::BLOCK_MAPPING) &&
                           (indent == c.indent);
                  });
            }

            // defer checking the existence of a key separator after the
            // following scalar until the next deserialize_scalar() call.
            continue;
          }
          ...
          m_context_stack.back().p_node->as_map().emplace(key_node,
                                                          basic_node_type());
          mp_current_node =
              &(m_context_stack.back().p_node->operator[](std::move(key_node)));
          m_context_stack.emplace_back(
              old_line, old_indent,
              context_state_t::BLOCK_MAPPING_EXPLICIT_VALUE, mp_current_node);
        ...
        case lexical_token_t::TAG_PREFIX:
          deserialize_node_properties(lexer, token, line, indent);
          // Skip updating the current indent to avoid stacking a wrong
          // indentation. Note that node properties for block sequences as a
          // mapping value are processed when a `lexical_token_t::KEY_SEPARATOR`
          // token is processed.
          //
          // ```yaml
          // &foo bar: baz
        ...
        case lexical_token_t::SEQUENCE_BLOCK_PREFIX: {
          ...
          if (indent == parent_indent) {
            // If the previous block sequence entry is empty, just move to the
            // parent context.
            // ```yaml
            // foo:
            //   -
            ...
            if FK_YAML_UNLIKELY (m_context_stack.back().state ==
              ...
              // # ^
              // ```
              throw parse_error("bad indentation of a mapping entry.", line,
                                indent);
            }
        ...
        case lexical_token_t::SEQUENCE_FLOW_BEGIN:
          if (m_flow_context_depth == 0) {
            ...
            if (indent <= m_context_stack.back().indent) {
              pop_to_parent_node(line, indent,
            ...
            throw parse_error(
                "Flow sequence beginning is found without separated with a "
                "comma.",
                line, indent);
          }
          ...
          switch (m_context_stack.back().state) {
            ...
            case context_state_t::FLOW_SEQUENCE:
              mp_current_node->as_seq().emplace_back(
                  basic_node_type::sequence());
              mp_current_node = &(mp_current_node->as_seq().back());
              m_context_stack.emplace_back(line, indent,
                                           context_state_t::FLOW_SEQUENCE,
                                           mp_current_node);
              break;
            case context_state_t::BLOCK_MAPPING:
            case context_state_t::FLOW_MAPPING:
              ...
              m_context_stack.emplace_back(
                  line, indent, context_state_t::FLOW_SEQUENCE_KEY,
        ...
        case lexical_token_t::SEQUENCE_FLOW_END: {
          if FK_YAML_UNLIKELY (m_flow_context_depth == 0) {
            throw parse_error(
                "Flow sequence ending is found outside the flow context.", line,
                indent);
          }
          ...
          auto itr = std::find_if(  // LCOV_EXCL_LINE
              m_context_stack.rbegin(), m_context_stack.rend(),
          ...
          if FK_YAML_UNLIKELY (!is_valid) {
            throw parse_error(
                "No corresponding flow sequence beginning is found.", line,
                indent);
          }
          ...
          if (!m_context_stack.empty() &&
              state == context_state_t::FLOW_SEQUENCE_KEY) {
        ...
        case lexical_token_t::MAPPING_FLOW_BEGIN:
                                   ...
                                   switch (c.state) {
                                     ...
                                     default:
                                      ...
                                      flow_token_state_t::
                                          NEEDS_SEPARATOR_OR_SUFFIX) {
            throw parse_error(
                "Flow mapping beginning is found without separated with a "
                "comma.",
                line, indent);
          }
          ...
          switch (m_context_stack.back().state) {
            ...
            case context_state_t::FLOW_SEQUENCE:
              ...
              mp_current_node = &(mp_current_node->as_seq().back());
              m_context_stack.emplace_back(
                  line, indent, context_state_t::FLOW_MAPPING, mp_current_node);
              break;
            case context_state_t::BLOCK_MAPPING:
            case context_state_t::FLOW_MAPPING:
              // heap-allocated node will be freed in handling the corresponding
              ...
              m_context_stack.emplace_back(
                  line, indent, context_state_t::FLOW_MAPPING_KEY,
        ...
        case lexical_token_t::MAPPING_FLOW_END: {
          if FK_YAML_UNLIKELY (m_flow_context_depth == 0) {
            throw parse_error(
                "Flow mapping ending is found outside the flow context.", line,
                indent);
          }
          ...
          if FK_YAML_UNLIKELY (!is_valid) {
            throw parse_error(
                "No corresponding flow mapping beginning is found.", line,
                indent);
          }
          ...
          if (!m_context_stack.empty() &&
              state == context_state_t::FLOW_MAPPING_KEY) {
            basic_node_type key_node = std::move(*mp_current_node);
            delete mp_current_node;
            mp_current_node = m_context_stack.back().p_node;
            m_flow_token_state = flow_token_state_t::NEEDS_VALUE_OR_SUFFIX;
        ...
        case lexical_token_t::VALUE_SEPARATOR:
          ...
          if FK_YAML_UNLIKELY (m_flow_token_state !=
                               flow_token_state_t::NEEDS_SEPARATOR_OR_SUFFIX) {
            throw parse_error("invalid value separator is found.", line,
                              indent);
          }
          m_flow_token_state = flow_token_state_t::NEEDS_VALUE_OR_SUFFIX;
          break;
        case lexical_token_t::ALIAS_PREFIX: {
          ...
          if FK_YAML_UNLIKELY (m_needs_tag_impl) {
            throw parse_error("Tag cannot be specified to an alias node", line,
                              indent);
          }
          if FK_YAML_UNLIKELY (m_needs_anchor_impl) {
            throw parse_error("Anchor cannot be specified to an alias node.",
                              line, indent);
          }

          std::string token_str =
              std::string(token.str.begin(), token.str.end());
          ...
          detail::node_attr_bits::set_anchor_offset(anchor_counts - 1,
                                                    node.m_attrs);
        ...
        case lexical_token_t::DOUBLE_QUOTED_SCALAR: {
          ...
          basic_node_type node =
              scalar_parser_type(line, indent)
                  .parse_flow(token.type, tag_type, token.str);
          apply_directive_set(node);
          apply_node_properties(node);
        ...
        case lexical_token_t::END_OF_DOCUMENT:
          if FK_YAML_UNLIKELY (m_flow_context_depth > 0) {
            throw parse_error(
                "An invalid document marker found in a flow collection", line,
                indent);
          }
          last_type = token.type;
          return;
  ...
  /// @brief Deserializes YAML node properties (anchor and/or tag names) if they
  /// exist
  /// @param lexer The lexical analyzer to be used.
  /// @param last_type The variable to store the last lexical token type.
  /// @param line The variable to store the line of either the first property or
  /// the last non-property token.
  /// @param indent The variable to store the indent of either the first
  /// property or the last non-property token.
  /// @return true if any property is found, false otherwise.
  bool deserialize_node_properties(lexer_type& lexer, lexical_token& last_token,
    ...
    do {
      ...
      switch (token.type) {
        case lexical_token_t::ANCHOR_PREFIX:
          if FK_YAML_UNLIKELY (m_needs_anchor_impl) {
            throw parse_error(
                "anchor name cannot be specified more than once to the same "
                "node.",
                lexer.get_lines_processed(), lexer.get_last_token_begin_pos());
          }
  ...
  /// @param indent The indentation width in the current line where the key is
  /// found.
  void add_new_key(basic_node_type&& key, const uint32_t line,
    ...
    if (m_flow_context_depth == 0) {
      ...
      if FK_YAML_UNLIKELY (m_flow_token_state !=
                           flow_token_state_t::NEEDS_VALUE_OR_SUFFIX) {
        throw parse_error(
            "Flow mapping entry is found without separated with a comma.", line,
            indent);
      }
      ...
      if (mp_current_node->is_sequence()) {
        ...
        mp_current_node =
            &(mp_current_node->operator[](mp_current_node->size() - 1));
        m_context_stack.emplace_back(
            line, indent, context_state_t::BLOCK_MAPPING, mp_current_node);
      }
    }
    ...
    auto itr =
        mp_current_node->as_map().emplace(std::move(key), basic_node_type());
  ...
  /// @param node_value A rvalue basic_node_type object to be assigned to the
  /// current node.
  void assign_node_value(basic_node_type&& node_value, const uint32_t line,
                         const uint32_t indent) {
    if (mp_current_node->is_sequence()) {
      FK_YAML_ASSERT(m_flow_context_depth > 0);
      ...
      if FK_YAML_UNLIKELY (m_flow_token_state !=
        ...
        // ```
        throw parse_error(
            "flow sequence entry is found without separated with a comma.",
            line, indent);
      }
  ...
  /// @param indent The current indentation width. Can be updated in this
  /// function.
  /// @param line The number of processed lines. Can be updated in this
  /// function.
  /// @param token The storage for last lexical token.
  /// @return true if next token has already been got, false otherwise.
  void deserialize_scalar(lexer_type& lexer, basic_node_type&& node,
                          uint32_t& indent, uint32_t& line,
                          lexical_token& token) {
    token = lexer.get_next_token();
    if (mp_current_node->is_mapping()) {
      ...
      if FK_YAML_UNLIKELY (!is_key_sep_followed) {
        throw parse_error(
            "The \":\" mapping value indicator must be followed after a "
            "mapping key.",
            lexer.get_lines_processed(), lexer.get_last_token_begin_pos());
      }
        ...
        if FK_YAML_LIKELY (!m_context_stack.empty()) {
          ...
          switch (cur_context.state) {
            ...
            case context_state_t::BLOCK_SEQUENCE_ENTRY:
              if FK_YAML_UNLIKELY (cur_context.indent >= indent) {
                // This handles combination of empty block sequence entry and
                // block mapping entry with the same indentation level, for
                // examples:
                // ```yaml
                // foo:
                //   bar:
          ...
          *mp_current_node = basic_node_type::mapping();
          ...
          if (!m_root_anchor_name.empty()) {
            mp_current_node->add_anchor_name(std::string(
                m_root_anchor_name.begin(), m_root_anchor_name.end()));
            m_root_anchor_name = {};
          }
          if (!m_root_tag_name.empty()) {
            mp_current_node->add_tag_name(
                std::string(m_root_tag_name.begin(), m_root_tag_name.end()));
            m_root_tag_name = {};
          }
        }
      }
  ...
  void pop_to_parent_node(uint32_t line, uint32_t indent, Pred&& pred) {
    ...
    const auto pop_num =
        static_cast<uint32_t>(std::distance(m_context_stack.rbegin(), itr));
  ...
  /// @brief Set YAML node properties (anchor and/or tag names) to the given
  /// node.
  /// @param node A node type object to be set YAML node properties.
  void apply_node_properties(basic_node_type& node) {
    if (m_needs_anchor_impl) {
      node.add_anchor_name(
          std::string(m_anchor_name.begin(), m_anchor_name.end()));
      m_needs_anchor_impl = false;
      m_anchor_name = {};
    }
 ...
 private:
  ...
  /// A flag to determine the need for a corresponding node with the last YAML
  /// tag.
  bool m_needs_tag_impl{false};
  /// A flag to determine the need for a value separator or a flow suffix to
  /// follow.
  flow_token_state_t m_flow_token_state{
      flow_token_state_t::NEEDS_VALUE_OR_SUFFIX};
...
/// @note Since fkYAML doesn't treat UTF-16/UTF-32 encoded characters per byte,
/// endians do not matter.
...
/// @note This function doesn't support the case where the first character is
/// null.
...
inline utf_encode_t detect_encoding_type(const std::array<uint8_t, 4>& bytes,
  ...
  if (byte0 == static_cast<uint8_t>(0xEFu) &&
      byte1 == static_cast<uint8_t>(0xBBu) &&
  ...
  if (byte0 == static_cast<uint8_t>(0xFFu) &&
      byte1 == static_cast<uint8_t>(0xFEu) && byte2 == 0 && byte3 == 0) {
    has_bom = true;
    return utf_encode_t::UTF_32LE;
  }

  if (byte0 == static_cast<uint8_t>(0xFEu) &&
      byte1 == static_cast<uint8_t>(0xFFu)) {
    has_bom = true;
    return utf_encode_t::UTF_16BE;
  }

  if (byte0 == static_cast<uint8_t>(0xFFu) &&
      byte1 == static_cast<uint8_t>(0xFEu)) {
    has_bom = true;
  ...
  if (byte0 == 0 && byte1 == 0 && byte2 == 0 && 0 < byte3 &&
      byte3 < static_cast<uint8_t>(0x80u)) {
    return utf_encode_t::UTF_32BE;
  }
...
/// @brief A class which detects UTF encoding type and the existence of a BOM at
/// the beginning.
...
struct utf_encode_detector<ItrType,
                           enable_if_t<is_iterator_of<ItrType, char>::value>> {
  /// @brief Detects the encoding type of the input, and consumes a BOM if it
  /// exists.
  ...
  static utf_encode_t detect(ItrType& begin, const ItrType& end) noexcept {
    ...
    for (int i = 0; i < 4 && current != end; i++, ++current) {
      bytes[i] = static_cast<uint8_t>(
          *current);  // NOLINT(cppcoreguidelines-pro-bounds-constant-array-index)
    }
...
/// @brief The partial specialization of utf_encode_detector for char8_t
/// iterators.
/// @tparam ItrType An iterator type.
template <typename ItrType>
struct utf_encode_detector<
    ItrType, enable_if_t<is_iterator_of<ItrType, char8_t>::value>> {
  /// @brief Detects the encoding type of the input, and consumes a BOM if it
  /// exists.
  /// @param begin The iterator to the first element of an input.
  /// @param end The iterator to the past-the end element of an input.
...
/// @brief The partial specialization of utf_encode_detector for char16_t
/// iterators.
/// @tparam ItrType An iterator type.
template <typename ItrType>
struct utf_encode_detector<
    ItrType, enable_if_t<is_iterator_of<ItrType, char16_t>::value>> {
  /// @brief Detects the encoding type of the input, and consumes a BOM if it
  /// exists.
  /// @param begin The iterator to the first element of an input.
  /// @param end The iterator to the past-the end element of an input.
  ...
  static utf_encode_t detect(ItrType& begin, const ItrType& end) {
    ...
    if FK_YAML_UNLIKELY (encode_type != utf_encode_t::UTF_16BE &&
      ...
      throw exception(
          "char16_t characters must be encoded in the UTF-16 format.");
    }
...
/// @brief The partial specialization of utf_encode_detector for char32_t
/// iterators.
/// @tparam ItrType An iterator type.
template <typename ItrType>
struct utf_encode_detector<
    ItrType, enable_if_t<is_iterator_of<ItrType, char32_t>::value>> {
  /// @brief Detects the encoding type of the input, and consumes a BOM if it
  /// exists.
  /// @param begin The iterator to the first element of an input.
  /// @param end The iterator to the past-the end element of an input.
  ...
  static utf_encode_t detect(ItrType& begin, const ItrType& end) {
    ...
    if FK_YAML_UNLIKELY (encode_type != utf_encode_t::UTF_32BE &&
      ...
      throw exception(
          "char32_t characters must be encoded in the UTF-32 format.");
    }
...
/// @brief A class which detects UTF encoding type and the existence of a BOM
/// from the input file.
struct file_utf_encode_detector {
  ...
  static utf_encode_t detect(std::FILE* p_file) noexcept {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      bytes[i] = static_cast<uint8_t>(
          byte &
          0xFF);  // NOLINT(cppcoreguidelines-pro-bounds-constant-array-index)
    }
...
class iterator_input_adapter<
    IterType, enable_if_t<is_iterator_of<IterType, char>::value>> {
 public:
 ...
 private:
  /// @brief The concrete implementation of get_buffer_view() for UTF-8 encoded
  /// inputs.
    ...
    while (current != m_end) {
      ...
      switch (num_bytes) {
        ...
        case 2: {
          ...
          if FK_YAML_UNLIKELY (!is_valid) {
            throw fkyaml::invalid_encoding("Invalid UTF-8 encoding.",
                                           {first, second});
          }
          break;
        }
  ...
  /// @brief The concrete implementation of get_buffer_view() for UTF-16 encoded
  /// inputs.
  /// @return View into the UTF-8 encoded input buffer contents.
  str_view get_buffer_view_utf16() {
    ...
    while (current != m_end || encoded_buf_size != 0) {
      while (current != m_end && encoded_buf_size < 2) {
        auto utf16 = static_cast<char16_t>(static_cast<uint8_t>(*current)
                                           << shift_bits[0]);
        utf16 |= static_cast<char16_t>(static_cast<uint8_t>(*++current)
                                       << shift_bits[1]);
        ++current;
      ...
      utf8::from_utf16(encoded_buffer, utf8_buffer, consumed_size,
                       utf8_buf_size);
      ...
      m_buffer.append(reinterpret_cast<const char*>(utf8_buffer.data()),
                      utf8_buf_size);
    }
  ...
  /// @brief The concrete implementation of get_buffer_view() for UTF-32 encoded
  /// inputs.
  /// @return View into the UTF-8 encoded input buffer contents.
...
template <typename IterType>
class iterator_input_adapter<
    IterType, enable_if_t<is_iterator_of<IterType, char8_t>::value>> {
 public:
  /// @brief Construct a new iterator_input_adapter object.
  iterator_input_adapter() = default;
  ...
  str_view get_buffer_view() {
    ...
    current = m_begin;
    for (const auto& cr_itr : cr_itrs) {
      std::transform(current, cr_itr, std::back_inserter(m_buffer),
                     [](char8_t c) { return static_cast<char>(c); });
      current = std::next(cr_itr);
    }
    std::transform(current, m_end, std::back_inserter(m_buffer),
                   [](char8_t c) { return static_cast<char>(c); });
...
template <typename IterType>
class iterator_input_adapter<
    IterType, enable_if_t<is_iterator_of<IterType, char16_t>::value>> {
 public:
  /// @brief Construct a new iterator_input_adapter object.
  iterator_input_adapter() = default;
...
template <typename IterType>
class iterator_input_adapter<
    IterType, enable_if_t<is_iterator_of<IterType, char32_t>::value>> {
 public:
  /// @brief Construct a new iterator_input_adapter object.
  iterator_input_adapter() = default;
  ...
  str_view get_buffer_view() {
    ...
    while (current != m_end) {
      ...
      ++current;
      const auto utf32 =
          static_cast<char32_t>(((tmp & 0xFF000000u) >> shift_bits[0]) |
                                ((tmp & 0x00FF0000u) >> shift_bits[1]) |
                                ((tmp & 0x0000FF00u) << shift_bits[2]) |
...
class file_input_adapter {
 public:
  ...
  /// @param file A file handle for this adapter. (A non-null pointer is
  /// assumed.)
  /// @param encode_type The encoding type for this input adapter.
  explicit file_input_adapter(std::FILE* file,
                              utf_encode_t encode_type) noexcept
 ...
 private:
  ...
  str_view get_buffer_view_utf8() {
    ...
    while ((read_size =
                std::fread(&tmp_buf[0], sizeof(char), buf_size, m_file)) > 0) {
  ...
  str_view get_buffer_view_utf16() {
    ...
    while (std::feof(m_file) == 0) {
      while (encoded_buf_size < 2 &&
             std::fread(&chars[0], sizeof(char), 2, m_file) == 2) {
...
/// @brief A concrete factory method for iterator_input_adapter objects with
/// iterators.
/// @tparam ItrType An iterator type.
/// @param begin The beginning of iterators.
/// @param end The end of iterators.
...
inline iterator_input_adapter<ItrType> create_iterator_input_adapter(
    ItrType begin, ItrType end, bool is_contiguous) {
  const utf_encode_t encode_type =
      utf_encode_detector<ItrType>::detect(begin, end);
  return iterator_input_adapter<ItrType>(begin, end, encode_type,
                                         is_contiguous);
}

/// @brief A factory method for iterator_input_adapter objects with iterator
/// values.
/// @tparam ItrType An iterator type.
/// @param begin The beginning of iterators.
/// @param end The end of iterators.
/// @return iterator_input_adapter<ItrType> An iterator_input_adapter object for
/// the target iterator type.
template <typename ItrType>
inline iterator_input_adapter<ItrType> input_adapter(ItrType begin,
                                                     ItrType end) {
  bool is_contiguous = true;
  const auto size = std::distance(begin, end);

  // Check if `begin` & `end` are contiguous iterators.
  // Getting distance between begin and (end - 1) avoids dereferencing an
  // invalid sentinel.
  if FK_YAML_LIKELY (size > 0) {
    using char_ptr_t =
        remove_cvref_t<typename std::iterator_traits<ItrType>::pointer>;
...
/// @brief A factory method for iterator_input_adapter objects with C-style
/// arrays.
/// @tparam T A type of arrayed objects.
/// @tparam N A size of an array.
/// @return decltype(input_adapter(array, array + N)) An iterator_input_adapter
/// object for the target array.
...
/// @brief A namespace to implement container_input_adapter_factory for internal
/// use.
...
/// @brief A partial specialization of container_input_adapter_factory if
/// begin()/end() are available for ContainerType.
/// @tparam ContainerType A container type.
...
struct container_input_adapter_factory<
  ...
  /// @brief A factory method of input adapter objects for the target container
  /// objects.
...
/// @return
/// input_adapter_factory::container_input_adapter_factory<ContainerType>::adapter_type
template <typename ContainerType>
inline typename input_adapter_factory::container_input_adapter_factory<
    ContainerType>::adapter_type
input_adapter(const ContainerType& container) {
  return input_adapter_factory::container_input_adapter_factory<
      ContainerType>::create(container);
}

/// @brief A factory method for file_input_adapter objects with C-style file
/// handles.
...
/// @brief A factory method for stream_input_adapter objects with std::istream
/// objects.
...
/// @brief The template definitions of type information used in @ref Iterator
/// class
...
struct iterator_holder {
  static_assert(is_basic_node<BasicNodeType>::value,
                "iterator_holder class only accepts a basic_node as its "
                "template parameter.");
...
class iterator {
 ...
 public:
  ...
  /// @brief Copy constructs an iterator from another iterator with different
  /// const-ness in ValueType.
  /// @note This copy constructor is not defined if ValueType is not const to
  /// avoid const removal from ValueType.
  /// @tparam OtherIterator The iterator type to copy from.
  /// @param other An iterator to copy from with different const-ness in
  /// ValueType.
  template <
  ...
  /// @return pointer A pointer to the BasicNodeType object internally
  /// referenced by the actual iterator object.
  ...
  /// @return reference Reference to the Node object internally referenced by
  /// the actual iterator object.
  ...
  /// @param i The difference from this Iterator object with which it moves
  /// forward.
  ...
  /// backward.
  /// @return iterator& Reference to this iterator object.
  iterator& operator-=(difference_type i) noexcept { return operator+=(-i); }
  ...
  template <typename Iterator,
            enable_if_t<
                disjunction<std::is_same<Iterator, iterator>,
  ...
  bool operator==(const Iterator& rhs) const {
    if FK_YAML_UNLIKELY (m_inner_iterator_type != rhs.m_inner_iterator_type) {
      throw fkyaml::exception(
          "Cannot compare iterators of different container types.");
    }
    ...
    // m_inner_iterator_type == iterator_t::MAPPING
    return (m_iterator_holder.mapping_iterator ==
            rhs.m_iterator_holder.mapping_iterator);
  }
  ...
  ///  @param rhs An iterator object to be compared with this iterator object.
  ///  @return true  This iterator object is either less than or equal to the
  ///  other.
  ///  @return false This iterator object is neither less than nor equal to the
  ///  other.
  template <typename Iterator,
            enable_if_t<
                disjunction<std::is_same<Iterator, iterator>,
  ...
  /// @param rhs An iterator object to be compared with this iterator object.
  /// @return true  This iterator object is either greater than or equal to the
  /// other.
  /// @return false This iterator object is neither greater than nor equal to
  /// the other.
  template <typename Iterator,
            enable_if_t<
                disjunction<std::is_same<Iterator, iterator>,
 ...
 public:
  ...
  iterator_t type() const noexcept { return m_inner_iterator_type; }
...
struct tuple_size<::fkyaml::detail::iterator<ValueType>>
...
struct tuple_element<I, ::fkyaml::detail::iterator<ValueType>> {
  using type =
      decltype(get<I>(std::declval<::fkyaml::detail::iterator<ValueType>>()));
};
...
class map_iterator_proxy {
 public:
  ...
  /// @brief The type to represent difference between the pointed-to elements by
  /// base iterators.
  using difference_type = std::ptrdiff_t;
  ...
  map_iterator_proxy(const Iterator& i) noexcept : m_base_iterator(i) {}
  ...
  reference operator*() noexcept { return m_base_iterator; }

  /// @brief Get pointer to the base iterator object.
  /// @return Pointer to the base iterator object.
  pointer operator->() noexcept { return &m_base_iterator; }
  ...
  /// @return true if this map_iterator_proxy object is equal to `rhs`, false
  /// otherwise.
  ...
  /// @param rhs A map_iterator_proxy object to compare with.
  /// @return true if this map_iterator_proxy object is not equal to `rhs`,
  /// false otherwise.
  bool operator!=(const map_iterator_proxy& rhs) const noexcept {
    return m_base_iterator != rhs.m_base_iterator;
  }
  ...
  typename Iterator::reference key() const { return m_base_iterator.key(); }
...
/// @brief A helper struct which allows accessing node iterator member functions
/// in range-based for loops.
/// @tparam BasicNodeType A basic_node template instance type.
template <typename BasicNodeType>
class map_range_proxy {
  static_assert(is_basic_node<BasicNodeType>::value,
                "map_range_proxy only accepts a basic_node type as its "
                "template parameter.");

 public:
  /// @brief The type of non-const iterators.
  using iterator = map_iterator_proxy<
      typename std::conditional<std::is_const<BasicNodeType>::value,
                                typename BasicNodeType::const_iterator,
                                typename BasicNodeType::iterator>::type>;

  /// @brief The type of const iterators.
  using const_iterator =
      map_iterator_proxy<typename BasicNodeType::const_iterator>;

  /// @brief Constructs a map_range_proxy object with a BasicNodeType object.
  /// @param map A mapping node object.
  map_range_proxy(BasicNodeType& map) noexcept : mp_map(&map) {}
...
/// @note This class makes it easier to handle lvalue basic_node objects in
/// basic_node ctor with std::initializer_list.
/// @tparam BasicNodeType A basic_node template instance type.
template <typename BasicNodeType>
class node_ref_storage {
  static_assert(is_basic_node<BasicNodeType>::value,
 ...
 public:
  /// @brief Construct a new node ref storage object with an rvalue basic_node
  /// object.
  ...
  /// @brief Construct a new node ref storage object with an lvalue basic_node
  /// object.
  /// @param n An lvalue basic_node object.
  explicit node_ref_storage(const node_type& n) noexcept : m_value_ref(&n) {}

  /// @brief Construct a new node ref storage object with a
  ...
  /// @brief Construct a new node ref storage object with variadic template
  /// arguments
...
inline enable_if_t<is_non_bool_integral<IntegerType>::value> to_string(
    IntegerType v, std::string& s) noexcept {
  s = std::to_string(v);
}
...
template <typename FloatType>
inline enable_if_t<std::is_floating_point<FloatType>::value> to_string(
    FloatType v, std::string& s) noexcept {
  if (std::isnan(v)) {
    s = ".nan";
    return;
  }
...
class basic_serializer {
 ...
 private:
    ...
    switch (node.get_type()) {
      ...
      case node_type::MAPPING:
        ...
        for (auto itr : node.map_items()) {
          ...
          if (is_appended) {
            // The trailing white space is necessary since anchor names can
            // contain a colon (:) at its end.
            str += " ";
          } else {
            const bool is_anchor_appended =
                try_append_anchor(key_node, false, str);
            const bool is_tag_appended =
                try_append_tag(key_node, is_anchor_appended, str);
            ...
            if (is_container) {
              // a newline code is already inserted in the above
              // serialize_node() call.
      ...
      case node_type::BOOLEAN:
        to_string(
            node.template get_value<typename BasicNodeType::boolean_type>(),
            m_tmp_str_buff);
        str += m_tmp_str_buff;
        break;
      case node_type::INTEGER:
        to_string(
            node.template get_value<typename BasicNodeType::integer_type>(),
            m_tmp_str_buff);
        str += m_tmp_str_buff;
        break;
      case node_type::FLOAT:
        to_string(node.template get_value<
                      typename BasicNodeType::float_number_type>(),
                  m_tmp_str_buff);
        str += m_tmp_str_buff;
        break;
      case node_type::STRING: {
        ...
        // The next line is intentionally excluded from the LCOV coverage target
        // since the next line is somehow misrecognized as it has a binary
        // branch. Possibly begin() or end() has some conditional branch(es)
        // internally. Confirmed with LCOV 1.14 on Ubuntu22.04.
        const node_type type_if_plain = scalar_scanner::scan(
            str_val.c_str(),
            str_val.c_str() + str_val.size());  // LCOV_EXCL_LINE

        if (type_if_plain != node_type::STRING) {
          // Surround a string value with double quotes to keep semantic
          // equality. Without them, serialized values will become non-string.
          // (e.g., "1" -> 1)
          str += '\"';
          str += str_val;
          str += '\"';
  ...
  std::size_t get_cur_indent(const std::string& s) const noexcept {
    ...
    return (last_lf_pos != std::string::npos) ? s.size() - last_lf_pos - 1
                                              : s.size();
  }
  ...
  /// @param prepends_space Whether to prepend a space before an anchor
  /// property.
  /// @param str A string to hold serialization result.
  ...
  /// @brief Get a string value from the given node and, if necessary, escape
  /// its contents.
  /// @param[in] node The target string YAML node.
  /// @param[out] is_escaped Whether the contents of an output string has been
  /// escaped.
  /// @return The (escaped) string node value.
  typename BasicNodeType::string_type get_string_node_value(
      const BasicNodeType& node, bool& is_escaped) {
...
/// @brief An iterator adapter class that reverses the direction of a given node
/// iterator.
/// @tparam Iterator The base iterator type.
template <typename Iterator>
class reverse_iterator {
  static_assert(is_basic_node<typename Iterator::value_type>::value,
                "reverse_iterator only accepts a basic_node type as the "
                "underlying iterator's value type");
 ...
 public:
  ...
  /// @brief The type to represent differences between the pointed-to elements
  /// by the base iterators.
  ...
  /// @brief Constructs a reverse_iterator object with an underlying iterator
  /// object.
  /// @param i A base iterator object.
  reverse_iterator(const Iterator& i) noexcept : m_current(i) {}

  /// @brief Copy constructs a reverse_iterator object with a compatible
  /// reverse_iterator object.
  /// @tparam U A compatible iterator type with Iterator.
  /// @param other A compatible reverse_iterator object.
  template <typename U,
            enable_if_t<negation<std::is_same<U, Iterator>>::value, int> = 0>
  reverse_iterator(const reverse_iterator<U>& other) noexcept
      : m_current(other.base()) {}

  /// @brief Copy assigns a reverse_iterator object with a compatible
  /// reverse_iterator object.
  /// @tparam U A compatible iterator type with Iterator.
  /// @param other A compatible reverse_iterator object.
  ...
  Iterator base() const noexcept { return m_current; }
  ...
  /// @return Reference to this reverse_iterator object with its underlying
  ...
  /// @return Reference to this reverse_iterator with its underlying iterator
  /// decremented.
  reverse_iterator& operator--() noexcept {
    ++m_current;
    return *this;
  ...
  /// @return A reverse_iterator object with the underlying iterator advanced by
  /// `n`.
  ...
  /// @brief Advances the underlying iterator object by `n`.
  /// @param n The distance by which the underlying iterator is advanced.
  /// @return Reference to this reverse_iterator object with the underlying
  ...
  /// @return A reverse_iterator object with the underlying iterator decremented
  /// by `n`.
  reverse_iterator operator-(difference_type n) const noexcept {
    return reverse_iterator(m_current + n);
  }
...
/// @param lhs A reverse_iterator object.
/// @param rhs A reverse_iterator object.
/// @return true if the two reverse_iterator objects are not equal, false
/// otherwise.
template <typename IteratorL, typename IteratorR>
inline bool operator!=(const reverse_iterator<IteratorL>& lhs,
                       const reverse_iterator<IteratorR>& rhs) {
...
inline yaml_version_t convert_from_yaml_version_type(
    yaml_version_type t) noexcept {
  switch (t) {
    case yaml_version_type::VERSION_1_1:
...
inline yaml_version_type convert_to_yaml_version_type(
    yaml_version_t t) noexcept {
  switch (t) {
...
/// @brief Utility traits type alias to detect constructible associative
/// container types from a mapping node, e.g., std::map or std::unordered_map.
/// @tparam T A target type for detection.
template <typename T>
using is_constructible_mapping_type =
    conjunction<detect::has_key_type<T>, detect::has_mapped_type<T>,
                detect::has_value_type<T>>;

/// @brief Utility traits type alias to detect constructible container types
/// from a sequence node, e.g., std::vector or std::list.
/// @tparam BasicNodeType A basic_node template instance type.
/// @tparam T A target type for detection.
template <typename BasicNodeType, typename T>
...
/// @brief Utility traits type alias to detect a sequence container adapter
/// type, e.g., std::stack or std::queue.
/// @tparam T A target type for detection.
template <typename T>
...
/// @brief Helper struct for reserve() member function call switch for types
/// which do not have reserve function.
/// @tparam ContainerType A container type.
template <typename ContainerType, typename = void>
struct call_reserve_if_available {
  /// @brief Do nothing since ContainerType does not have reserve function.
  static void call(ContainerType& /*unused*/,
                   typename ContainerType::size_type /*unused*/) {}
};

/// @brief Helper struct for reserve() member function call switch for types
/// which have reserve function.
/// @tparam ContainerType A container type.
template <typename ContainerType>
struct call_reserve_if_available<
    ContainerType, enable_if_t<detect::has_reserve<ContainerType>::value>> {
  /// @brief Call reserve function on the ContainerType object with a given
  /// size.
...
/// @brief from_node function for C-style 1D arrays whose element type must be a
...
inline auto from_node(const BasicNodeType& n, T (&array)[N])
  ...
  if FK_YAML_UNLIKELY (!n.is_sequence()) {
    throw type_error("The target node value type is not sequence type.",
                     n.get_type());
  }

  // call get_value_inplace(), not get_value(), since the storage to fill the
  // result into is already created.
  for (std::size_t i = 0; i < N; i++) {
    n.at(i).get_value_inplace(array[i]);
  }
}

/// @brief from_node function for C-style 2D arrays whose element type must be a
/// basic_node template instance type or a compatible type.
/// @tparam BasicNodeType A basic_node template instance type.
...
/// @brief from_node function for std::array objects whose element type must be
/// a basic_node template instance type or a compatible type. This function is
/// necessary since insert function is not implemented for std::array.
/// @tparam BasicNodeType A basic_node template instance type.
...
/// @brief from_node function for std::valarray objects whose element type must
/// be a basic_node template instance type or a compatible type. This function
/// is necessary since insert function is not implemented for std::valarray.
/// @tparam BasicNodeType A basic_node template instance type.
...
/// @brief from_node function for std::forward_list objects whose element type
/// must be a basic_node template instance type or a compatible type. This
/// function is necessary since insert function is not implemented for
/// std::forward_list.
/// @tparam BasicNodeType A basic_node template instance type.
...
inline auto from_node(const BasicNodeType& n, std::forward_list<T, Alloc>& fl)
  ...
  if FK_YAML_UNLIKELY (!n.is_sequence()) {
    throw type_error("The target node value is not sequence type.",
                     n.get_type());
  }
  ...
  for (const auto& elem : n) {
    insert_pos_itr =
        fl.emplace_after(insert_pos_itr, elem.template get_value<T>());
  }
}

/// @brief from_node function for container objects of only keys or values,
/// e.g., std::vector or std::set, whose element type must be a basic_node
...
template <
    ...
    enable_if_t<
        conjunction<
            ...
            negation<std::is_constructible<typename BasicNodeType::string_type,
                                           CompatSeqType>>>::value,
        int> = 0>
inline auto from_node(const BasicNodeType& n, CompatSeqType& s)
    -> decltype(n.template get_value<typename CompatSeqType::value_type>(),
                void()) {
  if FK_YAML_UNLIKELY (!n.is_sequence()) {
    throw type_error("The target node value is not sequence type.",
                     n.get_type());
  ...
  // transform a sequence node into a destination type object by calling insert
  // function.
  using std::end;
  std::transform(
      n.begin(), n.end(), std::inserter(s, end(s)),
      [](const BasicNodeType& elem) {
        return elem.template get_value<typename CompatSeqType::value_type>();
      });
}

/// @brief from_node function for sequence container adapter objects, e.g.,
/// std::stack or std::queue, whose element type must be either a basic_node
/// template instance type or a compatible type.
/// @tparam BasicNodeType A basic_node template instance type.
...
inline auto from_node(const BasicNodeType& n, SeqContainerAdapter& ca)
    -> decltype(n.template get_value<
                    typename SeqContainerAdapter::value_type>(),
                ca.push(
                    std::declval<typename SeqContainerAdapter::value_type>()),
                void()) {
  if FK_YAML_UNLIKELY (!n.is_sequence()) {
    throw type_error("The target node value is not sequence type.",
                     n.get_type());
  ...
  // clear existing elements manually since clear function is not implemeneted
  // for container adapter classes.
  while (!ca.empty()) {
    ca.pop();
  }
  ...
  for (const auto& elem : n) {
    ...
    // emplace function cannot be used in case
    // SeqContainerAdapter::container_type is std::vector<bool> in C++11.
    ca.push(
        elem.template get_value<typename SeqContainerAdapter::value_type>());
  }
}

/// @brief from_node function for mappings whose key and value are of both
/// compatible types.
/// @tparam BasicNodeType A basic_node template instance type.
/// @tparam CompatibleKeyType Mapping key type compatible with BasicNodeType.
/// @tparam CompatibleValueType Mapping value type compatible with
/// BasicNodeType.
...
inline auto from_node(const BasicNodeType& n, CompatMapType& m)
  ...
  if FK_YAML_UNLIKELY (!n.is_mapping()) {
    throw type_error("The target node value type is not mapping type.",
                     n.get_type());
  }
...
inline void from_node(const BasicNodeType& n, std::nullptr_t& null) {
  ...
  if FK_YAML_UNLIKELY (!n.is_null()) {
    throw type_error("The target node value type is not null type.",
                     n.get_type());
  }
  null = nullptr;
}
...
inline void from_node(const BasicNodeType& n, bool& b) {
  switch (n.get_type()) {
    ...
    default:
      throw type_error(
          "The target node value type is not compatible with boolean type.",
          n.get_type());
  }
}
...
/// @tparam BasicNodeType A basic_node template instance type.
/// @tparam IntType Target integer value type (same as
/// BasicNodeType::integer_type)
template <
    typename BasicNodeType, typename IntType,
...
struct from_node_int_helper {
  ...
  static IntType convert(const BasicNodeType& n) { return n.as_int(); }
};

/// @brief Helper struct for node-to-int conversion if IntType is not the node's
/// integer value type.
/// @tparam BasicNodeType A basic_node template instance type.
/// @tparam IntType Target integer value type (different from
/// BasicNodeType::integer_type)
...
struct from_node_int_helper<BasicNodeType, IntType, false> {
  ...
  static IntType convert(const BasicNodeType& n) {
    ...
    if (std::is_same<IntType, uint64_t>::value) {
      ...
      if FK_YAML_UNLIKELY (tmp_int < static_cast<node_int_type>(
                                         std::numeric_limits<IntType>::min())) {
        throw exception("Integer value underflow detected.");
      }
      if FK_YAML_UNLIKELY (static_cast<node_int_type>(
                               std::numeric_limits<IntType>::max()) < tmp_int) {
...
/// @note If node's value is null, boolean, or float, such a value is converted
/// into an integer internally.
/// @tparam BasicNodeType A basic_node template instance type.
/// @tparam IntegerType An integer value type.
/// @param n A node object.
/// @param i Storage for an integer value.
template <typename BasicNodeType, typename IntegerType,
...
inline void from_node(const BasicNodeType& n, IntegerType& i) {
  switch (n.get_type()) {
    case node_type::NULL_OBJECT:
      ...
      break;
    case node_type::BOOLEAN:
      i = static_cast<bool>(n.as_bool()) ? static_cast<IntegerType>(1)
    ...
    case node_type::INTEGER:
      ...
      break;
    case node_type::FLOAT: {
      // int64_t should be safe to express the integer part of possible floating
      // point types.
      ...
      if (std::is_same<IntegerType, uint64_t>::value) {
        ...
        if FK_YAML_UNLIKELY (static_cast<int64_t>(
    ...
    default:
      throw type_error(
          "The target node value type is not compatible with integer type.",
          n.get_type());
  }
}

/// @brief Helper struct for node-to-float conversion if FloatType is the node's
...
template <typename BasicNodeType, typename FloatType,
          bool = std::is_same<typename BasicNodeType::float_number_type,
                              FloatType>::value>
struct from_node_float_helper {
  /// @brief Convert node's floating point value to the target floating point
  /// type.
  /// @param n A node object.
  /// @return A floating point value converted from the node's floating point
  /// value.
  static FloatType convert(const BasicNodeType& n) { return n.as_float(); }
};

/// @brief Helper struct for node-to-float conversion if IntType is not the
/// node's floating point value type.
/// @tparam BasicNodeType A basic_node template instance type.
...
/// @note If node's value is null, boolean, or integer, such a value is
/// converted into a floating point internally.
/// @tparam BasicNodeType A basic_node template instance type.
...
inline void from_node(const BasicNodeType& n, FloatType& f) {
  switch (n.get_type()) {
    case node_type::NULL_OBJECT:
      ...
      break;
    case node_type::BOOLEAN:
      f = static_cast<bool>(n.as_bool()) ? static_cast<FloatType>(1.)
    ...
    default:
      throw type_error(
          "The target node value type is not compatible with float number "
          "type.",
          n.get_type());
  }
}
...
template <typename BasicNodeType,
          enable_if_t<is_basic_node<BasicNodeType>::value, int> = 0>
inline void from_node(const BasicNodeType& n,
                      typename BasicNodeType::string_type& s) {
  if FK_YAML_UNLIKELY (!n.is_string()) {
    throw type_error("The target node value type is not string type.",
                     n.get_type());
  }
  s = n.as_str();
}
...
template <typename BasicNodeType, typename CompatibleStringType,
          enable_if_t<
              conjunction<
                  is_basic_node<BasicNodeType>,
                  negation<std::is_same<CompatibleStringType,
                                        typename BasicNodeType::string_type>>,
                  disjunction<std::is_constructible<
                                  CompatibleStringType,
                                  const typename BasicNodeType::string_type&>,
                              std::is_assignable<CompatibleStringType,
                                                 const typename BasicNodeType::
                                                     string_type&>>>::value,
              int> = 0>
...
/// @brief from_node function for std::pair objects whose element types must be
/// either a basic_node template instance type or a compatible type.
/// @tparam BasicNodeType A basic_node template instance type.
...
inline auto from_node(const BasicNodeType& n, std::pair<T, U>& p)
    -> decltype(std::declval<const BasicNodeType&>().template get_value<T>(),
                std::declval<const BasicNodeType&>().template get_value<U>(),
                void()) {
  if FK_YAML_UNLIKELY (!n.is_sequence()) {
    throw type_error("The target node value type is not sequence type.",
                     n.get_type());
...
inline std::tuple<Types...> from_node_tuple_impl(
    const BasicNodeType& n, index_sequence<Idx...> /*unused*/) {
  return std::make_tuple(n.at(Idx).template get_value<Types>()...);
}

/// @brief from_node function for std::tuple objects whose value types must all
/// be either a basic_node template instance type or a compatible type
/// @tparam BasicNodeType A basic_node template instance type.
...
inline void from_node(const BasicNodeType& n, std::tuple<Types...>& t) {
  ...
  // Types... must be explicitly specified; the retun type would otherwise be
  // std::tuple with no value types.
  t = from_node_tuple_impl<BasicNodeType, Types...>(
      n, index_sequence_for<Types...>{});
}

#ifdef FK_YAML_HAS_CXX_17

/// @brief from_node function for std::optional objects whose value type must be
/// either a basic_node template instance type or a compatible type.
/// @tparam BasicNodeType A basic_node template instance type.
...
inline auto from_node(const BasicNodeType& n, std::optional<T>& o)
  ...
  try {
    ...
    // Any exception derived from std::exception is interpreted as a conversion
    // failure in some way since user-defined from_node function may throw a
    // different object from a fkyaml::type_error. and std::exception is usually
    // the base class of user-defined exception types.
    o = std::nullopt;
  }
}
...
/// @note User-defined specialization is available by providing implementation
/// **OUTSIDE** fkyaml namespace.
struct from_node_fn {
  ...
  /// @return decltype(from_node(n, std::forward<T>(val))) void by default. User
  /// can set it to some other type.
  template <typename BasicNodeType, typename T>
...
// see http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2015/n4381.html for
// why it's needed.
...
/// @note All the non-specialized instantiations results in compilation error
/// since such instantiations are not supported.
/// @warning All the specialization must call n.m_value.destroy() first in the
/// construct function to avoid memory leak.
...
struct external_node_constructor {
  ...
  static void sequence(BasicNodeType& n, Args&&... args) {
    ...
    n.m_value.p_seq = create_object<typename BasicNodeType::sequence_type>(
  ...
  static void mapping(BasicNodeType& n, Args&&... args) {
    ...
    n.m_value.p_map = create_object<typename BasicNodeType::mapping_type>(
        std::forward<Args>(args)...);
  }
  ...
  static void boolean_scalar(BasicNodeType& n,
                             const typename BasicNodeType::boolean_type b) {
    destroy(n);
    n.m_attrs |= node_attr_bits::bool_bit;
    n.m_value.boolean = b;
  }

  static void integer_scalar(BasicNodeType& n,
                             const typename BasicNodeType::integer_type i) {
    destroy(n);
    n.m_attrs |= node_attr_bits::int_bit;
    n.m_value.integer = i;
  }

  static void float_scalar(BasicNodeType& n,
                           const typename BasicNodeType::float_number_type f) {
    destroy(n);
  ...
  static void string_scalar(BasicNodeType& n, Args&&... args) {
    ...
    n.m_value.p_str = create_object<typename BasicNodeType::string_type>(
        std::forward<Args>(args)...);
  }
...
template <
    ...
    enable_if_t<conjunction<is_basic_node<BasicNodeType>,
                            std::is_same<typename BasicNodeType::sequence_type,
...
/// * BasicNodeType::string_type cannot be constructed from a CompatSeqType
/// object (string-like type)
/// @tparam BasicNodeType A basic_node template instance type.
/// @tparam CompatSeqType A container type.
/// @param n A basic_node object.
/// @param s A container object.
template <
    ...
    enable_if_t<
        conjunction<
            is_basic_node<BasicNodeType>,
            negation<std::is_same<typename BasicNodeType::sequence_type,
                                  remove_cvref_t<CompatSeqType>>>,
            negation<is_basic_node<CompatSeqType>>,
            detect::has_begin_end<CompatSeqType>,
            negation<conjunction<detect::has_key_type<CompatSeqType>,
                                 detect::has_mapped_type<CompatSeqType>>>,
            negation<std::is_constructible<typename BasicNodeType::string_type,
                                           CompatSeqType>>>::value,
        int> = 0>
// NOLINTNEXTLINE(cppcoreguidelines-missing-std-forward)
...
template <typename BasicNodeType, typename... Types, std::size_t... Idx>
inline void to_node_tuple_impl(BasicNodeType& n, const std::tuple<Types...>& t,
                               index_sequence<Idx...> /*unused*/) {
  n = {std::get<Idx>(t)...};
}

/// @brief to_node function for std::tuple objects with no value types.
/// @note This implementation is needed since calling `to_node_tuple_impl()`
/// with an empty tuple creates a null node.
/// @tparam BasicNodeType A basic_node template instance type.
/// @param n A basic_node object.
...
inline void to_node(BasicNodeType& n,
                    const std::tuple<FirstType, RestTypes...>& t) {
...
template <
    typename BasicNodeType, typename T,
    enable_if_t<conjunction<is_basic_node<BasicNodeType>,
                            std::is_same<typename BasicNodeType::mapping_type,
                                         remove_cvref_t<T>>>::value,
                int> = 0>
...
template <
    ...
    enable_if_t<
        conjunction<is_basic_node<BasicNodeType>,
                    ...
                    negation<std::is_same<typename BasicNodeType::mapping_type,
                                          remove_cvref_t<CompatMapType>>>,
...
template <typename BasicNodeType,
          enable_if_t<is_basic_node<BasicNodeType>::value, int> = 0>
inline void to_node(BasicNodeType& n,
                    typename BasicNodeType::boolean_type b) noexcept {
...
template <
    ...
    enable_if_t<
        conjunction<is_basic_node<BasicNodeType>, negation<is_null_pointer<T>>,
                    std::is_constructible<typename BasicNodeType::string_type,
                                          T>>::value,
        int> = 0>
inline void to_node(BasicNodeType& n, T&& s) {
  external_node_constructor<BasicNodeType>::string_scalar(n,
                                                          std::forward<T>(s));
}
...
struct to_node_fn {
  ...
  /// @return decltype(to_node(n, std::forward<T>(val))) void by default. User
  /// can set it to some other type.
  template <typename BasicNodeType, typename T>
...
// NOLINTNEXTLINE(misc-definitions-in-headers)
FK_YAML_INLINE_VAR constexpr const auto& to_node =
    detail::static_const<detail::to_node_fn>::value;
...
class node_value_converter {
 public:
  ...
  static auto from_node(BasicNodeType&& n, TargetType& val) noexcept(
      ...
      -> decltype(::fkyaml::from_node(std::forward<BasicNodeType>(n), val),
                  void()) {
...
class ordered_map : public std::vector<std::pair<const Key, Value>, Allocator> {
 ...
 public:
  ...
  /// @return mapped_type& Reference to a mapped_type object associated with the
  /// given key.
 ...
 public:
  ...
  /// @return std::pair<iterator, bool> A result of emplacement of the new
  /// key-value pair.
  ...
  /// @brief Find a value associated to the given key. Throws an exception if
  /// the search fails.
  /// @tparam KeyType A type for the input key.
  ...
  iterator find(
      KeyType&&
          key) noexcept {  // NOLINT(cppcoreguidelines-missing-std-forward)
    for (auto itr = this->begin(); itr != this->end(); ++itr) {
      if (m_compare(itr->first, key)) {
        return itr;
      }
  ...
  /// @tparam KeyType A type for the input key.
  /// @param key A key to find a value with.
  /// @return const_iterator The constant iterator for the found value, or the
  /// result of end().
  /// @sa https://fktn-k.github.io/fkYAML/api/ordered_map/find/
  template <typename KeyType,
            detail::enable_if_t<detail::is_usable_as_key_type<
  ...
  const_iterator find(KeyType&& key)
      const noexcept {  // NOLINT(cppcoreguidelines-missing-std-forward)
    for (auto itr = this->begin(); itr != this->end(); ++itr) {
      if (m_compare(itr->first, key)) {
        return itr;
      }
...
class basic_node {
 public:
  ...
  /// @note std::unordered_map is not supported since it does not allow
  /// incomplete types.
  ...
  using const_reverse_iterator =
      fkyaml::detail::reverse_iterator<const_iterator>;

  /// @brief A helper alias to determine converter type for the given target
  /// native data type.
  ...
  /// @brief A type for constant mapping range objects for the map_items()
  /// function.
  /// @sa https://fktn-k.github.io/fkYAML/api/basic_node/map_range/
 ...
 private:
  ...
  using initializer_list_t =
      std::initializer_list<detail::node_ref_storage<basic_node>>;

  /// @brief The actual storage for a YAML node value of the @ref basic_node
  /// class.
  /// @details This union combines the different storage types for the YAML
  /// value types defined in @ref node_t.
  /// @note Container types are stored as pointers so that the size of this
  /// union will not exceed 64 bits by default.
  union node_value {
    /// @brief Constructs a new basic_node Value object for null types.
    node_value() = default;

    /// @brief Constructs a new basic_node value object with a node type. The
    /// default value for the specified type will be assigned.
    ...
    /// @brief Destroys the existing Node value. This process is recursive if
    /// the specified node type is for containers.
    ...
    sequence_type* p_seq;
    /// A pointer to the value of mapping type. This pointer is also used when
    /// node type is null.
 ...
 public:
  ...
  basic_node(const basic_node& rhs)
    ...
    if FK_YAML_LIKELY (!has_anchor_name()) {
      switch (m_attrs & detail::node_attr_mask::value) {
        case detail::node_attr_bits::seq_bit:
          m_value.p_seq =
              detail::create_object<sequence_type>(*(rhs.m_value.p_seq));
          break;
        case detail::node_attr_bits::map_bit:
          m_value.p_map =
              detail::create_object<mapping_type>(*(rhs.m_value.p_map));
          break;
        case detail::node_attr_bits::null_bit:
        ...
        case detail::node_attr_bits::string_bit:
          m_value.p_str =
              detail::create_object<string_type>(*(rhs.m_value.p_str));
          break;
        default:                  // LCOV_EXCL_LINE
          detail::unreachable();  // LCOV_EXCL_LINE
      }
  ...
  /// @tparam CompatibleType Type of native data which is compatible with node
  /// values.
  /// @tparam U Type of compatible native data without cv-qualifiers and
  /// reference.
  ...
  template <typename CompatibleType,
            ...
            detail::enable_if_t<
                ...
                int> = 0>
  basic_node(CompatibleType&& val) noexcept(noexcept(
      ConverterType<U, void>::to_node(std::declval<basic_node&>(),
                                      std::declval<CompatibleType>()))) {
  ...
  template <
      ...
      detail::enable_if_t<
          detail::is_node_ref_storage<NodeRefStorageType>::value, int> = 0>
    ...
    bool is_mapping =
        std::all_of(init.begin(), init.end(),
                    [](const detail::node_ref_storage<basic_node>& node_ref) {
                      // Do not use is_sequence_impl() since node_ref may be an
                      // anchor or alias.
  ...
  ~basic_node() noexcept  // NOLINT(bugprone-exception-escape)
  {
    if (m_attrs & detail::node_attr_mask::anchoring) {
      if (m_attrs & detail::node_attr_bits::anchor_bit) {
        ...
        itr->second.m_value.destroy(itr->second.m_attrs &
 ...
 public:
  /// @brief Deserialize the first YAML document in the input into a basic_node
  /// object.
  /// @tparam InputType Type of a compatible input.
  /// @param[in] input An input source in the YAML format.
  /// @return The resulting basic_node object deserialized from the input
  /// source.
  ...
  /// @brief Deserialize the first YAML document in the input ranged by the
  ...
  /// sequence.
  /// @return The resulting basic_node object deserialized from the pair of
  /// iterators.
  /// @sa https://fktn-k.github.io/fkYAML/api/basic_node/deserialize/
  template <typename ItrType>
  ...
  /// @brief Deserialize all YAML documents in the input into basic_node
  /// objects.
  /// @tparam InputType Type of a compatible input.
  /// @param[in] input An input source in the YAML format.
  ...
  /// @brief Deserialize all YAML documents in the input ranged by the iterators
  /// into basic_node objects.
  /// @tparam ItrType Type of a compatible iterator.
  /// @param[in] begin An iterator to the first element of an input sequence.
  /// @param[in] end An iterator to the past-the-last element of an input
  /// sequence.
  /// @return The resulting basic_node objects deserialized from the pair of
  /// iterators.
  /// @sa https://fktn-k.github.io/fkYAML/api/basic_node/deserialize_docs/
  template <typename ItrType>
  static std::vector<basic_node> deserialize_docs(ItrType&& begin,
                                                  ItrType&& end) {
    return deserializer_type().deserialize_docs(detail::input_adapter(
        std::forward<ItrType>(begin), std::forward<ItrType>(end)));
  }

  /// @brief Serialize a basic_node object into a string.
  /// @param[in] node A basic_node object to be serialized.
  /// @return The resulting string object from the serialization of the given
  /// node.
  ...
  /// @brief A factory method for sequence basic_node objects without
  /// sequence_type objects.
  ...
  /// @brief A factory method for sequence basic_node objects with lvalue
  /// sequence_type objects.
  ...
  /// @brief A factory method for sequence basic_node objects with rvalue
  /// sequence_type objects.
  ...
  /// @brief A factory method for mapping basic_node objects without
  /// mapping_type objects.
  ...
  /// @brief A factory method for mapping basic_node objects with lvalue
  /// mapping_type objects.
  ...
  /// @brief A factory method for mapping basic_node objects with rvalue
  /// mapping_type objects.
  ...
  /// @brief A factory method for alias basic_node objects referencing the given
  /// anchor basic_node object.
  ...
  static basic_node alias_of(const basic_node& anchor_node) {
    constexpr detail::node_attr_t anchor_bit =
        detail::node_attr_bits::anchor_bit;

    if FK_YAML_UNLIKELY (!anchor_node.has_anchor_name() ||
                         !(anchor_node.m_attrs & anchor_bit)) {
 ...
 public:
  ...
  /// @brief A subscript operator of the basic_node class with a key of a
  /// compatible type with basic_node.
  /// @tparam KeyType A key type compatible with basic_node
  /// @param key A key to the target value in a sequence/mapping node.
  /// @return The value associated with the given key, or a default basic_node
  ...
  basic_node& operator[](KeyType&& key) {
    ...
    if FK_YAML_UNLIKELY (act_node.is_scalar_impl()) {
      throw fkyaml::type_error("operator[] is unavailable for a scalar node.",
                               get_type());
    }
    ...
    if (act_node.is_sequence_impl()) {
      ...
      if FK_YAML_UNLIKELY (!key_node.is_integer()) {
        throw fkyaml::type_error(
            "An argument of operator[] for sequence nodes must be an integer.",
            get_type());
      }
  ...
  /// @brief A subscript operator of the basic_node class with a basic_node key
  /// object.
  /// @tparam KeyType A key type which is a kind of the basic_node template
  /// class.
  /// @param key A key to the target value in a sequence/mapping node.
  /// @return The value associated with the given key, or a default basic_node
  /// object associated with the given key.
  /// @sa https://fktn-k.github.io/fkYAML/api/basic_node/operator[]/
  template <typename KeyType,
            detail::enable_if_t<detail::is_basic_node<KeyType>::value, int> = 0>
  basic_node& operator[](KeyType&& key) {
    ...
    if (is_sequence()) {
      ...
      return node_value.p_seq->operator[](
          std::forward<KeyType>(key).template get_value<int>());
    }
  ...
  bool operator==(const basic_node& rhs) const noexcept {
    ...
    const detail::node_attr_t lhs_val_bit =
        lhs.m_attrs & detail::node_attr_mask::value;
    ...
    switch (lhs_val_bit) {
      ...
      case detail::node_attr_bits::int_bit:
        ...
        break;
      case detail::node_attr_bits::float_bit:
        ret = (std::abs(lhs.m_value.float_val - act_rhs.m_value.float_val) <
  ...
  bool operator<(const basic_node& rhs) const noexcept {
    ...
    const detail::node_attr_t lhs_val_bit =
        lhs.m_attrs & detail::node_attr_mask::value;
    const detail::node_attr_t rhs_val_bit =
        act_rhs.m_attrs & detail::node_attr_mask::value;
 ...
 public:
  ...
  bool is_null() const noexcept { return resolve_reference().is_null_impl(); }
  ...
  /// @brief Tests whether the current basic_node value (sequence, mapping,
  /// string) is empty.
  ...
  bool empty() const {
    ...
    switch (act_node.m_attrs & detail::node_attr_mask::value) {
      ...
      default:
        throw fkyaml::type_error("The target node is not of a container type.",
                                 get_type());
    }
  }

  /// @brief Returns the size of the current basic_node value (sequence,
  /// mapping, string).
  ...
  /// @brief Check whether this basic_node object has a given key in its inner
  /// mapping node value.
  /// @tparam KeyType A key type compatible with basic_node.
  /// @param key A key to the target value in the mapping node value.
  /// @return true if the target node is a mapping and has the given key, false
  /// otherwise.
  ...
  template <typename KeyType,
            detail::enable_if_t<
                detail::disjunction<
                    ...
                    detail::is_node_compatible_type<
                        basic_node, detail::remove_cvref_t<KeyType>>>::value,
                int> = 0>
  ...
  /// @tparam KeyType A key type compatible with basic_node
  /// @param key A key to the target basic_node object in a sequence/mapping
  /// node.
  ...
  basic_node& at(KeyType&& key) {
    ...
    if FK_YAML_UNLIKELY (act_node.is_scalar_impl()) {
      throw fkyaml::type_error("at() is unavailable for a scalar node.",
                               get_type());
    }
    ...
    if (act_node.is_sequence_impl()) {
      if FK_YAML_UNLIKELY (!node_key.is_integer_impl()) {
        throw fkyaml::type_error(
            "An argument of at() for sequence nodes must be an integer.",
            get_type());
      }
  ...
  /// @param key A key to the target basic_node object in a sequence/mapping
  /// node.
  /// @return Constant reference to the basic_node object associated with the
  /// given key.
  /// @sa https://fktn-k.github.io/fkYAML/api/basic_node/at/
  template <
      typename KeyType,
  ...
  /// @return The YAML version if already set, `yaml_version_type::VERSION_1_2`
  /// otherwise.
  ...
  yaml_version_type get_yaml_version_type() const noexcept {
    return mp_meta->is_version_specified ? mp_meta->version
                                         : yaml_version_type::VERSION_1_2;
  }
  ...
  /// @return The YAML version if already set, `yaml_version_t::VER_1_2`
  /// otherwise.
  ...
  /// @brief Set the YAML version for this basic_node object.
  /// @deprecated Use set_yaml_version_type(yaml_version_type) function. (since
  /// 0.3.12)
  /// @param[in] version The target YAML version.
  /// @sa https://fktn-k.github.io/fkYAML/api/basic_node/set_yaml_version/
  FK_YAML_DEPRECATED(
      "Since 0.3.12; Use set_yaml_version_type(const yaml_version_type)")
  ...
  /// @brief Check whether this basic_node object has already had any anchor
  /// name.
  ...
  bool has_anchor_name() const noexcept {
    return (m_attrs & detail::node_attr_mask::anchoring) &&
           !m_prop.anchor.empty();
  }

  /// @brief Get the anchor name associated with this basic_node object.
  /// @note Some anchor name must be set before calling this method. Call
  /// has_anchor_name() to see if this basic_node object has any anchor name.
  ...
  /// @note If this basic_node object has already had any anchor name, the new
  /// anchor name will overwrite the old one.
  ...
  void add_anchor_name(const std::string& anchor_name) {
    ...
    mp_meta = p_meta;
    const auto offset =
        static_cast<uint32_t>(mp_meta->anchor_table.count(anchor_name) - 1);
  ...
  bool has_tag_name() const noexcept { return !m_prop.tag.empty(); }

  /// @brief Get the tag name associated with this basic_node object.
  /// @note Some tag name must be set before calling this method. Call
  /// has_tag_name() to see if this basic_node object has any tag name.
  ...
  /// @note If this basic_node object has already had any tag name, the new tag
  /// name will overwrite the old one.
  /// @param[in] tag_name A tag name to get associated with this basic_node
  /// object.
  /// @sa https://fktn-k.github.io/fkYAML/api/basic_node/add_tag_name/
  void add_tag_name(const std::string& tag_name) { m_prop.tag = tag_name; }
  ...
  /// @note This function requires T objects to be default constructible. Also,
  /// T cannot be either a reference, pointer or C-style array type.
  /// @tparam T A compatible value type which may be cv-qualified.
  /// @tparam ValueType A compatible value type (T without cv-qualifiers by
  /// default).
  ...
  template <
      ...
      detail::enable_if_t<
          detail::conjunction<std::is_default_constructible<ValueType>,
                              detail::negation<std::is_pointer<T>>>::value,
          int> = 0>
  T get_value() const noexcept(noexcept(
      std::declval<const basic_node&>().template get_value_impl<ValueType>(
          std::declval<ValueType&>()))) {
    // emit a compile error if T is either a reference, pointer or C-style array
    // type.
    static_assert(!std::is_reference<T>::value,
                  "get_value() cannot be called with reference types. "
                  "You might want to call one of as_seq(), as_map(), "
                  "as_bool(), as_int(), as_float() or as_str().");
    static_assert(!std::is_array<T>::value,
                  "get_value() cannot be called with C-style array types. You "
                  "might want to call get_value_inplace().");
  ...
  /// @brief Get the node value object converted into a given type. The
  /// conversion result is filled into `value_ref`.
  ...
  void get_value_inplace(T& value_ref) const
      noexcept(noexcept(std::declval<const basic_node&>()
                            .template get_value_impl<T>(std::declval<T&>()))) {
    resolve_reference().get_value_impl(value_ref);
  }

  /// @brief Get the node value object converted to a given type. If the
  /// conversion fails, this function returns a given default value instead.
  /// @note This function requires T to be default constructible. Also, T cannot
  /// be either a reference, pointer or C-style array type.
  /// @tparam T A compatible value type which may be cv-qualified.
  ...
  /// @return A value converted from this basic_node object if conversion
  /// succeeded, the given default value otherwise.
  /// @sa https://fktn-k.github.io/fkYAML/api/basic_node/get_value_or/
  template <
      typename T, typename U,
      detail::enable_if_t<
          detail::conjunction<std::is_constructible<T, U>,
  ...
  T get_value_or(U&& default_value) const noexcept {
    static_assert(!std::is_reference<T>::value,
                  ...
                  "as_bool(), as_int(), as_float() or as_str().");
    static_assert(!std::is_array<T>::value,
                  "get_value_or() cannot be called with C-style array types. "
                  "You might want to call get_value_inplace().");

    // TODO:
    // Ideally, there should be no exception thrown in this kind of function.
    // However, achieving that would require a lot of refactoring and/or some
    // API changes, especially `from_node` interface definition. So, try-catch
    // is used instead for now.
    try {
      return get_value<T>();
    } catch (const std::exception& /*unused*/) {
      // Any exception derived from std::exception is interpreted as a
      // conversion failure in some way since user-defined from_node function
      // may throw a different object from a fkyaml::type_error. and
      // std::exception is usually the base class of user-defined exception
      // types.
  ...
  FK_YAML_DEPRECATED(
      "Since 0.4.3; Use one of as_seq(), as_map(), as_bool(), as_int(), "
      "as_float() or as_str()")
  ReferenceType get_value_ref() {
    return get_value_ref_impl(
        static_cast<detail::add_pointer_t<ReferenceType>>(nullptr));
  }

  /// @brief Explicit reference access to the internally stored YAML node value.
  /// @tparam ReferenceType Constant reference type to the target YAML node
  /// value.
  ...
  sequence_type& as_seq() {
    basic_node& act_node =
        resolve_reference();  // NOLINT(misc-const-correctness)
  ...
  /// @brief Swaps the internally stored data with the specified basic_node
  /// object.
  ...
  /// @brief Returns an iterator to the first element of a container node
  /// (sequence or mapping).
  /// @throw `type_error` if this basic_node is neither a sequence nor mapping
  /// node.
  ...
  iterator begin() {
    ...
    switch (act_node.m_attrs & detail::node_attr_mask::value) {
      ...
      default:
        throw fkyaml::type_error(
            "The target node is neither of sequence nor mapping types.",
            get_type());
    }
  }

  /// @brief Returns a const iterator to the first element of a container node
  /// (sequence or mapping).
  /// @throw `type_error` if this basic_node is neither a sequence nor mapping
  /// node.
  ...
  /// @brief Returns an iterator to the past-the-last element of a container
  ...
  /// @brief Returns a const iterator to the past-the-last element of a
  ...
  /// @brief Returns an iterator to the reverse-beginning (i.e., last) element
  ...
  /// @brief Returns a const iterator to the reverse-beginning (i.e., last)
  /// element of a container node (sequence or mapping).
  /// @throw `type_error` if this basic_node is neither a sequence nor mapping
  /// node.
  /// @return A const iterator to the reverse-beginning element of a container
  /// node.
  /// @sa https://fktn-k.github.io/fkYAML/api/basic_node/rbegin/
  ...
  /// @brief Returns an iterator to the reverse-end (i.e., one before the first)
  /// element of a container node (sequence or mapping).
  /// @throw `type_error` if this basic_node is neither a sequence nor mapping
  /// node.
  ...
  /// @brief Returns a const iterator to the reverse-end (i.e., one before the
  /// first) element of a container node (sequence or mapping).
  /// @throw `type_error` if this basic_node is neither a sequence nor mapping
  /// node.
  ...
  map_range map_items() {
    if FK_YAML_UNLIKELY (!is_mapping()) {
      throw type_error("map_items() cannot be called on a non-mapping node.",
                       get_type());
    }
    return {*this};
  }
 ...
 private:
  /// @brief Resolves anchor/alias reference and returns reference to an actual
  /// value node.
  ...
  /// @brief Resolves anchor/alias reference and returns const reference to an
  /// actual value node.
  ...
  template <
      typename ValueType,
      detail::enable_if_t<
          detail::negation<detail::is_basic_node<ValueType>>::value, int> = 0>
  void get_value_impl(ValueType& v) const
      noexcept(noexcept(ConverterType<ValueType, void>::from_node(
          std::declval<const basic_node&>(), v))) {
  ...
  /// @throw fkyaml::exception The node value is not a sequence.
  /// @return Constant reference to the sequence node value.
  const sequence_type& get_value_ref_impl(
      const sequence_type* /*unused*/) const {
    return as_seq();
  }
  ...
  const float_number_type& get_value_ref_impl(
      const float_number_type* /*unused*/) const {
    return as_float();
  }
  ...
  /// @return Reference to the string node value.
  string_type& get_value_ref_impl(string_type* /*unused*/) { return as_str(); }
...
template <template <typename, typename...> class SequenceType,
          ...
          typename StringType,
          template <typename, typename = void> class ConverterType>
inline void swap(basic_node<SequenceType, MappingType, BooleanType, IntegerType,
                            FloatNumberType, StringType, ConverterType>& lhs,
                 basic_node<SequenceType, MappingType, BooleanType, IntegerType,
...
/// @brief Insertion operator for basic_node template class. A wrapper for the
...
inline std::ostream& operator<<(
    std::ostream& os,
    const basic_node<SequenceType, MappingType, BooleanType, IntegerType,
                     FloatNumberType, StringType, ConverterType>& n) {
  os << basic_node<SequenceType, MappingType, BooleanType, IntegerType,
                   FloatNumberType, StringType, ConverterType>::serialize(n);
  return os;
}

/// @brief Extraction operator for basic_node template class. A wrapper for the
/// deserialization feature with input streams.
...
inline std::istream& operator>>(
    ...
    basic_node<SequenceType, MappingType, BooleanType, IntegerType,
               FloatNumberType, StringType, ConverterType>& n) {
  n = basic_node<SequenceType, MappingType, BooleanType, IntegerType,
                 FloatNumberType, StringType, ConverterType>::deserialize(is);
  return is;
}
...
/// @brief The user-defined string literal which deserializes a `char` array
/// into a `node` object.
...
/// @brief The user-defined string literal which deserializes a `char16_t` array
/// into a `node` object.
...
/// @brief The user-defined string literal which deserializes a `char32_t` array
/// into a `node` object.
...
#if FK_YAML_HAS_CHAR8_T
/// @brief The user-defined string literal which deserializes a `char8_t` array
/// into a `node` object.
...
// NOLINTNEXTLINE(cert-dcl58-cpp)
struct hash<
    fkyaml::basic_node<SequenceType, MappingType, BooleanType, IntegerType,
                       FloatNumberType, StringType, ConverterType>> {
  using node_t =
      fkyaml::basic_node<SequenceType, MappingType, BooleanType, IntegerType,
                         FloatNumberType, StringType, ConverterType>;
  ...
  std::size_t operator()(const node_t& n) const {
    ...
    switch (type) {
      ...
      case fkyaml::node_type::BOOLEAN:
        hash_combine(seed, std::hash<boolean_type>()(
                               n.template get_value<boolean_type>()));
        return seed;
      case fkyaml::node_type::INTEGER:
        hash_combine(seed, std::hash<integer_type>()(
                               n.template get_value<integer_type>()));
        return seed;
      case fkyaml::node_type::FLOAT:
        hash_combine(seed, std::hash<float_number_type>()(
                               n.template get_value<float_number_type>()));
        return seed;
      case fkyaml::node_type::STRING:
        hash_combine(seed, std::hash<string_type>()(
                               n.template get_value<string_type>()));
        return seed;
      default:                          // LCOV_EXCL_LINE
