...
void Myconfig::load_config() {
  ...
  uint64_t l1_size = Utils::convert_size_str(yamlconfig["L1"]["size"].as<string>());
...
//    std::cout << yamlconfig["L1"]["path"][i].as<string>() << "\n";
  }
...
//    ofstream ofs(l1_miss_output_path.back(), std::ofstream::out | std::ofstream::trunc | std::ofstream::binary);
//    l1_miss_ofs.push_back(ofs);
  }
  l2_trace_path = output_path + "/l2.trace";
//  l2_trace_ofs.open(l2_trace_path, std::ofstream::out | std::ofstream::trunc | std::ofstream::binary);

  l2_mrc_output_path = output_path + "/l2.mrc";
//  l2_mrc_ofs.open(l2_mrc_output_path, std::ofstream::out | std::ofstream::trunc);
