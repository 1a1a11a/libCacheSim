...
ring_t *ch_ring_create_ring(int n_server, double *weight) {
  ...
  for (i = 0; i < n_server; i++) {
    ...
    if (weight != nullptr)
      ks = (unsigned int) floorf(weight[i] * (float) n_server * (N_VNODE_PER_SERVER / 4) );
...
//        printf("%d %d %d %d %u %u\n", i, k, h, cnt, vnodes[cnt].point, vnodes[cnt].server_id);
        cnt++;
      }
    }
  }
...
void ch_ring_get_servers(const char *const key, const ring_t * const ring, const unsigned int n, unsigned int *idxs) {
  ...
  while (i < n) {
    unsigned int server_id = vnodes[(start_vnode_idx + vnode_pos) % (ring->n_point)].server_id;
    ...
    if (vnode_pos > ring->n_point){
      printf("ERROR: searched all points on the consistent hash ring, but cannot find enough servers\n");
      abort();
    }
  }
}
