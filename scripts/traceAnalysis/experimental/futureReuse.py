...
def _load_future_reuse_data(datapath):
    ...
    assert desc_line_true in desc_line, "the input file might not be future_reuse data file, desc line " + desc_line + " data " + datapath
    ...
    for line in ifile:
        ...
        reuse_cnt, stop_reuse_cnt, reuse_access_age_sum, stop_reuse_access_age_sum, \
                reuse_freq_sum, stop_reuse_freq_sum = [int(i) for i in data_str.split(",")]
    ...
    return reuse_cnt_dict, stop_reuse_cnt_dict, reuse_access_age_sum_dict, stop_reuse_access_age_sum_dict, reuse_freq_sum_dict, stop_reuse_freq_sum_dict 
...
def plot_future_reuse(datapath_list, figname_prefix=""):
    ...
    for datapath in datapath_list:
        reuse_cnt_dict, stop_reuse_cnt_dict, reuse_access_age_sum_dict, stop_reuse_access_age_sum_dict, reuse_freq_sum_dict, stop_reuse_freq_sum_dict = _load_future_reuse_data(datapath) 
    ...
    plt.xlabel("Age (hour)")
    plt.savefig("{}/{}_noReuse_rt.{}".format(FIG_DIR, figname_prefix, FIG_TYPE), bbox_inches="tight")

    plt.xscale("log")
    plt.xticks(np.array([5, 60, 300, 3600, 86400, 86400*2, 86400*4])/3600, ["5 sec", "1 min", "5 min", "1 hour", "1 day", "", "4 day"], rotation=28)
