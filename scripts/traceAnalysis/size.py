...
def _load_size_data(datapath: str) -> Tuple[dict, dict]:
    ...
    assert "# object_size: req_cnt" in desc_line or "# object_size: freq" in desc_line, \
        "the input file might not be size data file, desc line " + desc_line + " data " + datapath
...
def plot_size_distribution(datapath: str, figname_prefix: str = ""):
    ...
    plt.savefig("{}/{}_size.{}".format(FIG_DIR, figname_prefix, FIG_TYPE),
                bbox_inches="tight")

    plt.xscale("log")
    plt.savefig("{}/{}_size_log.{}".format(FIG_DIR, figname_prefix, FIG_TYPE),
                bbox_inches="tight")
    plt.clf()

    logger.info("plot saved to {}/{}_size.{} {}/{}_size.{}".format(
