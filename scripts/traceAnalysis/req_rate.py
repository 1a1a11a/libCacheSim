...
def plot_req_rate(datapath: str, figname_prefix: str = ""):
    ...
    plt.grid(linestyle="--")
    plt.savefig("{}/{}_reqRate.{}".format(FIG_DIR, figname_prefix, FIG_TYPE),
                bbox_inches="tight")
    plt.clf()
    logger.info("request rate saved as {}/{}_reqRate.{}".format(
    ...
    plt.grid(linestyle="--")
    plt.savefig("{}/{}_byteRate.{}".format(FIG_DIR, figname_prefix, FIG_TYPE),
                bbox_inches="tight")
    plt.clf()
    logger.info("byte request rate saved as {}/{}_byteRate.{}".format(
        FIG_DIR, figname_prefix, FIG_TYPE))

    plt.plot([i * time_window / 3600 for i in range(len(obj_rate_list))],
             obj_rate_list,
             label="object",
             color=COLOR[0],
             linestyle="-",
             linewidth=1)
    plt.plot([i * time_window / 3600 for i in range(len(new_obj_rate_list))],
             new_obj_rate_list,
             label="new object",
             color=COLOR[1],
             linestyle="--",
    ...
    plt.grid(linestyle="--")
    plt.legend()
    plt.savefig("{}/{}_objRate.{}".format(FIG_DIR, figname_prefix, FIG_TYPE),
                bbox_inches="tight")
    plt.clf()
    logger.info("object request rate saved as {}/{}_objRate.{}".format(
        FIG_DIR, figname_prefix, FIG_TYPE))
