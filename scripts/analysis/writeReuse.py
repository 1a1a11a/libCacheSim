...
def _load_write_reuse_data(datapath):
    ...
    for line in ifile:
        if line[0] == "#" and "no reuse" in line:
            ...
            read_cnt, sum_read_freq, write_cnt, remove_cnt, first_read_cnt = [int(i) for i in data_list.split(",")]
...
def plot_write_reuse(datapath, figname_prefix=""):
    ...
    req_cnts = [read_reuse_req_cnt, write_reuse_req_cnt, remove_reuse_req_cnt, ]
    ...
    plt.savefig("{}/{}_writeReuse_rt_log.{}".format(FIG_DIR, figname_prefix, FIG_TYPE), bbox_inches="tight")
    plt.clf()
