...
typedef void (*evict_func_ptr_t)(cache_t *, request_t *, cache_obj_t* cache_obj);
...
static inline gboolean _add_to_slab(slab_t *slab, request_t *req, slab_cache_obj_t *cache_obj) {
  slab->slab_items[slab->n_stored_items++] = GSIZE_TO_POINTER(cache_obj->obj_id_int);
...
//  printf("allocate to %d global %d\n", id, g_queue_get_length(slab_params->global_slab_q));
