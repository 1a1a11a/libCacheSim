...
cache_t *slabLRU_init(common_cache_params_t ccache_params, void *cache_specific_init_params) {
  ...
  slabLRU_params->hashtable = create_hash_table_with_obj_id_type(OBJ_ID_NUM, NULL, free_slab_cache_obj, g_free, free_slab_cache_obj);
  slabLRU_params->slab_params.global_slab_q = g_queue_new();
  slabLRU_params->slab_params.slab_size = MiB;
...
cache_check_result_t slabLRU_get(cache_t *cache, request_t *req){
  cache_check_result_t cache_check = slabLRU_check(cache, req, true);
...
void _slabLRU_insert(cache_t *cache, request_t *req){
  ...
  g_hash_table_insert(slabLRU_params->hashtable, GSIZE_TO_POINTER(req->obj_id_int), (gpointer) cache_obj);
}

void _slabLRU_update(cache_t *cache, request_t *req){
  slabLRU_params_t *slabLRU_params = (slabLRU_params_t *) (cache->cache_params);
  slab_cache_obj_t *cache_obj = g_hash_table_lookup(slabLRU_params->hashtable, GSIZE_TO_POINTER(req->obj_id_int));
  slab_t *slab = (slab_t*) cache_obj->slab;
...
void _slabLRU_evict(cache_t *cache, request_t *req, cache_obj_t* evicted_obj){
  UNUSED(evicted_obj);
  ...
  for (guint32 i=0; i<slab_to_evict->n_stored_items; i++){
    slab_cache_obj_t *cache_obj = (slab_cache_obj_t *) g_hash_table_lookup(slabLRU_params->hashtable, GSIZE_TO_POINTER(slab_to_evict->slab_items[i]));
    g_hash_table_remove(slabLRU_params->hashtable, GSIZE_TO_POINTER(cache_obj->obj_id_int));
...
void slabLRU_remove_obj(cache_t *cache, void *obj_id_to_remove){
  ...
  slab_cache_obj_t *last_cache_obj = (slab_cache_obj_t *) g_hash_table_lookup(slabLRU_params->hashtable, GSIZE_TO_POINTER(slab->slab_items[cache_obj->item_pos_in_slab]));
