...
/* recording table size, unit: percentage of cacheAlgo size */
...
typedef struct {
  /* obj_id_type of cacheAlgo, LRU, FIFO, Optimal, AMP */
  char *cache_type;
  ...
  /** size of block
   *  this should be equal to block_unit_size in cacheAlgo->core
   **/
  guint64 block_size;
...
typedef struct {
  /* the underlying cacheAlgo, like LRU, LFU, AMP */
  cache_t *cache;
...
/**
 check whether an block/obj is in the cacheAlgo

 @param Mithril the cacheAlgo struct
...
/**
 add a request to cacheAlgo, if it is in the cacheAlgo, update it
 if not, insert into cacheAlgo, this is the upper level API
 return TRUE if request is in the cacheAlgo
...
/**
 update the obj/block in the cacheAlgo,
 it is used internally during a cacheAlgo hit
...
/**
 insert the obj/block into cacheAlgo
 it is used internally during a cacheAlgo miss
...
/**
 evict one obj/block from cacheAlgo
 it is used internally when cacheAlgo is full
...
/**
 the free function for cacheAlgo struct
...
/**
 ...
 Mithril_destroy_uniq only destroy uniq struct related to current cacheAlgo,
...
/**
 get current cacheAlgo size usage

 @param cache cacheAlgo struct
 @return current size
 */
