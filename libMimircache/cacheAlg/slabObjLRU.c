 ...
 *
 * slabObjLRU_init --
 *      initialize a slabObjLRU cacheAlg
 *
 * Input:
 *      size:       cacheAlg size
...
//  cacheAlg->core->cache_init = slabObjLRU_init;
//  cacheAlg->core->destroy = slabObjLRU_destroy;
//  cacheAlg->core->destroy_unique = slabObjLRU_destroy_unique;
//  cacheAlg->core->add = slabObjLRU_add;
//  cacheAlg->core->check = slabObjLRU_check;
//  cacheAlg->core->_insert = _slabObjLRU_insert;
//  cacheAlg->core->_update = _slabObjLRU_update;
//  cacheAlg->core->_evict = _slabObjLRU_evict;
//  cacheAlg->core->evict_with_return = _slabObjLRU_evict_with_return;
//  cacheAlg->core->get_current_size = slabObjLRU_get_size;
//  cacheAlg->core->remove_obj = slabObjLRU_remove_obj;
//  cacheAlg->core->cache_init_params = NULL;
//  cacheAlg->core->add_only = slabObjLRU_add_only;
//  cacheAlg->core->add_withsize = slabObjLRU_add_withsize;
...
void slabObjLRU_remove_obj(cache_t *cache, void *data_to_remove) {
  ...
  if (!data) {
    fprintf(stderr, "slabObjLRU_remove_obj: data to remove is not in the cacheAlg\n");
    exit(1);
  }
